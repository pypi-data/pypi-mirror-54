...
AuthCacheLimits = namedtuple('AuthCacheLimits', field_names=('soft_timeout', 'hard_timeout', 'reuse_limit'),
                             defaults=(DEFAULT_SOFT_TIMEOUT, DEFAULT_HARD_TIMEOUT, DEFAULT_REUSE_LIMIT))


AuthCacheState = namedtuple('AuthCacheState', field_names=('last_authenticated', 'last_used', 'reuse_count'),
                            defaults=(0, ))
...
class AuthenticationCache:
    def __init__(self, cache_file_path, limits=AuthCacheLimits()):
        ...
        self.log.debug("Initialized auth cache: %s; soft timeout: %s; hard timeout: %s; reuse limit: %s",
                       self.cache_file_path, self.limits.soft_timeout, self.limits.hard_timeout,
