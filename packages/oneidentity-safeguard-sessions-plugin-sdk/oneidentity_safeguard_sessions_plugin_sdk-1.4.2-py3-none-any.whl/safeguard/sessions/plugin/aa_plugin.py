...
"""
    ...
    def _extract_username(self):
        ...
        return (
                self.connection.gateway_user or
                self.connection.target_username or
                self.connection.key_value_pairs.get("username")
        )
    ...
    def _map_username_explicit(self):
        ...
        self.mfa_identity = UserMappingExplicit.map_username(
            self.mfa_identity,
            self.plugin_configuration,
            section="usermapping source=explicit"
    ...
    def _ldap_user_mapping(self):
        try:
            return UserMappingLdapServer.from_config(self.plugin_configuration,
                                                     section="usermapping source=ldap_server")
    ...
    def _ldap_group_whitelist(self):
        try:
            return LDAPServerGroupList.from_config(self.plugin_configuration,
                                                   section="whitelist source=ldap_server_group")
        except RequiredConfigurationSettingNotFound:
            return None
    ...
    def _ask_mfa_password(self):
        ...
        question = self.plugin_configuration.get(
            'auth',
            'prompt',
            default=self.MFA_PASSWORD_QUESTION
        )
    ...
    def _execute_authenticate(self):
        if not self._get_private('successful_authentication'):
            auth_verdict = self._execute_steps(
                checkpoints_key="authenticate_checkpoints",
                steps=self._authentication_steps(),
            )
    ...
    def _get_question(self, number):
        try:
            return (self.plugin_configuration.get('question_{}'.format(number), 'prompt', required=True) + ' ',
    ...
    def authorize(self, cookie, session_cookie, **kwargs):
        ...
        **Should not be modified.**
        ...
        self._setup_self(cookie, session_cookie, **kwargs)
        return self.finalize_hook(self._add_cookies(self._execute_steps(
            checkpoints_key="authorization_checkpoints",
            steps=self._authorization_steps()
        )))
    ...
    def session_ended(self, session_id, cookie, session_cookie):
        ...
        **Should not be modified.**
        ...
        return self.finalize_hook(self._execute_steps(
            checkpoints_key="session_ended_checkpoints",
            steps=self._session_ended_steps()
        ))
