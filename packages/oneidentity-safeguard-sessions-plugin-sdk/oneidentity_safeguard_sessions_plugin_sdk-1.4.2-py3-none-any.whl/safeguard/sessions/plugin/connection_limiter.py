...
class ConnectionLimiter:
    ...
    @classmethod
    def from_config(cls, plugin_config, keys, section='connection_limiter', plugin_context=None,
        ...
        state_file_path = os.path.join(
            plugin_context.ephemeral_plugin_state_directory,
            section,
            '_'.join(keys)
        )
    ...
    def try_connect(self, session_id):
        ...
        with lockfile_for(self.state_file_path, timeout=self.__lock_timeout, retries=self.__lock_retries):
            ...
            if len(connections) < self.limit:
                ...
                self.__logger.info("Connection exceeding limit."
                                   if self.limit_description is None else
                                   ("Connection exceeding limit for: %s" % self.limit_description))
                return False
