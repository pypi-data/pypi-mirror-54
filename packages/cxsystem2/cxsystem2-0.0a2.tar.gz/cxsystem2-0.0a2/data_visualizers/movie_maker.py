...
def spike_to_frame(groups, spikes, X_axis, Y_axis, w_coord, axis_precision, d_t = 0.001) :
    t_min = 0
    t_max = 0
    for group in groups:
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(os.path.join(movieMaker_dir,filename+'.gz')):
        ...
        all_frames,colors_,ts = spike_to_frame(groups, spikes, X_axis, Y_axis, w_coord, axis_precision)
        data_tosave = {'all_frames':all_frames,'colors_':colors_,'ts':ts}
        data_saver(os.path.join(movieMaker_dir,filename)+'.gz',data_tosave)
    else:
