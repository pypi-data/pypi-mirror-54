...
class MRequest(object):
    ...
    def dispatch_response(self, target):
        return Response(self.data, target)
...
def dispatch_multi(config, aws, member, raw_data):
    logging.info(aws.send_msg(member.endpoint, raw_data))
    logging.info('zone message dispached!')
    resp2 = aws.send_sns_to(member.arn, 'You have a new message')
    logging.info('push notification sended:%s with arn:%s', resp2, member.arn)
...
def process_message(config, message):
    ...
    if isinstance(message, dict):
        ...
        try:
            ...
            req = MRequest(raw, new_version=True, enc_raw=message.encode())
        except Exception as e:
            logging.error(formatError(e))
    return req
