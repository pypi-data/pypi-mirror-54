...
class ListenerUtils():
    ...
    def save_or_update_run(run_contract):
        if SdkAuthentication.isAuthorized:
            ...
            request = requests.post(Shield34Properties.api_base_url + '/report/save-run', data=payload, headers=headers, verify=False)
    ...
    def save_or_update_block_run(block_run_contract):
        if SdkAuthentication.isAuthorized:
            ...
            headers = {'content-type': 'application/json', 'Authorization': 'Shield34-Project ' + SdkAuthentication.get_user_token()}
            request = requests.post(Shield34Properties.api_base_url + '/report/save-block-run', data=payload, headers=headers, verify=False)
            if request.status_code == 200:
                content_as_json = request.json()
    ...
    def save_report_file(s3_file_details):
        if SdkAuthentication.isAuthorized:
            ...
            request = requests.post(Shield34Properties.api_base_url + '/report/save-report', data=payload, headers=headers, verify=False)
    ...
    def get_test_run_url(current_block_run):
        if SdkAuthentication.isAuthorized:
            ...
            headers = {'content-type': 'application/json', 'Authorization': 'Shield34-Project ' + SdkAuthentication.get_user_token()}
            request = requests.post(Shield34Properties.api_base_url + '/report/get-block-run-details', data=payload, headers=headers, verify=False)
            if request.status_code == 200:
                content_as_json = request.json()
    ...
    def write_browser_network_data_to_files(browser_network_data, path_to_save_files):
        ...
        for idx, browser_network in enumerate(converted_browser_network_data):
            ...
            utc_time = datetime.strptime(browser_network['startedDateTime'], "%Y-%m-%dT%H:%M:%S.%f%z")
