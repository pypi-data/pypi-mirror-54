...
class Workbook(ItemWithOwnerAndAcl):
    def __init__(self, definition=None, *, status=None, extra_workstep_tuples=None):
        ...
        if definition is not None and 'ID' in definition:
            self._pull(definition['ID'], extra_workstep_tuples=extra_workstep_tuples)
    ...
    def pull(item_id, *, status=None, extra_workstep_tuples=None):
        status = Status.validate(status)
        definition = Item._dict_from_id(item_id)
        item = Workbook(definition, status=status, extra_workstep_tuples=extra_workstep_tuples)
        item.specialize(item_id)
        return item
    ...
    def _pull(self, workbook_id, extra_workstep_tuples=None):
        ...
        for worksheet_id in worksheet_ids:
            ...
            self.worksheets.append(Worksheet(self, Item._dict_from_id(worksheet_id),
...
class Worksheet(Item):
    def __init__(self, workbook, definition=None, *, extra_workstep_tuples=None):
    ...
    def _pull(self, worksheet_id, extra_workstep_tuples=None):
        ...
        if extra_workstep_tuples:
            for workbook_id, worksheet_id, workstep_id in extra_workstep_tuples:
                if workbook_id == self.workbook.id and worksheet_id == self.id:
                    workstep_ids_to_pull.add(workstep_id)
    ...
    def push(self, workbook_id, item_map, label=None):
        ...
        items_api.set_properties(id=worksheet_id, body=props)
