...
def pull(items, *, start=None, end=None, grid='15min', header='__auto__', group_by=None, capsules_as='auto',
         tz_convert=None, calculation=None, bounding_values=False, return_status_df=False, errors='raise', quiet=False):
    """
            ...
            for index, item_name, series in list(pulled_series.queue):
                ...
                if group_by is None:
                    ...
                    if not isinstance(group_by, list):
                        raise RuntimeError('groupby argument must be a str or list(str)')
                    if not all(col in query_df.columns for col in group_by):
                        raise RuntimeError('groupby columns %s not present in query DataFrame' % group_by)
...
def _pull_condition(capsules_as, formulas_api, formula, parameters, item_id, item_name, output_df, pulled_series,
                    pd_start, pd_end, tz, column_names, index, status_df, _drain_status_updates):
    # noinspection PyBroadException
    try:
        ...
        while True:
            ...
            if capsules_as == 'signals':
                columns[item_name] = pd.Series(0, output_df.index)
                for capsule in capsules_output.capsules:
                    ...
                    for prop in capsule.properties:  # type: ScalarPropertyV1
                        ...
                        if colname not in columns:
                            columns[colname] = pd.Series(np.nan, output_df.index)
            ...
            status_df.at[index, 'Result'] = 'Pulling %s' % _common.convert_to_timestamp(current_start, tz)
            status_df.at[index, 'Count'] = capsule_count
            status_df.at[index, 'Time'] = _common.timer_elapsed(timer)

            _drain_status_updates()

        status_df.at[index, 'Result'] = 'Success'
        status_df.at[index, 'Count'] = capsule_count
        status_df.at[index, 'Time'] = _common.timer_elapsed(timer)
