        ...
        try:
            ...
            if include_referenced_workbooks and _common.get(row, 'Workbook Type') == 'Topic':
                for workbook_id, workstep_tuple in workbook.referenced_workbooks.items():
                    if workbook_id not in analyses_to_pull:
                        analyses_to_pull[workbook_id] = set()
                    ...
                    if workstep_tuple not in analyses_to_pull[workbook_id]:
                        analyses_to_pull[workbook_id].update(workstep_tuple)
