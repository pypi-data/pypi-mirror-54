...
class Folder(ItemWithOwnerAndAcl):
    ...
    def push(self, parent_folder_id, datasource_maps, item_map, *, owner_id=None, to_original_owner=False, label=None,
        ...
        item_map[self.id.upper()] = folder_item.id.upper()
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def _pull(self, workbook_id, extra_workstep_tuples=None):
        ...
        self.definition['Original Server URL'] = _login.client.host.replace('/api', '')
    ...
    def push(self, *, owner_id=None, to_original_owner=False, folder_id=None, item_map=None, label=None,
        ...
        if not workbook_item:
            ...
            workbook_item = workbooks_api.create_workbook(body=workbook_input)  # type: WorkbookOutputV1

            data_id = Workbook._data_id_from_item_id(label, self.id)
            items_api.set_properties(id=workbook_item.id, body=[
                ScalarPropertyV1(name='Datasource Class', value=_common.DEFAULT_DATASOURCE_CLASS),
                ScalarPropertyV1(name='Datasource ID', value=_common.DEFAULT_DATASOURCE_ID),
                ScalarPropertyV1(name='Data ID', value=data_id)])
            ...
            if folder_id is not None:
                folders_api.move_item_to_folder(folder_id=folder_id, item_id=workbook_item.id)

        self.status.df.loc[self.status.df['ID'] == self.id, 'Pushed Workbook ID'] = workbook_item.id

        if item_map is None:
            item_map = dict()

        item_map[self.id.upper()] = workbook_item.id.upper()

        if replace_acl:
            self._push_acl(workbook_item.id, self.datasource_maps, item_map, replace=True, strict=False)
            ...
            for item_id, item in dict_iterator.items():
                ...
                try:
                    item.push(self.datasource_maps,
                              pushed_workbook_id=workbook_item.id, item_map=item_map,
        ...
        for worksheet in self.worksheets:
            ...
            worksheet.push(workbook_item.id, item_map, label=label)
...
class Annotation:
    ...
    def push(self, pushed_workbook_id, pushed_worksheet_id, item_map):
        ...
        new_server_url = _login.client.host.replace('/api', '')
...
class Report(Annotation):
    ...
    def _find_workstep_references(self):
        ...
        for link_match in link_matches:
            ...
            if href_match:
                workstep_references.add((href_match.group(1), href_match.group(2), href_match.group(3)))
