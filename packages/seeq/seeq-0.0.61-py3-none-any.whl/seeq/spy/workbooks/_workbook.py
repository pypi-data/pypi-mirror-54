...
class Folder(ItemWithOwnerAndAcl):
    ...
    def push(self, parent_folder_id, datasource_maps, item_map, *, owner_id=None, to_original_owner=False, label=None,
        ...
        items_api = ItemsApi(_login.client)
        folders_api = FoldersApi(_login.client)
        folder_item = Workbook.find_item(label, self.id)
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def push(self, *, owner_id=None, to_original_owner=False, folder_id=None, item_map=None, label=None,
        ...
        if replace_acl:
            self._push_acl(workbook_id, self.datasource_maps, item_map, replace=True, strict=False)
            ...
            for item_id, item in dict_iterator.items():
                ...
                try:
                    item_output = item.push(self.datasource_maps,
                                            pushed_workbook_id=workbook_id, item_map=item_map,
    ...
    def _scrape_item_inventory(self):
        ...
        for reference in self.referenced_items:
            ...
            dependencies.extend(self._scrape_references_from_ancillaries(reference.id))
    ...
    def _scrape_references_from_ancillaries(self, item_id):
        items_api = ItemsApi(_login.client)
        referenced_items = list()
...
class Annotation:
    ...
    def push(self, workbook_id, worksheet_id, item_map):
        for referenced_workbook_id, referenced_worksheet_id, referenced_workstep_id in self.referenced_worksteps:
            if referenced_workbook_id not in item_map:
                raise DependencyNotFound(referenced_workbook_id)
            if referenced_worksheet_id not in item_map:
                raise DependencyNotFound(referenced_worksheet_id)
            if referenced_workstep_id not in item_map:
                raise DependencyNotFound(referenced_workstep_id)
...
class Chart(CalculatedItem):
    ...
    def push(self, datasource_maps, *, pushed_workbook_id=None, item_map=None, label=None,
        ...
        data_id = Workbook.data_id_from_item_id(label, self.definition['ID'])
        item = Workbook.find_item(label, self.definition['ID'], pushed_workbook_id)
