...
def search(query, *, content_filter='owner', all_properties=False, recursive=False, quiet=False):
    return _search(query, content_filter, all_properties, recursive, quiet)
    ...
    for content in folder_output_list['content']:
        ...
        if content['type'] == 'Folder' and ((recursive and len(path_filter_parts) == 0) or path_matches):
            ...
            child_results_df = _search(child_query, content_filter, all_properties, recursive, content['id'],
                                       new_parent_path, search_folder_id=search_folder_id, quiet=quiet)
