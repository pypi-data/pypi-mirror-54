...
def integrity_report(val):
    ...
    catnum = pd.DataFrame({'CATEGORICAL': [catfeat], 'NUMERICAL': [numfeat]})
    missdata = pd.DataFrame({'USEABLE': [availablerows], 'MISSING': [missing]})
    labels2 = missdata.columns
    labels3 = catnum.columns

    fig, (ax2, ax3) = plt.subplots(nrows=1, ncols=2, figsize=(16, 4))

    ax2.pie(missdata, labels=labels2, explode=None, radius=1.5, autopct='%1.1f%%', colors=['pink', 'lightcoral'],
            startangle=310)
    ax3.pie(catnum, labels=labels3, explode=None, radius=1.5, autopct='%1.1f%%',
            startangle=310, colors=['lightskyblue', 'steelblue'])
