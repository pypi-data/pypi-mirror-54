...
def get_worker_responses(output_uri, job_name):
    ...
    for response_key in s3.list_objects(uri=os.path.join(output_uri, job_name, 'annotations/worker-response')):
        response = s3.read_dict(bucket_name, response_key)
        worker_id = response['answers'][0]['workerId']
        responses = worker_responses.get(worker_id,[])
        responses.append(response['answers'][0])
        worker_responses[worker_id] = responses
...
def draw_annotations(annotations, image, color='green', number_them=False):
    try:
        ...
        for idx, item in enumerate(annotations):
            ...
            draw.rectangle(box, outline=color, width=round(image.width / 512) + 1)
            if number_them:
...
def scale_image_for_annotation(bucket=None, key=None, uri=None):
    try:
        from PIL import Image
        img = s3.read_pillow_image(bucket, key, uri)
        x, y = img.size
        if x * y < 8294400:
            return img, None
        else:
            scalar = math.sqrt(8294400/(x*y))
            new_x = int(scalar * x)
            new_y = int(scalar * y)
            return img.resize((new_x, new_y), Image.BICUBIC), scalar
    except ImportError as e:
        # Simply raise the ImportError to let the user know this requires Pillow to function
        raise e


def scale_oversized_images_in_manifest(manifest):
    new_manifest = []
    for item in manifest:
        new_item = item.copy()
        img, scalar = scale_image_for_annotation(new_item['source-ref'])
        if scalar is not None:
            uri = s3.write_temp_object(img, 'labeling_temp_images/')
...
def reverse_scaling_of_annotation(manifest, label_attribute_name):
    new_manifest = []
    for item in manifest:
        new_item = item.copy()
