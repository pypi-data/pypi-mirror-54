...
class BigQueryUtil:
    """
    Utilities for Big Query interation
    """
    ...
    def dataset(self) -> bigquery.dataset.DatasetReference:
        if not self._dataset:
            try:
                dataset_id = f'{self.project.project_id}.{self.dataset_id}'
                self._dataset = self.client.get_dataset(dataset_id)
            except NotFound:
    ...
    def check_table(self):
        ...
        if self.schema != self.table.schema:
            ...
            except DifferentTableSchemaException as e:
                logging.warning(e.message)

        return self

    def insert_rows(self, rows):
        self.client.insert_rows(self.table.table_id, rows)
