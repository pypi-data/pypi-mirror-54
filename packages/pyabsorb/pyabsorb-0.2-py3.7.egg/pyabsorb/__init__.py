...
class Database:
    ...
    def gimme(self, atom, z, elims):
        ...
        xsec_out = np.stack([ene[imin:imax], sig[imin:imax]])
        ...
        imin = np.where(ene > elims[0])[0][0]
        imax = np.where(ene < elims[1])[0][-1]
        fprime_out = np.stack([ene[imin:imax], f1p[imin:imax], f2p[imin:imax]])
        
        return xsec_out, fprime_out
...
class Atom:
    def __init__(self, name, elims=(1.0e3,1.0e5)):
        ...
        db = Database()
        self.xsec, self.fprime = db.gimme(self.name, self.z, elims)
...
class Mixture:
    ...
    def mix_mu(self, energy, thickness_cm=0.01):
        """
        Returns transmission in float format.
        Arguments:
        ...
        for w in self.stoi.keys():
            u = Atom(w)
            sig, sig_ua = u.atom_mu(energy)
            f1, f2 = u.atom_ff(energy)
            sigma_tot += sig_ua*self.weig[u.name]
            logger.info('%-6s %-7.3f %-7.3f %-7.3f %8.2f barns'\
                        %(u.name, self.weig[u.name],
                          f1, f2, sig*self.stoi[u.name]) )
        self.mu = sigma_tot*self.prop['d']
        self.transmission = np.exp(-self.mu*self.thick)
        logger.info('Total mu = %.2f cm^-1' %(self.mu))
        logger.info('Total muR = %.2f' %(self.mu*0.5*self.thick))
        logger.info('Transmission = %.4f %%' %(100*self.transmission))
        return self.transmission, f1, f2
...
def console():
    ...
    if bool(args.l) == False:
            #thelogger.logger.setLevel(logging.ERROR)
            thelogger.setLevel(logging.ERROR)
     ##--initialise mixture 
    if any([args.v, args.z]) == False and not args.d or not args.e or not args.f or not args.t:
        thelogger.logger.error('Not enough arguments. Check options -f, -e, -t, -d (or -v and -z)')
        sys.exit(1)
