...
class Fields(Plugin):
    ...
    def register_plugin(cls, name, version, module, **kwargs):
        ...
        super(Fields, cls).register_plugin(name, version, module,
                                           plugin_type='fields')
    ...
    def write(self, field_file, flx_fwd, flx_tl):
        """Write flux.
        
        Args:
            self (Fluxes): the Fluxes plugin
            field_file (str): the file where to write fluxes
            flx_fwd, flx_tl: fluxes data to write
            """
        raise PluginError('The function write was not defined')
    
    def make(self, field_file, flx_fwd, flx_tl):
        """Make fluxes.
