...
def detect_channel_binary_objects(
    ...
    Args:
        ...
        min_length (int): the minimum length an object must be to considered
            valid. By default 1.
    ...
    len_filter = lambda run: len(run) > 0 and run[1]-run[0] >= min_length-1
...
def detect_sequence_binary_objects(
    ...
    return [
        detect_channel_binary_objects(
            channel, mask_threshold, gap_threshold, min_length
...
def evaluate_segmentation(
    ...
    for channel_index in range(len(target_channel_matrix)):
        ...
        # get objects
        output_objects = detect_channel_binary_objects(output_channel, mask_threshold, gap_threshold)
        target_objects = detect_channel_binary_objects(target_channel, mask_threshold, gap_threshold)
