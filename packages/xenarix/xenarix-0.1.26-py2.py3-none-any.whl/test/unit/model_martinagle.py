...
def mc_shortrate(model, scenario_num=200, maxyear=3, moment_match=False, frequency=xen.TimeGridFrequency.Day):
...
def rw_shortrate(model, scenario_num=200, maxyear=3, moment_match=False, frequency=xen.TimeGridFrequency.Day):
    if not isinstance(model, xen.Ir1FModel):
        raise Exception('Ir1FModel Type is needed')
...
def equity(model, scenario_num=200, maxyear=3, moment_match=False, frequency=xen.TimeGridFrequency.Day):
...
class TestModelMartingale(unittest.TestCase):

    def test_eq_model(self):
        # model_list = [ xen_s.gbm('gbm'),
        #                xen_s.gbmconst('gbmconst'),
        #                xen_s.gk('gk') ]
    ...
    # real world
    def test_rw_ir_shortrate_model(self):
        model_list = [ xen_s.vasicek('vasicek') ]
