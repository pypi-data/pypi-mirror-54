	...
	def _from_data(self, data):
		...
		if type(data) == float:
			if int(data) == data:
				...
				raise DataTypeError(self.data_type_name, self._CONV_ERR % data)
		if isinstance(data, stringtypes) and not re.match(r'^\s*[+-]?\d+\s*$'):
			raise DataTypeError(self.data_type_name, self._CONV_ERR % data)
		try:
			i = int(data)
		except:
...
class ScriptExecSpec(object):
	...
	def __init__(self, **kwargs):
		...
		self.looptype = kwargs["looptype"].upper() if kwargs["looptype"] is not None else None
		self.loopcond = kwargs["loopcond"]
	def execute(self):
