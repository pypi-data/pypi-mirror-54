...
__all__ = ['register_norgatedata_equities_bundle','register_norgatedata_futures_bundle','futures_market_sessions_available','translate_futures_symbol','__version__','__author__'] 
import sys
...
def create_norgatedata_equities_bundle(bundlename,stock_price_adjustment_setting,watchlists,start_session,end_session):
    def ingest(environ,
        ...
        logger.info('Ingesting of equities bundle ' + bundlename + ' completed')
    return ingest
...
def create_norgatedata_futures_bundle(bundlename,watchlists,start_session,end_session):
    def ingest(environ,
        ...
        adjustment_writer.write(splits=divs_splits['splits'],
        dividends=divs_splits['divs'])
        logger.info('Ingesting of futures bundle ' + bundlename + ' completed')
    return ingest
...
def determine_symbols(watchlists,startdate):
    ...
    for watchlistname in watchlists:
        ...
        newsymbols = norgatedata.watchlist_symbols(watchlistname)
        if (len(newsymbols) == 0):
            logger.warn("Watchlist " + watchlistname + " contains zero symbols")
...
def determine_futures_symbols(watchlists,startdate):
    ...
    for symbol in reversed(symbols):  # Do in reversed order, because we will be deleting some symbols and this
        ...
        if (lqd < startdate):
            ...
            norgate_root_symbol = (norgatedata.futures_market_session_symbol(symbol))
            root_symbol = translate_futures_symbol(norgate_root_symbol)
            if (len(root_symbol) > 0 and not root_symbol in root_symbols):
                ...
                if (norgate_root_symbol == root_symbol):
                    logger.info("Futures Market Session added: " + root_symbol + " (" + marketname + ")")
                else:
                    logger.info("Futures Market Session added: " + root_symbol + " (" + marketname + ") (translated from Norgate symbol " + norgate_root_symbol + ")")
...
def futures_market_sessions_available():
    zipline_markets = {}
    symbols = norgatedata.futures_market_session_symbols()
    for symbol in symbols:
        zipline_symbol = translate_futures_symbol(symbol)
        name = norgatedata.futures_market_session_name(symbol)
