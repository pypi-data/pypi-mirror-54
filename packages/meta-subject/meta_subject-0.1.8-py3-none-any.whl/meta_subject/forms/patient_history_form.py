...
class PatientHistoryFormValidator(FormValidator):
    ...
    def clean(self):
        ...
        self.m2m_other_specify(
            OTHER,
            m2m_field="symptoms",
            field_other="other_symptoms",
        )

        self.date_not_before(
            "hiv_diagnosis_date", "arv_initiation_date",
            "Invalid. Cannot be before HIV diagnosis date.")

        self.date_not_before(
            "arv_initiation_date", "viral_load_date",
            "Invalid. Cannot be before ARV initiation date.")

        self.date_not_before(
            "arv_initiation_date", "current_arv_regimen_date",
            "Invalid. Cannot be before ARV initiation date.")
        ...
        self.date_not_before(
            "hiv_diagnosis_date", "viral_load_date",
            "Invalid. Cannot be before HIV diagnosis date.")
        ...
        self.required_if(YES, field="has_previous_arv_regimen",
                         field_required="previous_arv_regimen")

        if self.cleaned_data.get("has_previous_arv_regimen") == NO:
            self.date_equal(
                "arv_initiation_date", "current_arv_regimen_start_date",
                "Invalid. Expected current regimen date to equal initiation date.")

        self.required_if(
            YES, field="has_previous_arv_regimen",
            field_required="previous_arv_regimen")
        ...
        self.required_if(YES, field="on_oi_prophylaxis",
                         field_required="oi_prophylaxis")

        self.m2m_other_specify(
            OTHER,
            m2m_field="oi_prophylaxis",
            field_other="other_oi_prophylaxis",
        )

        self.required_if(YES, field="on_hypertension_treatment",
        ...
        self.m2m_other_specify(
            OTHER,
            m2m_field="past_year_symptoms",
            field_other="other_past_year_symptoms",
        )

        self.required_if(YES, field="is_heartbeat_regular",
                         field_required="irregular_heartbeat")

        self.required_if(YES, field="has_abdominal_tenderness",
                         field_required="abdominal_tenderness")
