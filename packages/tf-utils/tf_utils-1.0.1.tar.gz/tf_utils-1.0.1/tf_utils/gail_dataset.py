...
class GAIL_DataSet(object):
    def __init__(self, expert_path, train_fraction=0.7, ret_threshold=None, traj_limitation=np.inf, randomize=True):
        ...
        self.randomize = randomize
        self.dset = DataSet(self.obs, self.acs, self.randomize)
        # for behavior cloning
        self.train_set = DataSet(self.obs[:int(self.num_transition * train_fraction), :],
                                 self.acs[:int(self.num_transition * train_fraction), :], self.randomize)
        self.val_set = DataSet(self.obs[int(self.num_transition * train_fraction):, :],
                               self.acs[int(self.num_transition * train_fraction):, :], self.randomize)
        self.log_info()
