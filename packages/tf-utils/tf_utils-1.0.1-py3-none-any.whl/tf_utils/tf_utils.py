...
def save_state(fname, var_list=None, counter=None, max_to_keep=10):
    ...
    if counter is not None:
        saver.save(get_session(), fname, global_step=counter)
    else:
        saver.save(get_session(), fname)


def get_saver(name='default_saver', var_list=None, max_to_keep=1000):
...
def dummy_activation(layer):
    return layer


def dense(x, size, name, weight_init=normc_initializer(1.0), bias=True):
...
def get_any_session():
    """First try to retrieve tf session, in case of None it will return session from Keras"""
    sess = get_session()
    if sess is None:
...
class GetGrad(object):
    def __init__(self, loss, var_list, flat=True, sess=None):
        self.sess = sess
        if flat:
            self.op = flatgrad(loss, var_list)
        else:
            self.op = tf.gradients(loss, var_list)

    def __call__(self, feed_dict):
        if self.sess is None:
            return get_any_session().run(self.op, feed_dict=feed_dict)
        else:
            return self.sess.run(self.op, feed_dict=feed_dict)


class GetWeights(object):
    def __init__(self, var_list, flat=True, sess=None):
        """
        ...
        if flat:
            self.var_list = flat_weight(var_list)
        self.sess = sess

    def __call__(self):
        if self.sess is None:
            return get_any_session().run(self.var_list)
        else:
            return self.sess.run(self.var_list)


def flat_weight(var_list):
    return tf.concat(axis=0, values=[
        tf.reshape(v, [numel(v)]) for v in var_list])
