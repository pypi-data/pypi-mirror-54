...
class SpriteSheet:
    ...
    def __scan_neighbor(self, list_pixel, x, y, visited, label_map, label, height, width, image_format, background):
        ...
        visited[x][y] = 1
        label_map[x][y] = label
        neighbor = [(x-1, y), (x-1, y-1), (x, y-1), (x+1, y-1),
        ...
        while neighbor:
            ...
            if a in range(width) and b in range(height):
                if not visited[a][b] and not self.__is_background_color(background, list_pixel[a][b], image_format):
                    visited[a][b] = 1
                    label_map[a][b] = label
                    neighbor.extend([(a-1, b), (a-1, b-1), (a, b-1), (a+1, b-1),
    ...
    def __find_sprite_by_BFS(self, list_pixel, image_format, background):
        ...
        for i, row in enumerate(list_pixel):
            for j, pixel in enumerate(row):
                ...
                sprite = self.__scan_neighbor(list_pixel, i, j, visited, label_map, label,
                                              height, width, image_format, background)
    ...
    def __mask_color(self, background_color):
        ...
        if len(background_color) == 3:
            mask_color = (random.randint(0, 256), random.randint(
            ...
            mask_color = (random.randint(0, 256), random.randint(
                0, 256), random.randint(0, 256), 255)
            while mask_color == background_color:
                mask_color = (random.randint(0, 256), random.randint(
                    0, 256), random.randint(0, 256), 255)
            return mask_color

    def create_sprite_labels_image(self, sprites, label_map, background_color=(255, 255, 255)):
        ...
        for label, sprite in sprites.items():
            ...
            draw.rectangle([sprite.top_left, sprite.bottom_right],
                        outline=sprite_color[label], width=1)
        return image
