...
class LogManager(object):
    ...
    def __add_a_hanlder(self, handlerx: logging.Handler):
        if self.__judge_has_handler_type(type(handlerx)):
            return
        handlerx.setLevel(10)
        handlerx.setFormatter(self._formatter)
        self.logger.addHandler(handlerx)

    def __judge_has_handler_type(self, handler_type: type):
    ...
    def __add_handlers(self):
        ...
        # REMIND 添加控制台日志
        if not (self.__judge_has_handler_type(ColorHandler) or self.__judge_has_handler_type(logging.StreamHandler)) and self._is_add_stream_handler:
        ...
        if not self.__judge_has_handler_type(ConcurrentRotatingFileHandler) and all([self._log_path, self._log_filename]):
        ...
        # REMIND 添加mongo日志。
        if not self.__judge_has_handler_type(MongoHandler) and self._mongo_url:
            self.__add_a_hanlder(MongoHandler(self._mongo_url))

        # REMIND 添加es日志。
        # if app_config.env == 'test' and self._is_add_elastic_handler:
        if not self.__judge_has_handler_type(ElasticHandler) and app_config.env == 'testxxx':  # 使用kafka。不直接es。
            """
        ...
        if not self.__judge_has_handler_type(KafkaHandler) and app_config.env == 'test' and ALWAYS_ADD_KAFKA_HANDLER_IN_TEST_ENVIRONENT:
            self.__add_a_hanlder(KafkaHandler(KAFKA_BOOTSTRAP_SERVERS, ))

        # REMIND 添加钉钉日志。
        if not self.__judge_has_handler_type(DingTalkHandler) and self._ding_talk_token:
