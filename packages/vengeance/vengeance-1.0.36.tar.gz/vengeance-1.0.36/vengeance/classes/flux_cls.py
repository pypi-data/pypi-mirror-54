...
class flux_cls:
    ...
    def insert_columns(self, *inserted):
        ...
        eg inserted:
            flux.insert_columns('inserted')              insert column at beginning
        ...
        inserted = self.__validate_inserted_names(inserted)
        ...
        indices = [header_values.index(h) for _, h in inserted]
        indices.sort()
    ...
    def append_columns(self, *names):
        ...
        self.__validate_none_in_headers(names, self.headers)
    ...
    def delete_columns(self, *names):
        ...
        self.__validate_no_duplicates(names)
        self.__validate_all_in_headers(names, self.headers)

        _indices_ = list(range(self.num_cols))

        indices = [self.headers.get(h, h) for h in names]       # convert to integers
    ...
    def __sort_rows(self, f, order):
        ...
        f = modify_iteration_depth(f, depth=1)
    ...
    def index_row(self, *f):
        ...
        non-unique keys are overwritten by last encountered row

        (maybe a better name is row_index?)
        """
        f = self.__row_values_accessor(f)
        return OrderedDict((f(row), row) for row in self)

    def index_rows(self, *f):
        """ dictionary of {f(row): [row(s)]}
        appends rows to a list for non-unique keys

        (maybe a better name is rows_index?)
        """
        f = self.__row_values_accessor(f)

        items = [(f(row), row) for row in self]
        d = OrderedDefaultDict(list, items)
        ...
        try:
            ...
            if self.is_jagged:
                raise TypeError('Jagged rows detected in matrix; '
                                'mismatched number of headers and values') from e
            else:
                raise e
    ...
    def flux_rows(self, r_1='*h', r_2='*l'):
        ...
        return islice(self.matrix, r_1, r_2 + 1)

    def contiguous_rows(self, *f):
        """ :return: list of rows where values are contiguous

        sections = list(self.index_rows(f).values())?
        """
        sections = []
    ...
    def _to_flux_rows(self, m):
        ...
        self.__validate_header_values(m[0])
    ...
    def __validate_accessor(self, f):
        ...
        if isinstance(f, (str, list, tuple)):
            _f_ = modify_iteration_depth(f, depth=1)
    ...
    def __validate_inserted_names(self, inserted):
    ...
    @staticmethod
    def __validate_header_values(headers):
    ...
    @staticmethod
    def __validate_none_in_headers(names, headers):
        invalid = list(headers.keys() & set(names))
        if invalid:
            raise ValueError('column name(s) conflict with existing headers:\n{}'.format(invalid))

    @staticmethod
    def __validate_all_in_headers(names, headers):
        names   = (n for n in names if isinstance(n, str))
        invalid = list(set(names) - headers.keys())
        if invalid:
    ...
    def __setitem__(self, col_name, col_values):
        ...
        """
        col_values = modify_iteration_depth(col_values, depth=1)
