...
def open_workbook(path,
    ...
    if wb.ReadOnly is False and read_only:
        ...
        print_extra_line = True
    elif wb.ReadOnly and read_only is False:
...
def __excel_app_from_hwnd(window_h):
    ...
    try:
        ...
        import platform
        python_vers  = '.'.join(platform.python_version().split('.')[:2])
        gencache_dir = '%userprofile%\\AppData\\Local\\Temp\\gen_py\\{}\\'.format(python_vers)
        ...
        raise AttributeError('Error loading win32com module: \n\n'
                             ...
                             'gencache folder: {}'.format(gencache_dir)) from e
