...
def convert_to_bytes(input):
    ...
    for chunk in input:
        for b in chunk:
            bytes.append(decodeNum(b))
    return bytes

def convert_from_bytes(input):
    output = list()
    for chunk in input:
        chunk = int(chunk)
        num_str = '%x' % chunk
        if chunk >= 2 ** BITS_IN_BYTE:
            raise Exception("Number, %d, does not fit in %d bits" % (chunk, BITS_IN_BYTE))
        converted_elem = ('0' * (2 - len(num_str)) + num_str).decode('hex')
        output.append(converted_elem)
    return output
