...
class connection(object):
    ...
    def __init__(self, url=None, customerid=None, key=None, secret=None,
        ...
        self.timemachine_schema_url = ("%s/public/api/customers/%s/%s" %
                                    (url, customerid, "schema/timemachine"))
    ...
    def execute(self, sql, arg=None, **kwargs):
        ...
        if sql_string.startswith('set to_timestamp =', 0,
                                len('set to_timestamp =')):
            if len(sql_string_array) == 4:
                user_timestamp = sql_string_array[3]

                if ((user_timestamp.startswith('"') and
                    user_timestamp.endswith('"')) or
                    (user_timestamp.startswith("'") and
                    user_timestamp.endswith("'"))):
        ...
        if sql_string.startswith('set from_timestamp =', 0,
                                len('set from_timestamp =')):
            if len(sql_string_array) == 4:
                user_timestamp = sql_string_array[3]
        ...
        if sql_string.startswith('show parameters', 0,
                                    len('show parameters')):


            description_arr = [('parameter',
                                None,
                                None,
                                None,
                                None,
            ...
            if len(sql_string_array) == 2:
                ...
                if sql_string_array[2] == 'from_timestamp':
                    rows.append[('from_timestamp', self.from_timestamp)]
