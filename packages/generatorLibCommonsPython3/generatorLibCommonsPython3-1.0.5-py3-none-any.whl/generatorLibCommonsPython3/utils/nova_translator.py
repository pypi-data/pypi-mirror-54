...
class NovaTranslator:
    ...
    for novaMetadataObject in novaMetadatasObject:
      ...
      if novaInputFieldStringValue != None:
        ...
        if novaInputFieldType == "simple":
          ...
          serializedValue = NovaTranslator.serializeSimpleType(typeOf, novaInputFieldStringValue)
    ...
    if depth == 1:
      ...
      for itemArray in novaInputFieldStringValue:
        ...
        if NovaTranslator.isSimpleType(typeOf):
          ...
          # Simple type
          itemArrayValue = NovaTranslator.serializeSimpleType(typeOf, itemArray)
  ...
  def serializeSimpleType(typeOf, itemValue):

    if typeOf == bool:

      return NovaTranslator.serializeSimpleTypeBoolean(itemValue)

    elif typeOf == decimal.Decimal:

      return NovaTranslator.serializeSimpleTypeDecimal(itemValue)
  
    else:

      return itemValue
  
  def serializeSimpleTypeBoolean(inputBoolean):

    return inputBoolean != None and inputBoolean == "true"

  def serializeSimpleTypeDecimal(obj):
