...
def validate_method_signature(method: ResourceMethodWithReturnValue, uri_parameters: Set[str]) -> Tuple[Optional[str],
    ...
    :returns: Request body hints and method return hints.
    """
    ...
    validate_type_preconditions(returns)
    ...
    body_parameters = set(hints) - (uri_parameters | set(itertools.islice(arguments, 2)))
    ...
    return next(iter(body_parameters), None), returns
...
def patch_resource_methods(uri_template: str, resource: Any) -> None:
    ...
    resource.methods_body_parameters_hints = {}
    resource.methods_return_hints = {}
    ...
    for method_name in filter(functools.partial(hasattr, resource), _METHOD_NAMES):
        ...
        try:
            body_hints, return_hints = validate_method_signature(method, uri_parameters=uri_parameters)

            resource.methods_body_parameters_hints[method_name] = body_hints
            resource.methods_return_hints[method_name] = return_hints
