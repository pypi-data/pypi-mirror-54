...
class Type00_2(GeneratedsSuper):
    ...
    def validate_IntType01_1(self, value):
        ...
        if value is not None and Validate_simpletypes_ and self.gds_collector_ is not None:
            ...
            if value < 0:
                lineno = self.gds_get_node_lineno_()
                self.gds_collector_.add_message('Value "%(value)s" near line %(lineno)s does not match xsd minInclusive restriction on IntType01_1' % {"value": value, "lineno": lineno} )
                result = False
            if value > 127:
                lineno = self.gds_get_node_lineno_()
                self.gds_collector_.add_message('Value "%(value)s" near line %(lineno)s does not match xsd maxInclusive restriction on IntType01_1' % {"value": value, "lineno": lineno} )
                result = False
    def hasContent_(self):
        if (
