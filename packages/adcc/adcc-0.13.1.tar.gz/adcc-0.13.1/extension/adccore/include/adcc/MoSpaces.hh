...
struct MoSpaces {
  ...
  /** Construct an MoSpaces object from a HartreeFockSolution_i and a maximal block
   *  size to be used for the tensors. Convenience wrapper to above constructor, which
   *  selects the orbital indices automatically on the upper / lower end of the energy
   *  range.
   *
   *  \param hf                HartreeFockSolution_i object to use
   *  \param adcmem_ptr        ADC memory keep-alive object to be used in all Tensors
   ...
   */
  MoSpaces(const HartreeFockSolution_i& hf, std::shared_ptr<const AdcMemory> adcmem_ptr,
           size_t n_core_orbitals = 0, size_t n_frozen_core = 0,
           size_t n_frozen_virtuals = 0);
