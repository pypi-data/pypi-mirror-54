...
class ReferenceState {
 ...
 public:
   ...
   *                          providing the HartreeFockSolution data, which
   *                          will be provided by this object.
   * \param adcmem_ptr        Pointer to the adc memory management object.
  ...
  ReferenceState(std::shared_ptr<const HartreeFockSolution_i> hfsoln_ptr,
                 ...
                 std::vector<size_t> frozen_virtuals, bool symmetry_check_on_import)
        : ReferenceState(
                hfsoln_ptr,
                std::make_shared<MoSpaces>(*hfsoln_ptr, adcmem_ptr, core_orbitals,
                                           frozen_core_orbitals, frozen_virtuals),
                symmetry_check_on_import) {}

  /* Setup a ReferenceState object using plain numbers for core / frozen core
   * / frozen virtual orbitals. Convenience wrapper to above constructor, which
   *  selects the orbital indices automatically on the upper / lower end of the MO index
   *  range.
   *
   * \param hfsoln_ptr        Pointer to the Interface to the host program,
   *                          providing the HartreeFockSolution data, which
   *                          will be provided by this object.
   * \param adcmem_ptr        Pointer to the adc memory management object.
   * \param n_core_orbitals   Number of core orbitals to use.
   * \param n_frozen_core     Number of orbitals to put in a frozen core.
   * \param n_frozen_virtuals Number of frozen virtual orbitals.
   */
  ReferenceState(std::shared_ptr<const HartreeFockSolution_i> hfsoln_ptr,
                 std::shared_ptr<const AdcMemory> adcmem_ptr, size_t n_core_orbitals = 0,
                 size_t n_frozen_core_orbitals = 0, size_t n_frozen_virtuals = 0,
                 bool symmetry_check_on_import = false)
        : ReferenceState(
                hfsoln_ptr,
                std::make_shared<MoSpaces>(*hfsoln_ptr, adcmem_ptr, n_core_orbitals,
                                           n_frozen_core_orbitals, n_frozen_virtuals),
                symmetry_check_on_import) {}
