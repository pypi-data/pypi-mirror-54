...
class CitySafetyAccessory(TimePublicData):
    """City safety accessory class - this is a parent class that can be inherited by each individual city."""
    ...
    def __init__(
        ...
        super(CitySafetyAccessory, self).__init__(cols=cols, enable_telemetry=enable_telemetry)
        if enable_telemetry:
    ...
    def filter(self, env, min_date, max_date):
        ...
        ds = super(CitySafetyAccessory, self).filter(env, min_date, max_date)
