...
from .location_time_public_data import LocationTimePublicData
...
class NoaaIsdWeatherAccessory(LocationTimePublicData):
    """NOAA ISD historical weather accessory class."""
    ...
    def __init__(
        ...
        super(NoaaIsdWeatherAccessory, self).__init__(cols, enable_telemetry=enable_telemetry)
        self.limit = limit
        if enable_telemetry:
    ...
    def filter(self, env, min_date, max_date):
        ...
        ds = super(NoaaIsdWeatherAccessory, self).filter(env, min_date, max_date)
    ...
    def get_pandas_limit(self):
        ...
        return PandasDataLoadLimitToMonth(self.start_date, self.end_date, limit=self.limit)
