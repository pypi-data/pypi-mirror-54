...
from .accessories.nyc_tlc_fhv_accessory import NycTlcFhvAccessory
from .dataaccess.dataset_partition_prep import prep_partition_puYear_puMonth
from azureml.data import TabularDataset
from typing import List, Optional


class NycTlcFhv(AbstractTabularOpenDataset):
    """NYC TLC FHV data class."""

    def __init__(
            self,
            start_date: datetime = NycTlcFhvAccessory.default_start_date,
            end_date: datetime = NycTlcFhvAccessory.default_end_date,
            cols: Optional[List[str]] = None,
            dataset: TabularDataset = None,
            enable_telemetry: bool = True):
        """
        Initializes an instance of the NycTlcFhv class.
        ...
        """
        worker = NycTlcFhvAccessory(
            start_date=start_date,
            end_date=end_date,
            cols=cols,
            enable_telemetry=enable_telemetry)
        if dataset is not None:
            if start_date != NycTlcFhvAccessory.default_start_date and \
                end_date != NycTlcFhvAccessory.default_end_date and \
                    cols is not None:
...
at the same time! Please use either of them.')
            ...
            AbstractTabularOpenDataset.__init__(
                ...
                fine_grain_timestamp='pickupDateTime')

    @staticmethod
    def get(dataset: TabularDataset, enable_telemetry: bool = True):
        """Get an instance of NycTlcFhv.

        :param dataset: input an instance of TabularDataset.
        :type end_date: Dataset.
        :param enable_telemetry: whether to enable telemetry, disabled for UT only.
        :type enable_telemetry: bool
        :return: an instance of NycTlcFhv.
        """
        nyc = NycTlcFhv(dataset=dataset, enable_telemetry=enable_telemetry)
