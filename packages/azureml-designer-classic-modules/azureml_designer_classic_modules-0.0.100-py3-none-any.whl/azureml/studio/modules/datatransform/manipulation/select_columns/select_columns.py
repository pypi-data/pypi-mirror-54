...
class SelectColumnsModule(BaseModule):
    ...
    @module_entry(ModuleMeta(
        ...
        description="Selects columns to include or exclude from a dataset in an operation."
                    " Formerly known as Project Columns.",
    ...
    def _run_impl(cls, dt: DataTable, column_select: DataTableColumnSelection):
        ...
        selected_col_indexes = column_select.select_column_indexes(dt)
        # Compute the duplicated column indexes and raise error when there are duplicated indexes.
        selected_col_set = set()
        duplicated = filter(lambda x: x in selected_col_set or selected_col_set.add(x), selected_col_indexes)
        first_duplicated_idx = first(duplicated, default=None)
        if first_duplicated_idx is not None:
            name = dt.column_names[first_duplicated_idx]
            raise DuplicatedColumnNameError(duplicated_name=name, details="Duplicated columns are selected.")
        return dt.get_slice_by_column_indexes(selected_col_indexes, if_clone=False),
