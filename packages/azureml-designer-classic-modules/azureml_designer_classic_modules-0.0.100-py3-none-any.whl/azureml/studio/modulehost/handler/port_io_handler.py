...
from azureml.studio.core.utils.fileutils import get_file_name, make_file_name
...
class InputHandler:
    ...
    def handle_input_directory(file_path):
        with TimeProfile(f"Try to read from {file_path} via meta"):
            ...
            if isinstance(directory, DataFrameDirectory):
                dt_meta_path = directory.extension.get('DataTableMeta') if directory.extension else None
                # If DataTableMeta is in extension, initialize DataTable with it
                if dt_meta_path is not None:
                    # Assume the path is valid
                    dt_meta = read_with_pickle_from_file(os.path.join(file_path, dt_meta_path))
                    common_logger.info(f"Load DataTableMeta successfully, path={dt_meta_path}")
                    return DataTable(directory.data, dt_meta)
