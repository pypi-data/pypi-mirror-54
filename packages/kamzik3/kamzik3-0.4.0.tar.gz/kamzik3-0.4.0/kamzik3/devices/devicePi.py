...
class DevicePi(DeviceSocket):
    ...
    def _init_attributes(self):
        ...
        self.attributes.update({
            ATTR_SERIAL_NUMBER: Attribute(readonly=True),
            ATTR_CHANNELS: Attribute(None, readonly=True, default_type=np.uint16),
            ATTR_SYNTAX_VERSION: Attribute(None, readonly=True, default_type=np.float16, decimals=3),
            ATTR_POSITIONS: Attribute(None, readonly=True),
            ATTR_TRIGGER_OUTPUTS_COUNT:  Attribute(None, readonly=True, default_type=np.uint16),
            ATTR_WAVE_GEN_COUNT: Attribute(None, readonly=True, default_type=np.uint16),
            ATTR_SERVO_UPDATE_TIME: Attribute(None, readonly=True, default_type=np.float64, decimals=12, unit="ms"),
        })
    ...
    def _set_trigger_output(self, command, output):
        ...
        for line in output:
            ...
            if attribute is not None:
                ...
                self.attributes[trigger_group_name][ATTR_TRIGGER_LINE_DATA] = Attribute(None, readonly=True)
                ...
                if attribute == TRIG_OUTPUT_AXIS:
                    ...
                    self.attributes[trigger_group_name][attribute] = Attribute(int(value) - 1, default_type=np.uint16,
                                                                               set_function=set_attribute)
                elif attribute == TRIG_OUTPUT_MODE:
                    self.attributes[trigger_group_name][attribute] = Attribute(trigger_output_mode.inv.get(int(value)),
                                                                               default_type=list(
                                                                                   trigger_output_mode.keys()),
                                                                               set_function=set_attribute)
                else:
                    self.attributes[trigger_group_name][attribute] = Attribute(float(value), default_type=np.float64,
    ...
    def _set_wave_generators(self, command, output):
        ...
        for line in output:
            ...
            self.attributes[generator_group_name] = {}
            self.attributes[generator_group_name][ATTR_STATUS] = Attribute(STATUS_IDLE, readonly=True)
            self.attributes[generator_group_name][ATTR_WAVETABLE_DATA] = Attribute(None, readonly=True)
            self.attributes[generator_group_name][ATTR_RECORD_TABLE_DATA] = Attribute(None, readonly=True)
            self.attributes[generator_group_name][ATTR_WAVETABLE_LENGTH] = Attribute(None, readonly=True,
                                                                                     default_type=np.uint32)
            set_function = lambda offset, gen_id=generator_id: self.set_wave_gen_output_offset(gen_id, offset)
            self.attributes[generator_group_name][ATTR_WAVETABLE_OFFSET] = Attribute(None, readonly=False,
                                                                                     default_type=np.float64, decimals=12, set_function=set_function)
            self.attributes[generator_group_name][ATTR_WAVE_TABLE] = Attribute(None, readonly=True,
                                                                                     default_type=np.uint32)

            set_function = lambda table_rate, gen_id=generator_id: self.set_wave_generator_rate_and_interpolation_type(gen_id, wave_table_rate=table_rate)
            self.attributes[generator_group_name][ATTR_WAVETABLE_RATE] = Attribute(None, default_type=np.uint32, set_function=set_function)
            ...
            self.attributes[generator_group_name][ATTR_WAVETABLE_INTERPOLATION] = Attribute(None, default_type=list(
                                                                                   data_interpolation.keys()), set_function=set_function)
    ...
    def handle_readout(self, readout_buffer):
        ...
        if command == "IDN?\n":
            ...
            for generator_id in range(0, self.get_value(ATTR_WAVE_GEN_COUNT)):
                ...
                self.set_value(("Wave generator {}".format(generator_id), ATTR_STATUS), STATUS_BUSY if state else STATUS_IDLE)
            ...
            for line in lines:
                ...
                self.set_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_OFFSET], float(generator_offset))
        elif "WSL?" in command:
            lines = output.split(" \n")
            for line in lines:
    ...
    def get_memory_parameters(self, list_of_parameters, callback=None):
        ...
        attributes = " ".join([" ".join(c) for c in zip([str(b + 1) for b in range(len(list_of_parameters))], list_of_parameters)])
    ...
    def set_wave_generator_rate_and_interpolation_type(self, wave_generator_id=0, wave_table_rate=None, interpolation_type=None,
                                                       callback=None):
        """
        ...
        if interpolation_type is None:
            interpolation_type = self.get_value(["Wave generator {}".format(wave_generator_id), ATTR_WAVETABLE_INTERPOLATION])
    ...
    def set_connected_table_to_generator(self, wave_generator_id, wave_table_id, callback=None):
        ...
        return self.command("WSL {} {}\n".format(wave_generator_id + 1, wave_table_id + 1), returning=False, with_token=True,
                            callback=callback)
    ...
    def get_trigger_line(self, trigger_output, start_point=1, number_of_points=None, callback=None):
        ...
        return self.command("TWS? {} {} {}\n".format(start_point, number_of_points, trigger_output + 1), with_token=True, callback=callback)
    ...
    def get_wavetable_data(self, wave_table_id, start_point=1, number_of_points=None, callback=None):
        ...
        return self.command("GWD? {} {} {}\n".format(start_point, number_of_points, wave_table_id + 1), with_token=True, callback=callback)
    ...
    def set_wave_form_definition(self, wave_table_id, wave_add_method, wave_type, wave_type_param, callback=None):
        if wave_type == WAVE_TYPE_PNT:
            ...
            if prepared_command is not None:
                return self.command(_make_command(prepared_command, command_parameter, sliced_points), returning=False, with_token=True, callback=callback)
        ...
        return self.command("%s %s\n" % (command, command_parameter), returning=False, with_token=True, callback=callback)
    ...
    def set_recorded_configuration(self, table_id, source, option, callback=None):
        ...
        :return:
        """
        return self.command("DRC {} {} {}\n".format(table_id + 1, source + 1, option), returning=False, with_token=True, callback=callback)
    ...
    def get_recorder_data(self, table_id, start_point=1, number_of_points=None, callback=None):
        ...
        return self.command("DRR? {} {} {}\n".format(start_point, number_of_points, table_id + 1), with_token=True, callback=callback)
    ...
    def set_recorded_trigger_source(self, table_id, source, value, callback=None):
        ...
        :return:
        """
        return self.command("DRT {} {} {}\n".format(table_id + 1, source, value), returning=False, with_token=True, callback=callback)
    ...
    @expose_method({"channel": "0", "center": "Center of scan", "step_size": "steps size", "pixels": "Scan points", "dwell_time": "usec", "trigger_mode": "0", "interpolating": "0"})
    def set_fly_scan(self, channel=0, center=50, step_size=1, pixels=10, dwell_time=100, trigger_mode=TRIG_MODE_GENERATOR, interpolating=False):
        self.stop_polling()
        self.set_status(STATUS_BUSY)
        channel = int(channel)
        center = float(center)
        ...
        self.set_wave_generator_rate_and_interpolation_type(wave_table_rate=wave_table_rate, interpolation_type=interpolation_type)
...
class DevicePiChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.attributes.update({
            ATTR_ENCODER_BASE_UNIT: Attribute(None, readonly=True),
            ATTR_POSITION: Attribute(None, default_type=np.float64, decimals=9,
                                     set_function=self._move_absolute, set_value_when_set_function=False),
            ATTR_POSITION_UPPER_LIMIT: Attribute(None, default_type=np.float64, decimals=9,
                                                 set_function=self.set_position_upper_limit),
            ATTR_POSITION_LOWER_LIMIT: Attribute(None, default_type=np.float64, decimals=9,
                                                 set_function=self.set_position_lower_limit),
            ATTR_VELOCITY: Attribute(None, default_type=np.float64, decimals=9,
                                     set_function=self.set_velocity),
            ATTR_SERVO_STATE: Attribute(None, default_type=np.bool, set_function=self.set_servo_state),
            ATTR_REFERENCED: Attribute(None, readonly=True, default_type=np.bool),
            ATTR_MOVING: Attribute(None, readonly=True, default_type=np.bool),
            ATTR_SCANNING: Attribute(None, readonly=True, default_type=np.bool),
            ATTR_WAVE_GENERATOR: Attribute(None, readonly=True, default_type=np.uint16),
            ATTR_RECORD_TABLE_DATA: Attribute(None, readonly=True),
        })
    ...
    @expose_method({"Field_size": ATTR_POSITION, "Steps": "Number of steps", "Dwell_time": "milliseconds", "Interpolating": "bool"})
    def set_relative_fly_scan(self, Field_size, Steps, Dwell_time, Interpolating):
        ...
        generator_id = self.device.set_fly_scan(self.channel, actual_position, step_size, int(Steps), float(Dwell_time), interpolating=int(Interpolating))
