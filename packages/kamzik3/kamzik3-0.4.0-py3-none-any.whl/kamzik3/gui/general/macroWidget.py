...
class MacroWidget(QWidget, YamlSerializable):
    ...
    def _show_field_size_widget(self, row, default_unit=None):
        ...
        field_size_attribute[VALUE] = 0
        if self.config.get("defaults", False):
            field_size_attribute[VALUE] = self.config["defaults"].get("Field size", 0)
    ...
    def _show_step_size_widget(self, row, default_unit=None):
        ...
        step_size_attribute[VALUE] = 0
        if self.config.get("defaults", False):
            step_size_attribute[VALUE] = self.config["defaults"].get("Step size", 0)
    ...
    def _show_total_steps_widget(self, row):
        ...
        self.step_size_lock_button.setChecked(True)
