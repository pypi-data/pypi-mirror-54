...
class DevicePoller(Thread):
    # Minimum polling interval default is 10 ms
    min_poll = 10
    ...
    def run(self):
        ...
        # while not self.stopped.wait(min_poll_tick - poll_offset):
        while True:
            # begin = time.time()
            if check_connection_at <= 0:
                for device in self.commands_buffer.keys():
                    self.check_device_alive(device)
            ...
            # poll_offset = time.time() - begin
            # if poll_offset >= min_poll_tick:
            #     # It takes more time to serve all devices commands then minimum polling time
            #     self.logger.warning(u"Device poller cannot keep up with polling rate.")
            #     poll_offset = 0
            #
            # sleep(min_poll_tick - poll_offset)
    ...
    def check_devices_connection_timeout(self):
        # If any devices is connecting check if it is within connection_timeout
    ...
    def check_device_alive(device):
        if not device.connected or device.closing or device.response_error:
            return

        time_diff = device.request_timestamp - device.response_timestamp
        if time_diff >= device.response_timeout * 1e-3:
            if not device.retry_command():
                print(device.commands_buffer, device.commands_buffer)
                device.handle_response_error(u"Device response timeout")
