...
class DeviceKaraboBridge(Device):
    ...
    def _init_attributes(self):
        ...
        self.attributes[ATTR_REFRESHING] = Attribute(False, readonly=True, default_type=np.bool)
    ...
    def _update_attributes(self):
        ...
        for key, value in data.items():
            ...
            for sub_key, sub_value in value.items():
                ...
                self.attributes[key][sub_key] = Attribute(sub_value, readonly=True, default_type=default_type,
                                                          decimals=decimals)

        self.attributes[ATTR_MIN_UPDATE_TIMEOUT] = Attribute(0, default_type=np.float64, unit="ms",
                                                             decimals=0)
        self.attributes[ATTR_TRAIN_TIMEOUT] = Attribute(100, default_type=np.float64, unit="ms",
                                                        decimals=0)
        self.attributes[ATTR_REMOTE_TIMESTAMP] = Attribute(None, default_type=np.float64, unit="sec", readonly=True,
                                                           decimals=6)
        self.attributes[ATTR_LOCAL_TIMESTAMP] = Attribute(None, default_type=np.float64, unit="sec", readonly=True,
                                                          decimals=6)
        self.attributes[ATTR_DELTA_TIMESTAMP] = Attribute(0, default_type=np.int64, unit="ms", readonly=True,
                                                          decimals=6)
        self.attributes[ATTR_COUNTER] = Attribute(0, default_type=np.uint16, set_value_when_set_function=False,
                                                  set_function=self.count_trains)
        self.attributes[ATTR_LOCAL_TRAIN_ID] = Attribute(0, default_type=np.int64, readonly=True)
        self.attributes[ATTR_REMOTE_TRAIN_ID] = Attribute(0, default_type=np.int64, readonly=True)
        self.attributes[ATTR_TRAIN_OFFSET] = Attribute(0, default_type=np.int64)
