...
def simplify_strings(to_simplify, simplifier="standard"):
    ...
    simplifier : str, list ['capital', chars, ], optional
        desired simplifier can be specified. otherwise the standard ones will be selected

    Returns
    -------
    dict
        simplified values {normal_value: simplified_value}

    """
    standard_simplifier = "[_, | \n ' & \" % \\ * -]"

    if simplifier == "standard":
        simplifier = standard_simplifier
        capital = True
    elif simplifier == "capital":
        simplifier = False
        capital = True
    elif simplifier == "separators":
        simplifier = standard_simplifier
        capital = False

    else:
        capital = False
        simplifier = "".join(simplifier)
    ...
    if isinstance(to_simplify, list):
        if capital:
    ...
    raise TypeError(f"to_simplify needs to be either ")
