...
class StoreMetas:
    ...
    def __getitem__(self, key):
        ...
        return [elem[key].get_untracked() for elem in self.elems]
...
class StoreMeta:
    ...
    def __getattribute__(self, key):
        ...
        if elem:
            if key=='meta':
                return elem.meta.get_untracked()
            if isinstance(elem.data, dict):
                return elem.data.get(key).get_untracked()
    ...
    def __getitem__(self, key):
        ...
        if elem:
            if isinstance(key, int):
                return elem.data[key].get_untracked()
            else:
                return elem.data.get(key).get_untracked()
