...
from clearviewai.primary.processing.monitor import TrainingMonitor
from clearviewai.primary.utils.utils import check_point

from clearviewai.primary.config.constants import EPOCHS, BATCH_SIZE, LEARNING_RATE, VALIDATION_RATIO
from clearviewai.primary.processing.aspectresize import AspectAwareResize
from clearviewai.primary.processing.simple_processor import SimplePreprocessor, ImageToArray
...
class Train:
    ...
    def compile_model(self, model, rate=LEARNING_RATE, include_scheduler=False, schedular_type='norm', dec=0, momentum=0, nesterov=False):
        print("[INFO] compiling model")
        self.lrate = rate
        opt = SGD(lr=rate, decay=dec, momentum=momentum, nesterov=nesterov)
        self.model = model
        self.model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])

    def train(self, vrbse=1, aug=False, cp=False,
              cp_dir=None, cp_best=False, live_monitor=False):
