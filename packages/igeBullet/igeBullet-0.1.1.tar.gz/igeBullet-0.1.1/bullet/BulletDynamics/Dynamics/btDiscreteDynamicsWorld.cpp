...
} stepSimThreadArgType;

static int stepSimThread(unsigned int argsize,void* arg)
{
	stepSimThreadArgType* stepSimThreadArg=(stepSimThreadArgType*)arg;
	stepSimThreadArg->world->stepSimulation(stepSimThreadArg->timeStep,stepSimThreadArg->maxSubSteps,
											stepSimThreadArg->fixedTimeStep);
	return 0;
}
#endif

int	btDiscreteDynamicsWorld::stepSimulationAsync( btScalar timeStep,int maxSubSteps, btScalar fixedTimeStep)
{
#ifdef SN_TARGET_PSP2
	simthread=sceKernelCreateThread("stepSimThread",stepSimThread,SCE_KERNEL_DEFAULT_PRIORITY_USER,65536,0,
									SCE_KERNEL_CPU_MASK_USER_ALL,NULL);
	static stepSimThreadArgType stepSimThreadArg;
	stepSimThreadArg.world=this;
	stepSimThreadArg.fixedTimeStep=fixedTimeStep;
	stepSimThreadArg.maxSubSteps=maxSubSteps;
	stepSimThreadArg.timeStep=timeStep;
	sceKernelStartThread(simthread,sizeof(stepSimThreadArgType),(void*)&stepSimThreadArg);
	return 0;
#else
...
void btDiscreteDynamicsWorld::stepSimulationWait()
{
#ifdef SN_TARGET_PSP2
	sceKernelWaitThreadEnd(simthread,NULL,NULL);
	sceKernelDeleteThread(simthread);
#endif
}
