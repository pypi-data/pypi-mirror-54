...
class TaskScheduler(ABC):
    ...
    def check_if_logging_tables_exists(self):
        ...
        new_table_query = 'CREATE TABLE IF NOT EXISTS `task_execution_steps`(' \
                          ...
                          '`category`    enum(\'Idle\', \'Initialize\', \'Extract\', \'Transform\', \'Transfer\', \'Inform\')  DEFAULT \'Idle\' NOT NULL,' \
                          'PRIMARY KEY (`id`),' \
    ...
    @abstractmethod
    def create_task_execution_steps(self, step_number: int, step_description: str, step_category: str):
        """
        Place
        :param step_number:
        ...
        :return:
        """
        self.mysql.raw_query('REPLACE INTO task_execution_steps (`task_id`, `nr`, `description`, `category`) VALUES ({task_id},{nr},{description},{category})'.format(task_id=self.task_id,
                                                                                                                                                                      nr=step_number,
                                                                                                                                                                      description=step_description,
                                                                                                                                                                      category=step_category), insert=True)
