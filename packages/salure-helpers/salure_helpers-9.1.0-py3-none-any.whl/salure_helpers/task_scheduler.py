...
class TaskScheduler(ABC):
    ...
    def create_task_execution_steps(self, step_details: list):
        ...
        values = []
        for step in step_details:
            values.append((self.task_id, step['nr'], step['description'], step['category']))

        response = self.mysql.raw_query('REPLACE INTO task_execution_steps (`task_id`, `nr`, `description`, `category`) VALUES {step_details}'.format(step_details=tuple(values)), insert=True)
