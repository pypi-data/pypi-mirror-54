...
def query(query: str, when: str = None, timeout: float = None,
    ...
    base = (configuration or {}).get(
        "prometheus_base_url", "http://10.119.8.142:9090")
...
def query_alerts_exist_for_job(job: str) -> Dict[str, Any]:
    """
    Run an instant query for alerts for a particular job against a Prometheus server 
    and returns its result as-is.
    """
    result=query("ALERTS")

    alerts=jsonpath.jsonpath(result, "$.data.result[*][?(@.job=='{j}')]".format(j=job))

    if alerts:
            logger.debug("Alert(s) exist for {j}: {m}".format(j=job,m=alerts))
            return True
    return False

query_alerts_exist_for_job("AWS_Emissary")
