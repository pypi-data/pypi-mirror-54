...
def update_device(device_skiply_id, device_desc, client_id, group_id=None):    
    devices = get_device_from_skiply_id(device_skiply_id)

    if (devices is not None):
        if ((client_id is not None) & (d.client_id != client_id)):
            d.client_id = client_id
        if ((device_desc is not None) & (d.description != device_desc)):
            d.description = device_desc
        if ((group_id is not None) & (d.groupe_id != group_id)):
            d.groupe_id = group_id
        if not d.data_from_api:
            d.data_from_api = True
        if not d.status == "IN-SERVICE":
            d.status = "IN-SERVICE"