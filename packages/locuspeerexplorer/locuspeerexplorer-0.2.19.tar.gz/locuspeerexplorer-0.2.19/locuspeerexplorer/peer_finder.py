...
def _filter_population(df_data, area, pop):
    assert 'total_population' in df_data.columns, 'Missing population argument'
    pop_area = df_data[df_data['AREA'] == area][total_population].iloc[0]
    df_data = df_data.query('total_population >=(1-@pop)*@pop_area')
    df_data = df_data.query('total_population <(1+@pop)*@pop_area')
    return df_data
...
def get_top_n_fms_peers(
    ...
    if filter_pop is not None:
        df_data = _filter_population(df_data, area, filter_pop)
...
def get_distinguishing_features_peers(
    ...
    if filter_pop is not None:
        df_data = filter_population(df_data, area, filter_pop)
