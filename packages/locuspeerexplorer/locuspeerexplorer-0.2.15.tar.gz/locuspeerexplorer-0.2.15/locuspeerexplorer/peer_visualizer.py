...
def bar_all_fm(df_data, area, peers, cols, limit_fms=10, save_fig=True, show=False):
    ...
    fig.set_xticklabels(
        labels=df_data["FM"], rotation=20, horizontalalignment="right", fontsize="large"
    )
    plt.tight_layout()
    plt.show()
...
def duo_fm_viz(df_data, area, peers, col, save_fig=True, show=False):
    ...
    fig, axes = plt.subplots(figsize=(10, 10), ncols=2, sharex=True)
    ...
    sn.barplot(
        x=col,
        y="NAME",
        palette=sn.color_palette("Blues", n_colors=len(peers) + 1),
        data=subdata,
        ax=axes[0],
    )
    ...
    for i, p in enumerate(list(subdata["AREA"])):
        ...
        axes[1].axvline(
            x=val,
            lw=1,
            c=sn.color_palette("Blues", n_colors=subdata.shape[0]).as_hex()[i],
