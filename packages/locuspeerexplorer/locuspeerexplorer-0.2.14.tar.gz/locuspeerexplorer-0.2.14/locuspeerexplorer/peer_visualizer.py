...
def bar_all_fm(df_data, area, peers, cols, limit_fms=10, save_fig=True, show=False):
    plt.clf()
    cols = cols[:limit_fms]
    ...
    # nice_blue = sn.color_palette("Blues").as_hex()[-1]
    ...
    fig.set_xticklabels(labels=df_data["FM"], rotation=20, horizontalalignment="right")
    plt.tight_layout()
    plt.show()
...
def duo_fm_viz(df_data, area, peers, col, save_fig=True, show=False):
    ...
    sn.barplot(
        x=col,
        y="NAME",
        palette=sn.color_palette("Blues", n_colors=len(peers)),
        data=subdata,
        ax=axes[0],
    )
    ...
    axes[0].set_xlabel(f"Concentration of {fm} (in %)")
    axes[0].set_ylabel(f"")
    ...
    for i, p in enumerate(list(subdata["AREA"])):
        ...
        axes[1].axvline(
            x=val,
            lw=1,
            c=sn.color_palette("Blues", n_colors=len(peers)).as_hex()[i],
            label=_code2name(df_data, p),
        )

    axes[1].set_ylabel("% of all areas")
    axes[1].set_xlabel(f"Concentration of {fm} (in %)")
...
if __name__ == "__main__":
    ...
    duo_fm_viz(df_data, area, [35620] + peers, col=fms[30], show=True)
