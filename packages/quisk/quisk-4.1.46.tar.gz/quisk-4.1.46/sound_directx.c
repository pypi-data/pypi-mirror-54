...
static int match_name(LPCTSTR lpszDesc, LPVOID dev)
{
	int found;
	PyObject * py_unicode;
	...
	if ( ! py_unicode)
		return 0;
	py_substring = MakePyUnicode(((struct sound_dev *)dev)->name);
	if ( ! py_substring) {
        	Py_DECREF(py_unicode);
		return 0;
	}
#if PY_MAJOR_VERSION >= 3
	length = PyUnicode_GET_LENGTH(py_substring);
#else
	length = PyUnicode_GET_SIZE(py_substring);
#endif
	if (length <= 0) {
        	Py_DECREF(py_unicode);
