...
from azureml.studio.internal.error_handler import error_handler, ErrorHandler
from msrest.exceptions import HttpOperationError

from alghostDataioImport.azureml.common.workspaceutil import get_workspace, get_time_as_string
...
class ImportTabularDataModule:
    ...
    def get_path_key(store_type):
        ...
        if store_type in ['AzureSqlDatabase', 'AzurePostgreSql']:
    ...
    def process_input_dataset_request_dto(cls, input_dataset_request_dto):
        ...
        try:
            ...
            new_datapath = {}
            store_name = None or request['dataPath']['datastoreName']
    ...
    def register_dataset(cls):
        ...
        try:
            ...
            data_flow = cls.dataset.get_definition()
            logger.debug(f'Dataflow for registered requestDto:\n{data_flow.to_json()}')
            logger.info(f"{get_time_as_string()} Register input request dto is done and registered dataset id: {cls.dataset.id}")
            return cls.dataset
    ...
    def run(cls, input_dict: dict = None) -> DataFrameDirectory:
        ...
        cls.register_dataset()

        try:
            logger.info(f'Start to create output port dfd')
            if cls.dataset is not None:
                data_frame = cls.dataset.to_pandas_dataframe()
                if data_frame is None:
                    ErrorMapping.throw(InvalidDatasetError(dataset1=cls.dataset.id))
                schema = DataFrameSchema.data_frame_to_dict(data_frame)
            else:
                logger.error(f'Dataset is None and dataflow:\n{cls.dataset.get_definition().to_json()}')
                raise UnexpectedError(Exception(f'Dataset is None and dataflow:\n{cls.dataset.get_definition().to_json()}'))
