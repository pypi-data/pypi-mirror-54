...
ALLOWED_EXTENSIONS = set(["xlsx", "sav"])
...
def load():
    ...
    if g.form["file_config"] == "remove":
        ...
        if g.form["file_config"] == "excel_google_form":
        ...
        html_data = dataset.head(500).to_html(table_id="head_data", classes="table table-striped table-bordered")
...
def action():
    ...
    elif g.form["select_action"] == "recode":
        dataset = pd.read_excel("./static/uploads/" + flask.session["selected_file"])
        variable_name_type = OrderedDict()
        variable_name_type.update(dataset.dtypes.to_dict())

        return render_template("test_jquery.html", variable_name_type=variable_name_type,
                               dataset=flask.session["selected_file"])
...
def calc_file():
    ...
    return render_template("test_jquery.html", variable_name_type=variable_name_type, dataset=flask.session["selected_file"])
...
def monovariate_plot_sorted():
    ...
    chart = altair_monovariate(data=datatest, options_tipo_var=options_tipo_var, lista_ordinale=options_categorical_list)
...
def bivariate_plot():
    ...
    result.savefig("./static/images/{}.png".format(img_name))
...
def ncrosstab_output():
    ...
    return render_template("crosstab_output.html",
                           table = result.to_html(classes="draggable", table_id="corr_tab"))
...
def cluster_output():
    ...
    if len(variabile) == 2:
        ...
        print(y)
        flask.session["img.png"] = img_name
        result = sns.scatterplot(x=x, y=y, hue=nome_var, data=output)

        result.figure.savefig("./static/images/{}.png".format(img_name))
        img_link = url_for("get_image", image_name="{}.png".format(img_name))
    elif len(variabile) == 3:
        img_name = uuid.uuid4()
        x = output[variabile[0]]
        y = output[variabile[1]]
        z = output[variabile[2]]
        c = output[nome_var]
        ...
        result.scatter3D(x, y, z, c=c, cmap="tab20c", label=c)
        result.legend()
...
def post_arrive():
    ...
    print(posted)
    script = flask.session["script"]

    data = pd.read_excel("./static/uploads/" + flask.session["selected_file"])
    crosstab = eval(script, {'data': data, "pd": pd})

    recode = crosstab.copy()
    for item in range(0, len(posted)):
        row = posted[item][0]
        column = posted[item][1]
        classification = posted[item][2]
        if ((row >= crosstab.index.nlevels) and (column >= crosstab.columns.nlevels)):
            try:

                row_set = row - crosstab.index.nlevels - 1
                column_set = column - crosstab.columns.nlevels
                recode.iloc[row_set, column_set] = classification
            except:
                print("error", row_set, column_set)
...
def get_image(image_name):
    try:
        return send_from_directory(app.config["CLIENT_IMAGES"], filename= "{}.png".format(flask.session["img.png"]), as_attachment=False)
