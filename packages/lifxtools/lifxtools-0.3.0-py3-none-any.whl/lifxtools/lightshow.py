...
def print_bar(LightShow_Object,_name,_val,_seg_active,_seg_inactive,_total_segments=100):
    ...
    if not (type(_seg_active) == BarSegment or type(_seg_inactive) == BarSegment):
        ...
        if (LightShow_Object.loop_time != None):
            ...
            lrString = "({}Hz) ({:>3}Hz Δ) {}=[{}{}{}] ({})".format(target_Hz, delta_Hz, _name, active_string, inactive_string, Style.RESET_ALL, int(_val*100))
            print(lrString)
...
class LightShow:
    ...
    def __init__(self,mlifx,pyaudio,packetHz,fade,colorCycleHz=0.5,color_temp=6500,min_brightness=1,max_brightness=65535,min_saturation=0,max_saturation=65535,amp_sensitivity=9,initial_hue=0,hue_cycle_duration=5):
        ...
        self.hue_rel = 0 # relative hue (modified by external factors, isolated from normal hue)
        ...
        self.needs_update = False # make sure we don't run loops until data needed for those loops is ready
    ...
    def _set_loop(self):
        ...
        while (self.running == True):
            ...
            if (self.needs_update == True):
                ...
                self.hue_rel = self.hue + self.hue*np.sin(self.audio_proportion*self.audio_hue_impact)

                self.hue_rel = self.hue_rel % self.hue_max # MAGIC PART THAT MAKES THE VALUE WRAP AROUND PERFECTLY
                ...
                for m_tc in self.mlifx.managed_tilechains:
                    ...
                    m_tc.canvas = np.roll(m_tc.canvas, 1, axis=0) # shift canvas on axis
                ...
                while (t2-t1 < 1/self.packetHz): # loop until we've waited as long as the packetHz specifies
                    t2 = process_time()
    ...
    def _hue_loop(self):
        ...
        while (self.running == True):
            ...
            if (self.hue > self.hue_max): self.hue = self.hue - self.hue_max
            elif (self.hue < self.hue_min): self.hue = self.hue + self.hue_max
    ...
    def _intense_HSVK(self):
        ...
        h = self.hue_rel
        s = self.min_saturation + (self.max_saturation - self.min_saturation)*self.audio_proportion # regular saturation - higher levels = higher saturation
        v = self.min_brightness + (self.max_brightness - self.min_brightness)*self.audio_proportion
        k = self.color_temp
    ...
    def _whiteout_HSVK(self):
        ...
        h = self.hue_rel
        s = self.max_saturation*np.cos(self.min_saturation-self.audio_proportion) # inverse saturation - higher levels = lower saturation
        v = self.min_brightness + ((self.max_brightness-self.min_brightness)*self.audio_proportion) # regular brightness
        k = self.color_temp
    ...
    def _blackout_HSVK(self):
        ...
        h = self.hue_rel
        s = self.min_saturation + (self.max_saturation - self.min_saturation)*self.audio_proportion # regular saturation - higher levels = higher saturation
        v = self.max_brightness - (self.max_brightness - self.min_brightness)*self.audio_proportion
        k = self.color_temp
