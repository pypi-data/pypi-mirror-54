...
class ManagedLifx:
    ...
    def __init__(self,lifx,verbose=False,create_managed=True):
        ...
        self.devices, self.lights, self.tilechains = [], [], []
        self.managed_lights, self.managed_tilechains = [], []
        self.device_count, self.light_count, self.tilechain_count = None, None, None

        if (self.verbose == True): print("discovering LIFX devices...")
    ...
    def refresh_devices(self,sort=True,refresh_managed=False):
    ...
    def _sort_devices(self,get_counts=True):
        ...
        for device in self.devices:
            ...
            if (device_type == lifxlan.light.Light):
                ...
                raise TypeError("type of",device.get_label(),"is",device_type,"and we don't recognize it!")

        if (self.verbose == True): print("finished sorting devices!")
        if (self.verbose == True): self.print_sorted_lists()

        if (get_counts == True): self._get_device_counts()
