...
def get_deck_from_code(code, return_type="objects"):
    result = []
    ...
    if return_type == "objects":
        ...
        return [str(c.count) + ":" + c.card_code for c in result]
    elif return_type == "string":
        result_string = ""
        for c in result:
            result_string += str(c.count) + ":" + c.card_code + "\n"
        return result_string.rstrip("\n")


def get_code_from_deck(deck):
    str_to_ccac = False
    if type(deck) is list and type(deck[0]) is str:
        str_to_ccac = True
    ...
    if str_to_ccac:
        for i in range(len(deck)):
            card = deck[i].split(":")
            deck[i] = CardCodeAndCount(card[1], int(card[0]))
