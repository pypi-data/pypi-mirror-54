...
class BeamTrace():
    ...
    plt.show()
    '''
    def __init__(self, name=None, isCavity=True, verbose=True, wavelength=1064e-9):
        self.name = name
...
#         total_gouy = np.sign(B) * np.arccos(0.5 * (A + D))
    ...
    def print(self):
        ...
        print()
        print('BeamTrace {}'.format(self.name))
        print()
    ...
    def calculate_cavity_ABCD(self, verbose=True):
        ...
        self.check_unity(verbose=verbose)
        self.check_stability(verbose=verbose)
    ...
    def scan_cavity(self, steps=500, round_trip=False):
        ...
        dz = length_scaler * z_end / steps
        for key in self.comp_dict.keys():
            if self.comp_dict[key]['type'] == 'space':
                length = self.comp_dict[key]['length']
                if dz > length:
                    if self.verbose:
                        print()
                        print(bcolor.WARNING + 'WARNING for BeamTrace.scan_cavity() {}'.format(self.name) + bcolor.ENDC)
                        print('delta-z = {} m for number of steps = {} is larger than the space = {} m'.format(dz, steps, length))
                        print('Increasing the number of steps to lower delta-z')

                    dz = length
                    steps = int(length_scaler * z_end / length + 1)
                    if self.verbose:
                        print('New Steps = {}, new delta-z = {} m'.format(steps, dz))
        ...
        for z_temp in zz:
            if z_temp < z_end: # traveling to the end mirror
                if z_temp >= locations[next_index]: # if we've reached the next component that's not a space
                    ...
                    # Get the last component name and location
    ...
    def plot_cavity_scan(self, steps=500, round_trip=False, color='C3', ls='-', label=None, fig=None):
        ...
        if newFig:
            ...
            s1.set_title('{} Cavity Scan'.format(self.name))
