...
def prior_catalog():
    ...
    for file_name in os.listdir(data_dir):
        if re.match(file_name, '^A-COSMOS_prior.*.fits'):
...
def blind_catalog():
    data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Data', data_version())
    for file_name in os.listdir(data_dir):
        if re.match(file_name, '^A-COSMOS_blind.*.fits'):
            return os.path.join(data_dir, file_name)
    return None

def read_prior_catalog():
    catalog = Table.read(prior_catalog())
    #<TODO># homogenize
    return catalog

def read_blind_catalog():
    catalog = Table.read(blind_catalog())
    #<TODO># homogenize
    return catalog
...
def print_found_catalog_rows(catalog, finding_method, found_indices, recording_file = None):
    for i, found_index in enumerate(found_indices):
        ...
        image_file = catalog['image_file'][found_index]
        printing_str = 'Found source with %s at row %d, RA Dec %.8f %.8f, flux %s ± %s [mJy], wavelength %s [um], image_file \"%s\".'%(finding_method, found_index+1, RA, Dec, flux, flux_err, image_file)
        print(printing_str)
...
def find_source(RA = np.nan, Dec = np.nan, ID = -1, sep = 1.5):
    ...
    # otherwise search by RA Dec in both catalogs
        ...
        found_indices = np.argwhere(np.sqrt(((catalog['RA'] - RA)*np.cos(np.deg2rad(Dec)))**2 + (catalog['Dec'] - Dec)**2) <= sep)[0].tolist()
        if np.count_nonzero(found_indices) == 0:
            print('No source found in prior catalog with %s!'%(finding_method))
        else:
            with open('a3cosmos_find_source_in_prior_catalog.log', 'w') as fp:
                print('Found %d sources in prior catalog with %s!'%(np.count_nonzero(found_indices), finding_method))
                print_found_catalog_rows(catalog, finding_method, found_indices)
        # 
