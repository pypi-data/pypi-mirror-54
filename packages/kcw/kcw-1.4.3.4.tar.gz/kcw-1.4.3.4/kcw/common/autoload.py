...
from kcw.utill.db import mysql
from kcw.utill.db import mongodb
from kcw.utill.db import sqlite
from kcw.utill.cache import cache
from kcw.utill.redis import redis as rediss
from kcw.utill.http import Http
from kcw.utill import app
redis=rediss()
def M(table=None,config=None):
    """数据库操作实例
...
def set_cache(name,values,expire="no"):
    """设置缓存
        ...
        return Boolean类型
        """
    return cache.cache().set_cache(name,values,expire)
def get_cache(name):
    """获取缓存

    return 或者的值
    """
    return cache.cache().get_cache(name)
def del_cache(name):
    """删除缓存

    return Boolean类型
    """
    return cache.cache().del_cache(name)
def md5(strs):
    """md5加密"""
    m = hashlib.md5()
