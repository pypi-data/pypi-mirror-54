...
#sqlite配置
sqlite={}
sqlite['host']='127.0.0.1'
sqlite['port']=''
sqlite['user']=''
sqlite['password']=''
sqlite['db']='test'
sqlite['pattern']=False # True数据库长连接模式 False数据库短连接模式  注：建议web应用使用短连接，cli应用使用长连接
sqlite['cli']=False # 是否以cli方式运行
sqlite['dbObjcount']=False # 连接池数量（单个数据库地址链接数量），数据库链接实例数量 mysql长链接模式下有效
sqlite['break']=False #断线重连次数，0表示不重连。 注：cli模式下 10秒进行一次重连并且连接次数是当前配置的300倍
...
mongo['password']=''
mongo['db']='test'
mongo['pattern']=False # True数据库长连接模式 False数据库短连接模式  注：建议web应用使用短连接，cli应用使用长连接
mongo['cli']=False # 是否以cli方式运行
mongo['dbObjcount']=False # 连接池数量（单个数据库地址链接数量），数据库链接实例数量 mysql长链接模式下有效
mongo['break']=False #断线重连次数，0表示不重连。 注：cli模式下 10秒进行一次重连并且连接次数是当前配置的300倍
...
confkcw['version']='1.4.3.3'							#项目版本
confkcw['description']='一个快速cli和web开发框架,增加命令行程序'    #项目的简单描述
