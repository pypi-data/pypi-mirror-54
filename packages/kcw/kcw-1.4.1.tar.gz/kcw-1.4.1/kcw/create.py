import os,re,traceback,shutil,platform
# import config
...
class create:
    ...
    def init(self):
        ...
        #     # os.popen('setx PATH "%PATH%;'+get_folder()+'/kcw'+str(confkcw['version'])+'/app/binaryfile"  /M')
        #     os.popen('setx PATH "%PATH%;'+get_folder()+'/kcw/app/binaryfile"  /M')
        ...
        #         PATH=None
        #     if PATH:  #设置软链接
        #         os.system("ln -s "+get_folder()+"/kcw/binaryfile/kcw /usr/bin")
    ...
    def webapp(self):
        ...
        if os.path.exists(framepath):
            ...
            self.__execcommon(self.appname+'/'+self.name+"/common/__init__.py")
            if not os.path.exists(framepath+"/start"):
                os.makedirs(framepath+"/start")
            f=open(framepath+"/start/server.py","w+",encoding='utf-8')
            f.write('import sys\n'+
            'sys.path.append("../../../")\n'+
            'from '+self.appname+'.'+self.name+'.common import *\n'+
            ...
            'flaskapp.register_blueprint('+self.name+'app)\n'+
            'class flaskconfig():\n'+
            '    DEBUG=config.app["app_debug"]\n'+
            '    SECRET_KEY=config.app["SECRET_KEY"]\n'+
            'flaskapp.config.from_object(flaskconfig)\n'+
            ...
            '       except NameError:\n'+
            '           print("您已开启签权验证，但您却没有定义签权方法，您应该在'+self.appname+'/'+self.name+'/common/autoload.py文件中定义oauth方法并且接收一个参数")\n'+
            '       else:\n'+
            '           if not oa[0]:\n'+
            '               return oa[1]\n'+
            '@flaskapp.after_request  #请求后执行\n'+
            'def after_request(response):\n'+
            '    return response\n'+
            'if __name__ == "__main__":\n'+
            '    flaskapp.run(host="'+self.host+'",port="'+self.port+'")\n')
            f.close()
