...
class mongo:
    __config={
        'host':'mysql.fanshukeji.com', #服务器地址
        'port':'27017', #端口
        'user':'',
        'password':'', #密码
        'db':'test', #数据库名
        'pattern':False, # True数据库长连接模式 False数据库短连接模式  注：建议web应用使用短连接，cli应用使用长连接
        'cli':False,# 是否以cli方式运行
        'dbObjcount':1, # 连接池数量（单个数据库地址链接数量），数据库链接实例数量 mysql长链接模式下有效
        'break':0 #断线重连次数，0表示不重连。 注：cli模式下 10秒进行一次重连并且连接次数是当前配置的300倍
    }
    __clientobj=None
    __dbobj=None
    __tabobj=None
