...
class Http:
    ...
    get_header={} #响应头
    get_cookies=None #得到最后的响应cookie
    get_text='' #得到body响应内容
    get_content='' #得到二进制内容
    get_response='' #得到响应对象
    ...
    def httpurl(self,url,method="GET",data=None,files=None):
        ...
        try:
            ...
            if self.get_cookies:
                self.get_cookies=self.__merge(self.get_cookies,requests.utils.dict_from_cookiejar(response.cookies))
            else:
                self.get_cookies=requests.utils.dict_from_cookiejar(response.cookies)
            if self.set_cookies:
            ...
            self.get_status_code=response.status_code
            # print(self.req_header)
