...
class create:
    ...
    def init(self):
        if 'Windows' in platform.system():
            ...
            PATH=PATH.split(";")[-1]
            if PATH and not os.path.isfile(PATH+"/kcw.exe"):
                shutil.copy(get_folder()+'/kcw/file/kcw.exe',PATH+"/kcw.exe")
                print("命令行已添加到",PATH+"/kcw.exe")
        if 'Linux' in platform.system():
            if not os.path.isfile("/usr/bin"):
                os.system("ln -s "+get_folder()+"/kcw/file/kcw /usr/bin")
                print("命令行软链接已添加到","/usr/bin")
