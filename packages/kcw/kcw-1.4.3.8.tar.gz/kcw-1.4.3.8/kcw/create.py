    ...
    def init(self):
        if 'Windows' in platform.system():
            ...
            if PATH and not os.path.isfile(PATH+"/kcw.exe"):
                shutil.copy(get_folder()+'/kcw/file/kcw.exe',PATH)
                print("命令行已添加到",PATH+"/kcw.exe")
        elif 'Linux' in platform.system():
            if not os.path.isfile("/usr/bin/kcw"):
                # os.system("ln -s "+get_folder()+"/kcw/file/kcw /usr/bin/kcw")
                shutil.copy(get_folder()+"/kcw/file/kcw","/usr/bin/")
