...
class JSONFormatter(logging.Formatter):
    ...
    def json_record(self, message, extra, record):
        ...
        if '_timestamp' not in extra:
            extra['_timestamp'] = float(time.time())

        if '_time' not in extra:
            extra['_time'] = datetime.utcnow()

        extra['_so_version'] = platform.release()
        extra['_python_version'] = platform.python_version()
        extra['_service_ip'] = socket.gethostbyname(socket.gethostname())
