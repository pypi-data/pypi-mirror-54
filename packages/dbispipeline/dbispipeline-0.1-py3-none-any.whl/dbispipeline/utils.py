...
def _notify(message, subject):
    configuration = load_project_config()
    if 'mail' not in configuration:
    ...
    for field in required_fields:
        if field not in configuration['mail']:
            LOGGER.error(f'missing field in mail configuration: {field}')
            return

    recipient = configuration['mail']['recipient']
    sender = configuration['mail']['sender']
    smtp_server = configuration['mail']['smtp_server']
