...
class LofarDataCleaned:
    ...
    def load_fits(self,fname):
        if len(fname)>0:
            self.havedata = True
            self.fname = fname
            self.t_obs = sunpy.time.parse_time(header['DATE-OBS'])
...
# TODO : to make a map data structure which will be compatible with SUNPY
    ...
    def plot_image(self,fname,vmax_set=np.nan,log_scale=False):
        ...
        beam0 = Ellipse((-500, -1800), b_maj, b_min, -b_angel)
