...
class MgvTransfert(object):
    ...
    def setFileB(self):
        result = QtWidgets.QFileDialog.getSaveFileName(self.root, 'Export File', '', selectedFilter='*.json')
        if result[0]:
                ...
                for line in liste:
                    ...
                    if path.startswith('FILE:'):
                        ...
                        try:
                            ...
                            errors += '%s : json error !\n' % path[5:]
                    if key != 'To File':
                        project = MgvProject.Project(key)
                    new_obj = None
                    if code == 'TYPE':
                        if project is None or name not in [x.getName() for x in project.getTypes()]:
                            self.exportList[key].remove(line)
                            if path.startswith('FILE:'):
                                for obj in root:
                    ...
                    if code == 'HUD':
                        if project is None or name not in [x.getName() for x in project.getHud()]:
                            self.exportList[key].remove(line)
                            if path.startswith('FILE:'):
                                for obj in root:
                            ...
                            errors += '%s already exists !\n' % name
                    if code == 'BATCHSCRIPT':
                        if project is None or name not in [x.getName() for x in project.batchScripts]:
                            self.exportList[key].remove(line)
                            if path.startswith('FILE:'):
                                for obj in root:
                    ...
                    if new_obj:
                        if key == 'To File':
                            json_list.append(new_obj.getJson())
                        else:
