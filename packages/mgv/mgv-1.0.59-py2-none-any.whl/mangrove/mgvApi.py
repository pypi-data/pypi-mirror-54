        ...
        if self.node.type:
            for param in self.node.type.getParameters(version_id=self.typeForceVersion):
                ...
                if param.type == 'python':
                    v = "import sys\nsys.path.append('%s')\n%s\n%s" % (os.path.dirname(os.path.dirname(os.path.abspath(__file__))).replace('\\', '\\\\'),
...
%s
    ...
    mgvWrapper.setNodeAttr({'project_name': os.getenv('MGVPROJECTNAME'), 'path': os.getenv('MGVGRAPHFILEPATH'), 'code': 'NodeVersion', 'uuid': '%s'},
                           output=out)
""" % (os.path.dirname(os.path.dirname(os.path.abspath(__file__))), choosen_wrapper, inputs, script, self.node.getName(),
