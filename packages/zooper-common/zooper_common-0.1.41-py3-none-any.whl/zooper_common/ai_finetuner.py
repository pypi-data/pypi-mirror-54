...
class ZooperVisionFineTuner:
    ...
    def create_top_model(self, input_shape, enable_regularizer=True):
        """ Create dense (top) layer. """
        model = Sequential()
        model.add(Flatten(input_shape=input_shape))
        if enable_regularizer:
            model.add(Dense(self.dense_layer_size, activation='relu',
...
#           fill_mode='nearest',
    ...
    def create_full_model(self,
        ...
        if (weights_config['top_layer']
            ...
            print('Loading top-level weights: {} ...'.format(pretrained_weights_path))
            # top_model.load_weights(pretrained_weights_path)

        model = Model(inputs=base_model.input, outputs=top_model(base_model.output))

        if not weights_config['top_layer']:
            print('Loading full weights: {} ...'.format(pretrained_weights_path))
        ...
        if mode == 'training':
            ...
            layers = model.layers[:train_last_k_layers]
        elif mode == 'inference':
            layers = model.layers
        ...
        for layer in layers:
            ...
            if layer.name.endswith('_bn'):
                print('Freezing BN layers ... {}'.format(layer.name))
