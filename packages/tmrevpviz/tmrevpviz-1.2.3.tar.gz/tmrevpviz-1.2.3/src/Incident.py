...
class Incident:
    ...
    def __init__(self, incident_id, incident_detail_rows, csv_main, NO_CYCLE_INTERVALS):
        ...
        if eta.shape[0] == 0:
            print("Id {} did not have any 'running' states".format(self.incident_id)) #INCIDENTLOG
        ...
        if complete_rows.shape[0] == 0:
            print("Id {} did not have any 'cancelled' or 'terminated' states".format(self.incident_id)) #INCIDENTLOG
            self.is_complete = False
            return None
        else:
        ...
        if (type(self.intervention_main_row) == bool) and (self.intervention_main_row == False):
            print("Id {} did not have median_cycle for".format(self.incident_id)) #INCIDENTLOG
            print()
            self.is_complete = False
            return None
    ...
    def set_ev_accepted(self):
        # incident_detail_rows[incident_detail_rows["Incident Id"] == incident_id].iloc[0]["Update Time"]
    ...
    def get_intersection_validation(self, i_vpp, i_io, VALIDATION_WORKBOOK, VALIDATION_WORKBOOK_INDEX):
        ...
        # print("pedestian_clearance: ", pedestian_clearance)
        ...
        earliest_time = last_phase_calltime - pd.Timedelta(seconds=pedestian_clearance)
        ...
        # Validation - ETI Last time intervention phase is called (C)
        # ETI > ev_accepted : Fault
        accepted_time = self.ev_accepted.iloc[-1]['Update Time']
        if eti > accepted_time:
            diff = (eti - accepted_time).total_seconds()
            # print('======= BREAKS: Intervention phase {} is called {} s after ETI ({})'.format(intervention_label, diff, eti))
            VALIDATION_WORKSHEET.write(VALIDATION_WORKBOOK_INDEX, 0, self.incident_id)
            VALIDATION_WORKSHEET.write(VALIDATION_WORKBOOK_INDEX, 1, self.intervention_main_row["Time"], date_format)
            VALIDATION_WORKSHEET.write(VALIDATION_WORKBOOK_INDEX, 2, 'C')
            VALIDATION_WORKSHEET.write(VALIDATION_WORKBOOK_INDEX, 3, 'Intervention phase {} is called {} s after ETI ({})'.format(intervention_label, diff, eti))
            VALIDATION_WORKBOOK_INDEX += 1


        # Validattion - Last time a phase can be called (B)
        ...
        if earliest_time > intervention_start:
            # Before 6AM?
