...
def requestPost(url, data, headers, useIp=False, ssp=None,timeout=15):
  ...
  try:
    if(not header.get('User-Agent') and resource.useragent and len(resource.useragent)>0):
      header['User-Agent']=random.choice(resource.useragent)
    ...
    if(useIp or (ssp and ssp.use_ip)):
      p = url[0:url.index(':')]
      if(resource.proxyips and resource.proxyips.get(p) and len(resource.proxyips[p])>0):
        opener = _setProxy(p, random.choice(resource.proxyips[p]))
...
def requestGet(url, headers, useIp, ssp=None,timeout=10):
  ...
  try:
    ...
    if(useIp or (ssp and ssp.use_ip)):
      p = url[0:url.index(':')]
      if(resource.proxyips and resource.proxyips[p] and len(resource.proxyips[p])>0):
        opener = _setProxy(p, random.choice(resource.proxyips[p]))
    if(not opener): opener = urllib2.build_opener()
...
# print requestGet('http://en.people.cn/NMediaFile/2019/0925/FOREIGN201909251608000361930267899.JPG',None,None)
