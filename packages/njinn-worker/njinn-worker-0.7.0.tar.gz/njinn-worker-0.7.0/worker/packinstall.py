...
class PackInstallation:
    ...
    def get_pip_path(self):
        if is_windows():
            ...
            pip_path = f"{self.venv_path}/Scripts/pip.exe"
        else:
            pip_path = f"{self.venv_path}/bin/pip"
        return pip_path

    def install_requests_jwt_and_cryptography(self):
        """
        Install ``requests_jwt`` and ``cryptography`` packages.
        """

        log.debug("%s: Installing standard dependencies.", self.display_name)
        dir_path = os.path.dirname(os.path.realpath(__file__))
        subprocess.check_call(
            [self.get_pip_path(), 'install', 'requests_jwt', 'cryptography'],
    ...
    def install_requirements(self):
        ...
        """
        log.debug("%s: Installing requirements. ", self.display_name)

        requirements_path = f"{self.target_path}/requirements.txt"
        dir_path = os.path.dirname(os.path.realpath(__file__))

        if os.path.exists(requirements_path):
            log.info("Installing pack requirements from: %s", requirements_path)
            completed = subprocess.run(
                [self.get_pip_path(), 'install', '-r', requirements_path],
                universal_newlines=True,
                cwd=dir_path,
                check=True
            )
            if completed.stderr:
                log.warning("Error installing requirements: %s", completed.stderr)
        else:
