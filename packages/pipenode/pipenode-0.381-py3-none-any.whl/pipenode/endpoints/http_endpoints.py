...
class HTTPOutputEndpoint(HTTPClient, AbstractCoroutineOutputEndpoint):
    ...
    async def _put(self, task):
        while True:
            try:
                async with aiohttp.ClientSession(conn_timeout=self._timeout, raise_for_status=True) as session:
                    async with session.request(self._method, self._url,
                                               data=task.get_raw_data(),
                                               timeout=self._timeout) as response:
                        if response.status != 200:
                            logger.info("status:{}, resp:{}".format(response.status, await response.text()))
            except Exception as e:
                exc_info = (type(e), e, e.__traceback__)
