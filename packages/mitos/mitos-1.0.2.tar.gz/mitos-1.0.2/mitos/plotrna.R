...
reverselog_trans <- function(base = exp(1)) {
    ...
    trans_new(paste("reverselog-", sep="", collapse=format(base)), trans, inv, 
...
p <- p +geom_text(data = t, aes(x=mid,y=evalue,label=sname, angle=0, vjust=-0.5)) 
#	scale_y_log10( ) +
p <- p +scale_y_continuous( expand=c(0,2), trans=reverselog_trans(10), breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)) ) 
...
if( packageVersion("ggplot2") <= '0.9.1' ){
	p <- p + opts(legend.key.width = unit(1.25, "cm"), legend.position="top", legend.box ="horizontal")
}else{
