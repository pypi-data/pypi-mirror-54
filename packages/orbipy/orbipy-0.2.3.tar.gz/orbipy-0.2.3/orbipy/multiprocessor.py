...
class mp:
    def __init__(self, workname, 
        ...
        self.jobs_path = os.path.join(self.workfolder, self.workname+'_jobs.csv')
    ...
    def get_job_table(self):
        if self.jobs is not None:
            if self.log is not None:
                return pd.merge(self.jobs, self.log, on='hash', how='left')
            df = self.jobs.copy()
            df['start'] = pd.NaT
            df['done'] = pd.NaT
            df['fail'] = pd.NaT
    ...
    def append_csv(path, lst, sep=','):
        ...
        if not os.path.exists(path):
            with open(path, 'w', newline='') as f:
                ...
                writer.writerow(['datetime','hash','status'])
    ...
    def plot_stat(self, records='all', 
...
#        matplotlib.use('Qt5Agg')
    ...
    def save_proxy(N, do_save, queue, log_path, outfolder):
        '''
        ...
        while i < N:
            if queue.empty():
                ...
                if item['res'] is not None:                    
                    ...
                    job = item['job']
                    mp.append_csv(log_path,[now, job['hash'], 'done'])
                else:
                    mp.append_csv(log_path,[now, job['hash'], 'fail'])
    ...
    def calc_proxy(arg):
        ...
        try:
            t0 = datetime.now()
            mp.append_csv(log_path,[t0, job['hash'], 'start'])
