...
class PackageValidator:
    ...
    def valid_year_format(self) -> None:
        """Year value is max 4 chars long"""
    ...
    def valid_boolean_format(self) -> None:
        """Boolean values are lowercase and in {true, false}"""
        concepts = self._read_csv(self.path / 'ddf--concepts.csv')
        booleans = concepts[concepts.concept_type == 'boolean'].concept.to_list()
        for boolean in booleans:
            files = [x for x in self.path.glob('*.csv') if 'datap' in str(x) or 'entit' in str(x)]
            dfs = [self._read_csv(x) for x in files]
            dfs = [x for x in dfs if boolean in x]
            for df in dfs:
                allowed_values = ['true', 'false']
                values = df[boolean].dropna().astype(str)
                if not (values.str.lower() == values).all():
                    raise exceptions.PackageInvalidBooleanException
                if not values.isin(allowed_values).all():
                    raise exceptions.PackageInvalidBooleanException

    def valid_day_format(self) -> None:
        """Day values are 10 chars long and contain only digits"""
        files = [x for x in self.path.glob('*.csv') if '--day' in str(x.stem)]
        for file in files:
            df = self._read_csv(file)
            if not (df.day.astype(str).str.len() == 8).all():
                error = df[df.day.astype(str).str.len() != 8].iloc[0].day
                raise exceptions.PackageInvalidDayException(error)
            if not (df.day.astype(str).str.isdigit()).all():
