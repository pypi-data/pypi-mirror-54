import pandas as pd
from kyc.errors.tej_error import ColumnNotFound
...
class DataMixin(object):
    ...
    def to_pandas(self, keep_column_indexes=[], chinese_column_name=False):
        ...
        if chinese_column_name is True and 'descriptions' in self.meta.keys():
            df = pd.DataFrame(data=data, columns=self.descriptions)
            for index, type in enumerate(self.column_types):
                if type == 'Date' or type=='datetime':
                    df[self.descriptions[index]] = df[self.descriptions[index]].apply(lambda x: pd.to_datetime(x,errors='coerce'))            
