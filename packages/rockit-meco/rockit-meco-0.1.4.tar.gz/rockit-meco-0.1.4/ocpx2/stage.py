...
class Stage:
    ...
    def der(self, expr, method=None):
        # if method is None:
        #     method = {}
        # default_control = method["control"] if "control" in method else ("fd", 0)
        # default_state = method["state"] if "state" in method else ("exact", 0)
        ode = self._ode()
        return jtimes(expr, vertcat(self.x ,self.u), vertcat(ode(x=self.x, u=self.u, z=self.z, p=self.p, t=self.t)["ode"],self.du))
    ...
    def is_signal(self, expr):
        ...
        return depends_on(expr, vertcat(self.x, self.u, self.t, vcat(self.variables['control']+self.variables['states']),self.du))
    ...
    def _get_subst_set(self, **kwargs):
        ...
        if "du" in kwargs:
            subst_from.append(self.du)
            subst_to.append(kwargs["du"]) 
