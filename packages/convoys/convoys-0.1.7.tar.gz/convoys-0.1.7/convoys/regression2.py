...
from autograd.numpy import isnan, exp, dot, log, sum, power
    ...
    **Solving the optimization problem**
    ...
    def fit(self, X, B, T, W=None, fix_k=None, fix_p=None):
        ...
        def callback(LL, value_history=[]):
            ...
            sys.stdout.write('Finding MAP: %13d  (%18.6e)\n' %
        ...
        for i, g in enumerate(gradient):
            step = numpy.zeros(result['map'].shape)
            step[i] = 1
            eps = 1e-3
            print(g, result['map'][i], (f(result['map'] + eps * step) - f(result['map'])) / eps)

        step = numpy.zeros(result['map'].shape)
        step[1] = 1
        for delta in numpy.linspace(-1, 1, 100):
            print(result['map'][1] + delta, f(result['map'] + step*delta))
