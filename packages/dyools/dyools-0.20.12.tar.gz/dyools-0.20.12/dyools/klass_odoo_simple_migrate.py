    ...
    for f_name, f_spec in fields.items():
        ...
        if f_name in ['__last_update', 'write_date', 'write_uid', 'create_date', 'create_uid']:
            if f_name not in include_fields:
                continue
        if not f_spec.get('store') and not f_spec.get('company_dependent') and f_name not in include_fields:
            continue
...
class OdooSimpleMigrate(object):
    ...
    def __init__(self, src, dest):
        ...
        self._current_job_counter = Counter()
    ...
    def migrate(self,
                ...
                debug=False,
                require=False,
                func=lambda src, dest, data: data,
                unlink_domain=None,
                ):
        kwargs = {}
        ...
        self._unlink = unlink_domain if isinstance(unlink_domain, list) else None
        self._migrate()

    def _migrate(self):
        self._counter.resume()
        self._current_job_counter.restart()
        kwargs = dict()
        if self._order:
            kwargs['order'] = self._order
        if self._debug:
            Print.debug('header : {}'.format(pprint.pformat(self._fields)))
        if isinstance(self._unlink, list):
            unlink_exist_ids = self._dest.env[self._dest_model].with_context(self._dest_context).search(self._unlink, **kwargs)
            Print.info('unlink all data from the model=<{}> domain={} count={}'.format(self._dest_model, self._unlink, len(unlink_exist_ids)))
            self._dest.env[self._dest_model].browse(unlink_exist_ids).unlink()
        ...
        for offset, limit in OffsetLimit(0, self._by, self._count):
            ...
            if self._strategy == EXPORT_IMPORT_XMLID:
                ...
                data = self._trans_func(self._src.env[self._src_model], self._dest.env[self._dest_model], data['datas'])
                res = self._dest.env[self._dest_model].with_context(self._dest_context).load(self._fields,data)
                if not res.get('ids'):
                    Print.error(res)
                if self._debug:
                ...
                for line in data:
                    ...
                    line = self._trans_func(self._src.env[self._src_model], self._dest.env[self._dest_model], line)
            ...
            Print.info('progression: [{}-{}]/{} model from <{}> to <{}> current=[{}] global=[{}]'.format(
                offset,
                offset + limit,
                self._count,
                self._src_model,
                self._dest_model,
                self._current_job_counter.to_str(),
                self._counter.to_str(),
            ))
