"""File contains class for server that registers new users. Based on PyQt5 classes."""
...
class RegisterUser(QDialog):
    """
    Register constructor. Based on PyQt5 QDialog class.
    Receive database object and socket object
    """
    ...
    def save_data(self):
        """Saving method. Require username and password."""
        if not self.client_name.text():
            self.messages.critical(self, 'Error', 'No username')
            return
        elif self.client_passwd.text() != self.client_conf.text():
            self.messages.critical(self, 'Error', "Passwords doesn't match")
            return
        elif self.database.check_user(self.client_name.text()):
            self.messages.critical(self, 'Error', 'User is already exists')
            return
        else:
            ...
            self.messages.information(self, 'Done', 'User has been successfully registered')
