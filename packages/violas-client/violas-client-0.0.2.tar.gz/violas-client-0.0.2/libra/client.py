...
from libra.transaction import RawTransaction, SignedTransaction, Script, TransactionPayload, Module
...
from libra.violas_resource import ViolasResource
...
class Client:
    ...
    def submit_signed_txn(self,signed_txn):
        resp = self.submit_transaction_non_block(signed_txn)
    ...
    def publish_violas_coin(self,sender_account, is_blocking = False):
    ...
    def init_violas_coin(self,sender_account, module_address, is_blocking = False):
        script = Script.gen_violas_init_script(module_address)
        payload = TransactionPayload('Script', script)
        return self.submit_payload(sender_account, payload, is_blocking=is_blocking)

    def mint_violas_coins(self, receiver_address, micro_violas, sender_account, is_blocking=False):
        script = Script.gen_violas_mint_script(receiver_address, micro_violas, sender_account.address)
        payload = TransactionPayload('Script', script)
        return self.submit_payload(sender_account, payload, is_blocking=is_blocking)

    def transfer_violas_coin(self, sender_account, receiver_address, micro_libra, module_address,
        max_gas=140_000, unit_price=0, is_blocking=False, txn_expiration=100):
        if isinstance(receiver_address, bytes):
            receiver_address = bytes_to_hex(receiver_address)
        if isinstance(receiver_address, list):
            receiver_address = int_list_to_hex(receiver_address)
    ...
    def get_violas_balance(self,account_address,module_address):
        try:
            if isinstance(account_address, bytes):
                account_address = bytes_to_hex(account_address)
            if isinstance(account_address, list):
                account_address = int_list_to_hex(account_address)
            if isinstance(module_address, bytes):
                module_address = bytes_to_hex(module_address)
            if isinstance(module_address, list):
                module_address = int_list_to_hex(module_address)
            account_state = self.get_account_state(account_address)
            resource = account_state.get_violas_resource(module_address)
            return ViolasResource.deserialize(resource).balance
        except:
            return 0
