...
class ClientProxy:
    ...
    def publish_violas_coin(self,sender_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        self.grpc_client.publish_violas_coin(sender_account, is_blocking)

    def init_violas_coin(self,sender_address_or_refid, module_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        self.grpc_client.init_violas_coin(sender_account, module_address, is_blocking)

    def mint_violas_coins(self, receiver_address_or_refid, violas, module_address_or_refid, is_blocking):
        micro_violas = Uint64.int_safe(violas) * 1_000_000
        module_account = self.address_or_refid_to_account(module_address_or_refid)
        receiver_address = self.parse_address_or_refid(receiver_address_or_refid)
        self.grpc_client.mint_violas_coins(receiver_address, micro_violas, module_account, is_blocking)

    def transfer_violas_coins(self, sender, recevier, coin, module_address_or_refid, max_gas, unit_price, is_blocking):
        sender = self.address_or_refid_to_account(sender)
        recevier = self.parse_address_or_refid(recevier)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        micro_libra = Uint64.int_safe(coin) * 1_000_000
        self.grpc_client.transfer_violas_coin(sender, recevier, micro_libra, module_address, max_gas, unit_price, is_blocking)
        return sender.sequence_number

    def get_violas_balance(self, account_address_or_refid,module_address_or_refid):
        account_address = self.parse_address_or_refid(account_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        micro_libra = self.grpc_client.get_violas_balance(account_address,module_address)
        return micro_libra / 1_000_000
