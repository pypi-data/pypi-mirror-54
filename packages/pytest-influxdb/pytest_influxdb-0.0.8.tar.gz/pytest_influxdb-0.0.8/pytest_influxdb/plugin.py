    ...
    if pytestconfig.getoption('--pytest-influxdb'):
        def send_test_data_to_db():
            ...
            data_manager.generate_test_result_data(test_result_dto, project, version, run,
                                                   data_manager.get_report(request), merged)
...
def pytest_terminal_summary(config, terminalreporter):
    yield
    influxdb_creds = data_manager.get_valid_influxdb_creds(terminalreporter, config)
    influxdb_components = Influxdb_Components(influxdb_creds.get("db_host"), influxdb_creds.get("db_port"),
                                              influxdb_creds.get("db_user"), influxdb_creds.get("db_password"),
                                              influxdb_creds.get("db_name"))
