...
class OffenseTeamData:
    ...
    def __init__(self, seasons):
        ...
        self.teams = self.generator.pbp_get_stat(
            statName='yardsRushed',
            object='offenseTeam',
            seasons=seasons,
        )
        self.teams.drop(columns = self.teams.columns[2:],inplace = True)
        self.teams.sort_values(by ="teamId",inplace=True)
    ...
    def _build_all_metrics(self, seasons):
        ...
        for statName in self.statList:
            if _statsDict[statName]['Doit'] !='y':
                continue
            self.metrics[statName] = self.teams.copy()[["teamId","teamName"]].set_index('teamId')
