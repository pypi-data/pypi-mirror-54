...
class Parser(walker.Walker, errors.CriticalErrors):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        errors.CriticalErrors.__init__(self)
    ...
    def backup(self, kind):
        while len(self.group) != 0:
            knd, _ = self.group[-1]
            if knd not in [kind, Language.EOF]:
                break
            self.move(-1)
        return None
    ...
    def parse(self):
        ...
        for kind, value in super().__iter__():
            ...
            if kind == Language.DIGIT:
                ...
                err = errors.ErrorSource(
                        f'Unknown kind: {str(kind)}', source=''.join(value),
                        position=self.position, cursor=self.cursor)
                self.error(err)
