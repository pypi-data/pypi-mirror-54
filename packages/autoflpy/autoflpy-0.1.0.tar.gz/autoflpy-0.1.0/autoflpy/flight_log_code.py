...
code_version_name = "flight_log_code"
...
def flight_log_maker_interface(template_file_path="Data",
                               flight_data_file_path="Data",
                               arduino_flight_data_file_path="Data",
                               checklist_data_file_path="Data",
                               metar_file_path="Data",
                               icao_code="ICAO",
                               flight_log_destination="Data"):
    root = tk.Tk()
    root.configure(background="#eeeeee")
    # Changes the title bar
    root.title("Automatic Flight Log Maker")
    img = tk.Image("photo", file="images" + os.sep + "Plane.gif")
    root.tk.call("wm", "iconphoto", root._w, img)
    # Creates a title
    heading_label = tk.Label(root, text="Automatic Flight Log Maker",
                             bg="white", pady=25)
    ...
    # Creates a label for the template row.
    template_label = tk.Label(root, text="File path of " +
                              "the template notebook*")
    # Places the row label in the grid
    template_label.grid(row=2, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    template_text = tk.Text(root, height=2, width=50)
    template_text.insert(tk.END, "Data")
    template_text.grid(row=2, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def template_file_path_text():
        text = filedialog.askopenfilename(initialdir=template_file_path)
    ...
    # This is the button for the template
    template_button = tk.Button(root, text="Select file",
                                command=template_file_path_text, width="10",
                                height="2", bg="#dddddd")
    # This places the template button in the grid
    template_button.grid(row=2, column=6, columnspan=2)
    # Creates a label for the flight data
    flight_data_label = tk.Label(root, text="File path of " +
                                 "the flight data")
    # Places the row label in the grid
    flight_data_label.grid(row=3, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    flight_data_text = tk.Text(root, height=2, width=50)
    flight_data_text.insert(tk.END, "Data")
    flight_data_text.grid(row=3, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def flight_data_file_path_text():
        text = filedialog.askopenfilename(initialdir=flight_data_file_path)
        # If nothing entered, do not clear the text box.
        if text == "":
            return
        # Deletes text currently in box.
        flight_data_text.delete("1.0", tk.END)
        # Inserts text from file selecter
        flight_data_text.insert(tk.END, text)
    # This is the button for the template
    flight_data_button = tk.Button(root, text="Select file",
                                   command=flight_data_file_path_text,
                                   width="10", height="2", bg="#dddddd")
    # This places the flight data button in the grid
    flight_data_button.grid(row=3, column=6, columnspan=2)
    # Creates a label for the arduino flight data
    arduino_flight_data_label = tk.Label(root, text="File " +
                                         "path of the flight data recorded" +
                                         " by the arduino micro")
    # Places the row label in the grid
    arduino_flight_data_label.grid(row=4, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    arduino_flight_data_text = tk.Text(root, height=2, width=50)
    arduino_flight_data_text.insert(tk.END, "Data")
    arduino_flight_data_text.grid(row=4, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def arduino_flight_data_file_path_text():
        text = filedialog.askopenfilename(
            initialdir=arduino_flight_data_file_path)
        # If nothing entered, do not clear the text box.
        if text == "":
            return
        # Deletes text currently in box.
        arduino_flight_data_text.delete("1.0", tk.END)
        # Inserts text from file selecter
        arduino_flight_data_text.insert(tk.END, text)
    # This is the button for the template
    arduino_flight_data_button =\
        tk.Button(root, text="Select file",
                  command=arduino_flight_data_file_path_text, width="10",
                  height="2", bg="#dddddd")
    # This places the flight data button in the grid
    arduino_flight_data_button.grid(row=4, column=6, columnspan=2)
    # Creates a label for the arduino flight data
    checklist_data_label = tk.Label(root, text="File " +
                                    "path of the checklists")
    checklist_data_label.grid(row=5, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    checklist_data_text = tk.Text(root, height=2, width=50)
    checklist_data_text.insert(tk.END, checklist_data_file_path)
    checklist_data_text.grid(row=5, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def checklist_data_file_path_text():
        text = filedialog.askdirectory(initialdir=checklist_data_file_path)
        # If nothing entered, do not clear the text box.
        if text == "":
            return
        # Deletes text currently in box.
        checklist_data_text.delete("1.0", tk.END)
        # Inserts text from file selecter
        checklist_data_text.insert(tk.END, text)
    # This is the button for the template
    checklist_data_button =\
        tk.Button(root, text="Select folder",
                  command=checklist_data_file_path_text, width="10",
                  height="2", bg="#dddddd")
    # This places the flight data button in the grid
    checklist_data_button.grid(row=5, column=6, columnspan=2)
    # Creates a label for the arduino flight data
    metar_data_label = tk.Label(root, text="File " +
                                "path of the metar storage")
    metar_data_label.grid(row=6, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    metar_data_text = tk.Text(root, height=2, width=50)
    metar_data_text.insert(tk.END, metar_file_path)
    metar_data_text.grid(row=6, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def metar_data_file_path_text():
        text = filedialog.askdirectory(initialdir=metar_file_path)
        # If nothing entered, do not clear the text box.
        if text == "":
            return
        # Deletes text currently in box.
        metar_data_text.delete("1.0", tk.END)
        # Inserts text from file selecter
        metar_data_text.insert(tk.END, text)
    # This is the button for the template
    metar_data_button =\
        tk.Button(root, text="Select folder",
                  command=metar_data_file_path_text, width="10", height="2",
                  bg="#dddddd")
    # This places the flight data button in the grid
    metar_data_button.grid(row=6, column=6, columnspan=2)
    ...
    def four_length_and_alpha_check(entry):
        ...
        # If not it returns false
        return(False)
    four_alpha_callback = root.register(four_length_and_alpha_check)
    ...
    # Creates label for ICAO airfield
    icao_label = tk.Label(root, text="ICAO code of the" +
                          " nearest airfield.")
    # Places the row label in the grid
    icao_label.grid(row=8, column=4, sticky="w")
    # This creates the text box for the ICAO code
    icao_text = tk.Entry(root, width=10)
    icao_text.insert(tk.END, icao_code)
    # Checks data entry and ensures it is the correct format.
    icao_text.configure(validate="key", validatecommand=(four_alpha_callback,
                                                         "%P"))
    icao_text.grid(row=8, column=7, sticky="w")
    # Creates label for flight hour
    start_hour_label = tk.Label(root, text="Hour of" +
                                " take off rounded down to the nearest" +
                                " hour.")
    # Places the row label in the grid
    start_hour_label.grid(row=9, column=0, columnspan=2, sticky="w", pady=10)
    # This creates the text box for the Year
    start_hour_text = tk.Entry(root, width=10)
    start_hour_text.insert(tk.END, "Start Hour")
    # Checks data entry and ensures it is the correct format.
    start_hour_text.configure(validate="key", validatecommand=(two_callback,
                                                               "%P"))
    start_hour_text.grid(row=9, column=3, sticky="w")
    # Creates label for flight hour
    end_hour_label = tk.Label(root, text="Hour of" +
                              " landing rounded up to the nearest hour.")
    # Places the row label in the grid
    end_hour_label.grid(row=9, column=4, columnspan=3, sticky="w")
    # This creates the text box for the Year
    end_hour_text = tk.Entry(root, width=10)
    end_hour_text.insert(tk.END, "End Hour")
    # Checks data entry and ensures it is the correct format.
    end_hour_text.configure(validate="key", validatecommand=(two_callback,
                                                             "%P"))
    end_hour_text.grid(row=9, column=7, sticky="w")
    # Creates a label for the flight_log row.
    flight_log_label = tk.Label(root, text="Folder in which you would like " +
                                "the file log to be stored in*")
    # Places the row label in the grid
    flight_log_label.grid(row=14, column=0, columnspan=2, sticky="nw")
    # This creates the text box for the file path
    flight_log_text = tk.Text(root, height=2, width=50)
    flight_log_text.insert(tk.END, flight_log_destination)
    flight_log_text.grid(row=14, column=3, columnspan=3)
    # This function changes the data in the text box to match the data
    # selected.

    def flight_log_file_path_text():
        text = filedialog.askdirectory(initialdir=flight_log_destination)
        # If nothing entered, do not clear the text box.
        if text == "":
            return
        # Deletes text currently in box.
        flight_log_text.delete("1.0", tk.END)
        # Inserts text from file selecter
        flight_log_text.insert(tk.END, text)
    # This is the button for the template
    flight_log_button = tk.Button(root, text="Select folder",
                                  command=flight_log_file_path_text,
                                  width="10", height="2", bg="#dddddd")
    # This places the template button in the grid
    flight_log_button.grid(row=14, column=6, columnspan=2)

    def flight_log_maker_input():
        ...
        link = template_text.get("1.0", tk.END).replace("\\", "/").split("/")
        template_file_path = ""
        template_file_name = ""
        # Checks to see if the link is long enough to be valid.
        if len(link) >= 2:
            for location in link[:-2]:
                # Creates a string with seperators between each word in the
                # link.
                template_file_path += location + os.sep
            # Adds on the final word
            template_file_path += link[-2]
            template_file_path = template_file_path.replace("\n", "")
            # Finds the name of the template.
            template_file_name = link[-1]
            template_file_name = template_file_name.replace("\n", "")
            # Check to see if template is available.
            template_available = True
        else:
            # Check to see if template is available.
            template_available = False
        # Appends result to check_data list
        check_data.append(template_available)
        # This creates a list with removes any backslashes and replaces them
        # with forward slashes.
        link = flight_log_text.get("1.0", tk.END).replace("\\", "/").split("/")
        flight_log_file_path = ""
        # Checks to see if the link is long enough to be valid.
        if len(link) >= 1:
            for location in link[:-1]:
                flight_log_file_path += location + os.sep
            flight_log_file_path += link[-1]
            flight_log_file_path = flight_log_file_path.replace("\n", "")
            # Check to see if flight data is available.
            flight_log_file_path_available = True
        else:
            # Check to see if flight data is available.
            flight_log_file_path_available = False
        # Appends result to check_data list
        check_data.append(flight_log_file_path_available)
        # This creates a list with removes any backslashes and replaces them
        # with forward slashes.
        link = flight_data_text.get("1.0", tk.END).replace(
                "\\", "/").split("/")
        ...
        if len(link) >= 2:
            for location in link[:-2]:
                # Creates a string with seperators between each word in the
                # link.
                flight_data_file_path += location + os.sep
            # Adds on the final word
            flight_data_file_path += link[-2]
            flight_data_file_path = flight_data_file_path.replace("\n", "")
            # Finds the name of the file
            flight_data_file_name = link[-1]
            flight_data_file_name = flight_data_file_name.replace("\n", "")
        # This creates a list with removes any backslashes and replaces them
        # with forward slashes.
        link = arduino_flight_data_text.get("1.0", tk.END).replace(
            "\\", "/").split("/")
        ...
        if len(link) >= 2:
            for location in link[:-2]:
                # Creates a string with seperators between each word in the
                # link.
                arduino_flight_data_file_path += location + os.sep
            # Adds on the final word
            arduino_flight_data_file_path += link[-2]
            arduino_flight_data_file_path =\
                arduino_flight_data_file_path.replace("\n", "")
            # Finds the name of the file
            arduino_flight_data_name = link[-1]
            arduino_flight_data_name =\
                arduino_flight_data_name.replace("\n", "")
        ...
        # This creates a list with removes any backslashes and replaces them
        # with forward slashes.
        link = checklist_data_text.get("1.0", tk.END).replace(
            "\\", "/").split("/")
        checklist_file_path = ""
        # Checks to see if the link is long enough to be valid.
        if len(link) >= 1:
            for location in link[:-1]:
                # Creates a string with seperators between each word in the
                # link.
                checklist_file_path += location + os.sep
            # Adds on the final word
            checklist_file_path += link[-1]
            checklist_file_path = checklist_file_path.replace("\n", "")
        # retrieves ICAO airport code.
        icao_airfield = icao_text.get().replace("\n", "")
        # Retries and ensure that the start time entered is the correct length.
        start_time_hours = start_hour_text.get().replace("\n", "")
        if len(start_time_hours) < 2:
            start_time_hours = "0" + start_time_hours
        # Retries and ensure that the end time entered is the correct length.
        end_time_hours = end_hour_text.get().replace("\n", "")
        if len(end_time_hours) < 2:
            end_time_hours = "0" + end_time_hours
        # This creates a list with removes any backslashes and replaces them
        # with forward slashes.
        link = metar_data_text.get("1.0", tk.END).replace("\\", "/").split("/")
        metar_file_path = ""
        # Checks to see if the link is long enough to be valid.
        if len(link) >= 1:
            for location in link[:-1]:
                # Creates a string with seperators between each word in the
                # link.
                metar_file_path += location + os.sep
            # Adds on the final word
            metar_file_path += link[-1]
            metar_file_path = metar_file_path.replace("\n", "")
        ...
        if not all(check_data) is True:
            ...
            return
        # Runs flight log maker code.
        try:
            flight_log_maker(template_file_path, template_file_name,
                             flight_log_file_path, flight_data_file_path,
                             flight_data_file_name,
                             arduino_flight_data_file_path,
                             arduino_flight_data_name,
                             flight_date, flight_number,
                             "Automatic Flight Log ", checklist_file_path,
                             code_version_name, icao_airfield,
    ...
    # Text states code is ready to generate a flight log
    indicator_text = indicator.create_text(100, 7.5, text="Ready")
    ...
    generate_button = tk.Button(root, text="Generate Flight Log",
                                command=flight_log_maker_input, bg="#22ee22")
    ...
    exit_button = tk.Button(root, text="Quit",
                            command=close, width="10", bg="#ee2222")
...
def flight_log_maker(template_file_path, template_file_name,
    ...
    if checklist_file_path != "data" or checklist_file_path != "":
        ...
        contents = flight_log_checklist(filtered_frame_nominal,
                                        ...
                                        "CHECKLIST_INFORMATION", contents)[0]
        # Removes checklist related lines if no checklist data available and
        # removes checklist line labels if checklist can be used.
        if flight_log_checklist(filtered_frame_nominal,
                                filtered_frame_emergency,
                                "CHECKLIST_INFORMATION", contents)[1] is False:
...
def flight_data(file_path, file_name):
    ...
    # Excel file.
    file = pd.ExcelFile(file_path + os.sep + file_name)
