...
Visitors to the English site will then see a link labeled "français" in the footer, which goes to the French site -- and the French site will have an "English" one which goes to, well, the English one. Simple! (This feature isn't based on Springheel's own localization but on the `autonym()` function of `langcodes`, so you can use it even if Springheel itself isn't available in the comics' language yet.)
