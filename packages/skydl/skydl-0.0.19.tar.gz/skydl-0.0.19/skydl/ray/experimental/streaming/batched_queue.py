...
from ray.experimental.async_api import thread_safe_client
...
def experimental_plasma_get(object_id):
    """Get an object directly from plasma without going through object table.
    Precondition: plasma_prefetch(object_id) has been called before.
    fixed issue from ray>=0.7.5(v0.7.4 is ok): ray.exceptions.UnreconstructableError: Object beb3495e54ebcd4975589c820000000000000000 is lost (either LRU evicted or deleted by user) and cannot be reconstructed. Try increasing the object store memory available with ray.init(object_store_memory=<bytes>) or setting object store limits with ray.remote(object_store_memory=<bytes>)
    """
    plasma_id = ray.pyarrow.plasma.ObjectID(object_id)
    plasma_client = thread_safe_client(
        ray.pyarrow.plasma.connect(ray.worker.global_worker.node.plasma_store_socket_name, None, 0, 300))
    while not plasma_client.contains(plasma_id):
        pass
    return plasma_client.get(plasma_id)
...
class BatchedQueue(object):
    ...
    def _read_next_batch(self):
        ...
        # fixed issue: either LRU evicted or deleted by user
        self.read_buffer = experimental_plasma_get(self._batch_id(self.read_batch_offset))
        # ray==0.7.4:
