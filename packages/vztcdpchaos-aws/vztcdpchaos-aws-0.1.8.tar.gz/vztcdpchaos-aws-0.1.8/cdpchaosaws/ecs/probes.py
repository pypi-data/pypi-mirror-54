...
from cdpchaosaws import aws_client
...
def service_is_deploying(cluster: str,
    ...
    """
    client = aws_client("ecs", configuration)
...
def get_container_status(cluster: str,
                     service: str,
                     configuration: Configuration = None,
                     secrets: Secrets = None) -> AWSResponse:

    response =  describe_service(cluster,
                         service,
                         configuration,
                         secrets)
    services = response.get('services')
    services = services[0]
    running_count = services['runningCount']
    desired_count = services['desiredCount']
    top_events_list = services['events'][0]
    status = top_events_list['message']

    response = { "Status of the container ": status ,
                "No.of instances running" : running_count,
                "No. of instances desired" : desired_count
