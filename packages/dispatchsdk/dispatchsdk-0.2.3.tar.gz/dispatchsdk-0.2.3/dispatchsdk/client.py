...
class Client:
	...
	def __init__(self, client_id='', client_secret='', username=None, password=None, api_url="https://api.dispatch.me", auth_mode='bearer', hmac_public_key=None, hmac_secret_key=None):
		...
		# python 3-3.4 compatibility of urllib.parse for convenience
		major, minor, _, _, _ = sys.version_info
		print("detected python version: %s.%s" % (major, minor))
		if major == 3 and minor < 5:
			print("downgrading urllib.parse.quote_plus")
	...
	def __uri(self, path, query=None):
		...
		if query is not None:
			...
			full = full + '?' + urllib.parse.urlencode(full_query, quote_via=urllib.parse.quote_plus)
		return full
