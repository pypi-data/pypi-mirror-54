        ...
        try:
            ...
            for binding in book.xpath('res:binding', namespaces=NAMESPACES):
                ...
                if attr in ('authors', 'book'):
                    ...
                    value = six.text_type(binding.xpath('res:value', namespaces=NAMESPACES)[0].text)
                bookinfo[attr] = value
            print('----------', bookinfo['book'])
            set_isbn(bookinfo)
            set_author(bookinfo)
...
def set_author(bookinfo):
    ...
    try:
        ...
        auth_name = (
            tree.xpath('/rdf:RDF/rdf:Description/foaf:name',
                       namespaces=NAMESPACES)
            or tree.xpath('/rdf:RDF/rdf:Description/rdfs:label',
                          namespaces=NAMESPACES))
        if len(auth_name) > 0:
