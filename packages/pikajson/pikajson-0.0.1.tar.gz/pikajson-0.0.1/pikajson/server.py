...
class PikaConsumer:
    ...
    def __init__(self, handlers, config_path, config_section='consumer'):
    ...
    def connect(self):
        logging.warning("start connecting on host %s queue %s" % (self._host, self._queue))
    ...
    def disconnect(self):
        ...
        if self._connection is not None and not self._connection.is_closed:
            logging.warning("start server disconnection on host %s queue %s" % (self._host, self._queue))
            try:
    ...
    def start(self):
        while not self._terminate:
            try:
                self.connect()
                logging.warning("start consuming on host %s queue %s" % (self._host, self._queue))
                ...
                self.disconnect()
        logging.warning("rabbit consumer terminated on host %s queue %s" % (self._host, self._queue))
