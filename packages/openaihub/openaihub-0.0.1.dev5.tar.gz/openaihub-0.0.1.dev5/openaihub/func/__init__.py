...
def get_applications(namespace, package_name, logpath='.', loglevel='INFO', openshift=False, csv=None):
    ...
    logger.addHandler(logging.FileHandler(os.path.join(logpath, "openaihub-%s.log" % package_name)))
    if csv is None:
        csv = get_csv(namespace, package_name)
    if csv is not None:
...
def install_application(namespace, package_name, application, logpath, loglevel, openshift, operator_version=None, custom_resource=None):
    logger.setLevel(loglevel.upper())
    logger.addHandler(logging.FileHandler(os.path.join(logpath, "openaihub-%s.log" % application)))
