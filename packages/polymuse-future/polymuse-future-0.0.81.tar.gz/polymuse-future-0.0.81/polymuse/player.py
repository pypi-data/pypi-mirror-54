...
def play_3_track_no_time(input_file, raw = False, instruments = ['piano', 'guitar','choir aahs'], midi = True, midi_fname = 'default', wav = False, play = False):
    ...
    st = 'stateless/'
    models = [load_model(dutils.get_all_files(model_home + constant.type3tracks[i])[0], custom_objects= {'rmsecat_' : rmsecat(constant.depths_of_3tracks[i])}) for i in range(3)]
    print(models)
