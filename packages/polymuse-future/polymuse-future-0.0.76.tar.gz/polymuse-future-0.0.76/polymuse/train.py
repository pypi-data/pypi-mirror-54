...
"""
def train(dataset_path, maxx = 1, graph = False):
...
def train_gpu(dataset_path, maxx = 1, graph = False):
    fs = dutils.get_all_midis(dataset_path, maxx= maxx) #get all the midi files path in dataset
    ...
    for i in range(3):
        ...
        rnn_gpu.build_sFlat_model(data_gen, typ = constant.type3tracks[i], epochs=50, dev = True, cell_count= 512) # makes a models and train on Note data, sFlat representaion of midi, and then to octave encoding used for training 
    if graph : 
        #need to plot graph after traing each model. The all information regarding epochs is store in hist/model_type/model_nature/
        #hist store in json format, human redable format
