...
class Subsystem_nb:
    ...
    def __init__(self, network, state, nodes, cm=None, base=None):
    	...
    	self.sates_all_nodes=[list(x[::-1])  for x in  list(product(self.states_node, repeat=len(self.nodes))) ]
    	...
    	self.distance_matrices=np.load('distance_matrices.npy',allow_pickle=True)
    ...
    def _validate_base(self,base):
        ...
        if base!=self.network.base:
            raise NameError("Base does not match network's")
    ...
    def _shape(self, purview):
        ...
        return [self.base if i in purview else 1 for i in range(len(self.nodes))] 
    ...
    def _single_node_cause_repertoire(self,mechanism,purview):
        ...
        factor2=np.power(np.power(self.tpm.shape[0],1/len(self.nodes)), len(purview))
    ...
    def node_tpm(self, cut1, cut2):
        ...
        for element in self.nodes: #this creates the tpm for each element in the subsystem
            ...
            if remain_connections==self.nodes: # if the elment is connected to everyone (including itself), no marg. of any input
                ...
                factor=self.base* len(set(self.nodes)-set(remain_connections))
    ...
    def apply_cut(self,cut_tpm):
        ...
        self.clear_cache()
        if self.base==2: self.dummy_subsys=self._dummy()
