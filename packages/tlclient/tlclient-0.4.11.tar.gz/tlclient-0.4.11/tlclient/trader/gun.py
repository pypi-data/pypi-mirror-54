...
'''.format(FIST_TYPES)
    ...
    def cmd(self):
        ...
        parser.add_argument('-f', '--target_fist', help='which fist to send this cmd ("*") for all')
        parser.add_argument('-c', '--content', help='message content in string format (json-parsable)')
        ...
        cmd.create_fist()
        cmd.start()
        cmd.send_req_command(args.target_fist, json.loads(args.content))
        if args.wait_time > 0:
            time.sleep(args.wait_time)
        elif args.wait_time == -1:
            while not cmd.has_rsp() and not cmd.is_stopped():
                time.sleep(0.001)
        print(cmd.rsps)
        cmd.stop()
        cmd.join()