...
class GatewayHeartBeat(ctypes.Structure):
    _fields_ = [
        ('fist_type', ctypes.c_short),
        ('fist_name', ctypes.c_char * FIST_NAME_MAX_LIMIT),
        ('gateway_name', ctypes.c_char * FIST_NAME_MAX_LIMIT),
    ...
    def __str__(self):
        return '<ghb fist_type={} fist_name={} gateway_name={} valid={}>'.format(
            self.fist_type, self.fist_name, self.gateway_name, self.is_valid
        )

    def to_influx(self):
        return {
            "measurement": "GatewayHeartBeat",
            "tags": {
                "fist_type": self.fist_type,
                "fist_name": self.fist_name,
                "gateway_name": self.gateway_name
