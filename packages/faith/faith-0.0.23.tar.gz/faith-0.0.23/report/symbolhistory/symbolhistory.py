...
T = Tuple[str, 'OrderedDict[str, Any]', OptionInstrument, Optional[OptionMarketData], int]
...
class SymbolHistory(object):
    ...
    def get_current_position_report(self, symbol: str, equity: Equity) -> Tuple[str, str]:
        ...
        exp_dates = sorted(list({
            ...
            for _, _, option_instrument, _, _ in option_holding_data
        }))
        for exp_date in exp_dates:
            days_to_expire: Optional[int] = None
            current_exp_date_data: List[T] = []
            for option_key, option_details, option_instrument, market_data, quantity in option_holding_data:
                if option_instrument.exp_date == exp_date:
                    current_exp_date_data.append((option_key, option_details, option_instrument, market_data, quantity))
            ...
            if market_data is not None:
                ...
                rho_total = 0.0
                total_quantity = 0
                for _, _, _, market_data, quantity in current_exp_date_data:
            ...
            # Invidual Holding
            for option_key, option_details, _, _, _ in sorted(
    ...
    def _get_current_option_buy_or_sell_holding(
        ...
        for _, item in holding_dict.items():
            ...
            yield (option_sell_key, option_sell_details, item.option_instrument, market_data, item.quantity)
