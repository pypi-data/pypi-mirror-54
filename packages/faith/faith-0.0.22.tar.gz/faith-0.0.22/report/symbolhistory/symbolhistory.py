...
class SymbolHistory(object):
    ...
    def get_current_position_report(self, symbol: str, equity: Equity) -> Tuple[str, str]:
        ...
        # Option Buy and Sell
        option_holding_data: List[Tuple[str, OrderedDict[str, Any], OptionInstrument]] = []
        for buy_or_sell in ['Buy', 'Sell']:
            option_holding_data.extend(self._get_current_option_buy_or_sell_holding(
                event=event,
                last_price=equity.last_price,
        ...
        exp_dates = sorted(list({
            ...
            for _, _, option_instrument in option_holding_data
        }))
        for exp_date in exp_dates:
            days_to_expire: Optional[int] = None
            current_exp_date_data: List[Tuple[str, OrderedDict[str, Any], OptionInstrument]] = []
            for option_key, option_details, option_instrument in option_holding_data:
                if option_instrument.exp_date == exp_date:
                    current_exp_date_data.append((option_key, option_details, option_instrument))
            ...
            for option_key, option_details, _ in sorted(
    ...
    def _get_current_option_buy_or_sell_holding(
        ...
        last_price: float,
        buy_or_sell: str,
    ) -> 'Iterator[Tuple[str, OrderedDict[str, Any], OptionInstrument]]':
        if buy_or_sell == 'Buy':
        ...
        for _, item in holding_dict.items():
            ...
            yield (option_sell_key, option_sell_details, item.option_instrument)
            ...
            for event in symbol_events:
                if event.event_name in ('SHORT PUT', 'SHORT CALL'):
                    ...
                    context.sell_to_open(
                        ...
                        quantity=int(event.quantity) * leg.ratio_quantity,
                        price=unit_price - event.unit_price,
                        option_instrument=leg.option_instrument,
                    )
