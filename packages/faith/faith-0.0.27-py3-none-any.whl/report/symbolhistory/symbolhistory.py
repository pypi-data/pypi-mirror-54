...
class SymbolHistory(object):
    ...
    def show_current_positions(self) -> None:
        ...
        for future in as_completed(futures):
            symbol, report, profit = future.result()
            reports_by_symbol.update({symbol: report})
            total_profit += profit

        color = 'green' if total_profit >= 0 else 'red'
        print(f'Total Profit: {colored(format_price(total_profit), color)}')
        print()
    ...
    def get_current_position_report(self, symbol: str, equity: Equity) -> Tuple[str, str, float]:
        events = self.get_events(symbol=symbol)
        if len(events) == 0:
            report = f'No event for {symbol}...'
            return (symbol, report, 0.0)
        ...
        tree = LeftAligned()
        return (symbol, tree(data), total_profit)
        ...
        for _, item in holding_dict.items():
            ...
            if market_data is None:
                ...
                if market_data.medium_fill_rate_sell_price is None:
                    ...
                    profit_value = (item.avg_unit_price - market_data.medium_fill_rate_sell_price) * item.quantity * 100
