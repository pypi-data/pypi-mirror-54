...
python3 main.py casa --ml=xgboost > files/tests/maincasa-$(date "+%Y-%m-%d-%H").txt
...
python3 errors.py --api="damegender" --ml="svc" > files/tests/errorsminsvc-$(date "+%Y-%m-%d-%H").txt

if ! cmp files/tests/errorsminsvc.txt files/tests/errorsminsvc-$(date "+%Y-%m-%d-%H").txt >/dev/null 2>&1
then
    echo "errors test min svc is failing"
else
    echo "errors test min svc is ok"
fi
