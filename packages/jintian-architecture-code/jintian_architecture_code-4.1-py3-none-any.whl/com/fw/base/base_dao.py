...
class BaseDao(object):
    ...
    def delete(self, db="mysql", **kwargs):
        ...
        try:
            if db == "mysql":
                self.mysql_dao.delete_by_id(self.table_name, self.id, **kwargs)
            else:
                self.mongo_dao.remove_by_id(self.table_name, self.id, **kwargs)
    ...
    def insert(self, db="mysql", **kwargs):
        ...
        try:
            if db == "mysql":
                self.mysql_dao.insert(self, **kwargs)
            else:
                self.mongo_dao.save(self, **kwargs)
        except Exception as e:
    ...
    def update_dao(self, db="mysql", **kwargs):
        ...
        try:
            if db == "mysql":
                self.mysql_dao.update_dao(self, **kwargs)
            else:
                self.mongo_dao.save(self, **kwargs)
        except Exception as e:
