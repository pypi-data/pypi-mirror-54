...
def query_tiles(sql_client, cos_client, cos_url, target_cos_url, layers, start_date, end_date, bbox, download_dir, search_buffer_in_degrees=0, merge_tiles=True, force_bbox=True):
    if isinstance(layers, six.string_types):
        layers = [layers]
    ...
    if expanded_bbox[2] > expanded_bbox[0]:
        ...
        AND lat between {4[1]} AND {4[3]}
        AND lon between {4[0]} AND {4[2]}
        INTO {5} STORED AS parquet
        """.format(cos_url, tuple(layers), start_date, end_date, expanded_bbox, target_cos_url)
        ...
        AND date between {2} AND {3}
        AND lat between {4[1]} AND {4[3]}
        AND (lon between {4[0]} AND 180 OR lon between -180 AND {4[2]})
        INTO {5} STORED AS parquet
        """.format(cos_url, tuple(layers), start_date, end_date, expanded_bbox, target_cos_url)
        # ST_Contains(ST_WKTToSQL('BOUNDINGBOX({} {}, {} {})'), ST_Point(lon, lat))
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if bbox[2] > bbox[0]:
                    ...
                    if force_bbox:
                        ...
                        merge(tiles, merged_image)
                    merged_images.append(merged_image)
                # If the given bbox spans over anti-meridian, we have to merge into two parts. Note that this will also
                # overwrite the value of force_bbox and set it to true.
                else:
