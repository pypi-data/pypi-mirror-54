...
class Course(models.Model):
    ...
    punch_period = models.IntegerField(choices=((1,'天'),),verbose_name='周期',help_text='用于课程打卡，如不需要可不填写',null=True,blank=True)  # month/week/day
...
class UserCourse(models.Model):
    ...
    def get_user_course_punch_count(self):
        ...
        if course.punch_period==1:
            sql='''
            select count(*)from(select start_time::date from "public".bee_django_course_user_live where user_id = %s and start_time >= %s union select question_passed_at::date from "public".bee_django_course_user_course_section where user_course_id = %s and question_passed_at >= %s union select upload_at::date from "public".bee_django_course_user_assignment_image where user_course_section_id in (select id from "public".bee_django_course_user_course_section where user_course_id = %s) and upload_at >= %s union select created_at::date from "public".bee_django_course_user_assignment where user_course_section_id in (select id from "public".bee_django_course_user_course_section where user_course_id = %s) and created_at >= %s) as a;
            '''
            with connection.cursor() as cursor:
                cursor.execute(sql, [self.user.id,created_at,self.id,created_at,self.id,created_at,self.id, created_at ])
                row = cursor.fetchone()
                count = row[0]
        # 按周打卡
