import numpy as np
from ..transformation import vector_magnitude, flip_and_swap


def test_vector_magnitude():
    # test with a single row of data
    X = np.array([[1., 1., 1.]])
    result = vector_magnitude(X)
...
def test_flip_and_swap():
    ...
    # test with flip
    X = np.array([[1., 1., 1.]])
    result = flip_and_swap(X, x_flip='x', y_flip='y', z_flip='-z')
    assert np.allclose(result, np.array([[1., 1., -1.]]), atol=0.001)
    # test with swap
    X = np.array([[1., 2., 3.]])
    result = flip_and_swap(X, x_flip='y', y_flip='x', z_flip='z')
