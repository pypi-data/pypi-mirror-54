...
def test_print_tree():
    ...
    assert tree_1.getvalue() == (
        'fw://test (14 bytes / 4 files)'.ljust(72) + '\n'
        '`- a (3 bytes / 1 file)\n'
        '   `- b (10 bytes / 2 files)\n'
    )
...
def test_s3_dst():
    ...
    boto.client.assert_has_calls([
        ...
        call().head_bucket(Bucket='bucket'),
        call().put_object(Bucket='bucket', Key=ANY, Body=b'test'),
        call().delete_object(Bucket='bucket', Key=ANY),
