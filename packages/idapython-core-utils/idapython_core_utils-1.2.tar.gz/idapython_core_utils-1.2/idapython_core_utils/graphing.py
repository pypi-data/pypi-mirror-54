...
from core import function_boundaries
...
class BlockGraph():
    ...
    def find_connected_paths(self, bb_start=None, bb_end=None, co=0):
        """
        ...
        try:
            ...
            print '[!] find_connected_paths: check start & end basic blocks!'
            return None
...
def binary_graph():
    ...
    for f_ea in Functions():
        for xref in XrefsTo(f_ea, True):
            ...
            (s, e) = function_boundaries(xref.frm)
            if s:
                fg.add_edge(s, f_ea)
