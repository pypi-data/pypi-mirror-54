...
from . import model
from .fileUtils import copy_conf2logpath as _copy_conf
from . import file
...
def export(save_path=None, conf=None, pbtxt=True):
    if conf is not None:
        _env['conf'] = conf
    _make_log_dir()
    _main.run('export', conf=_env['conf'], useSpark=False, logpath=_env['logpath'], save_path=save_path, pbtxt=pbtxt)
...
def export4online(conf=None, save_data_type=None, from_pb=True, **kwargs):
    ...
    if conf == '' and kwargs:
        ...
        _main.run('export_online', conf=_env['conf'], useSpark=False, output_data_type=output_data_type,
