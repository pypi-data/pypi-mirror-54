...
class CronUnitParserBase(object):
    ...
    def _comma_handler(self, date_pointer, value):
        ...
        value_index = getattr(date_pointer, self.unit_name) % 12
    ...
    def _digit_handler(self, date_pointer, value):
    ...
    def parse(self, date_pointer=None, value=None, reverse=False, ignore_pointer=False, recalculate_parent=False):
        value = self.convert_value(value)
        current_time_value = getattr(date_pointer, self.unit_name)

        # TODO: Validate value
        if value.isdigit():
            value_of_time, unit_boundary_reached, recalculate_units = self._digit_handler(date_pointer, value)
        elif "/" in value:
            ...
            value_of_time, unit_boundary_reached, recalculate_units = self._comma_handler(date_pointer, value)
        elif "-" in value:
...
class CronDayParser(CronUnitParserBase):
    ...
    def parse(self, date_pointer=None, value=None, reverse=False, ignore_pointer=False, recalculate_parent=False):
        ...
        if value == "?":
            ...
            parsed_date, unit_boundary_reached, recalculate_units = super().parse(
        ...
        return parsed_date or date_pointer, unit_boundary_reached, recalculate_units
...
class CronMonthParser(CronUnitParserBase):
    ...
    def parse(self, date_pointer=None, value=None, reverse=False, ignore_pointer=False,
...
class CronDayOfWeekParser(CronUnitParserBase):
    ...
    def _digit_handler(self, value, date_pointer, ignore_pointer=False):
        """ Handles Cron Syntax parsing for `[0-9]*` values. """
        ...
        for week in month_calendar:
            if week[day_of_week] <= date_pointer.day and not ignore_pointer:
                continue
...
class CronYearParser(CronUnitParserBase):
    ...
    def parse(self, date_pointer=None, value=None, reverse=False, ignore_pointer=False, recalculate_parent=False):
        ...
        if value == "":
            return date_pointer, unit_boundary_reached, recalculate_units
