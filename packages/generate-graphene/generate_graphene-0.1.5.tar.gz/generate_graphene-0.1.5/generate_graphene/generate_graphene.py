...
from graphene_django_extras import DjangoObjectType, DjangoSerializerMutation, DjangoListObjectType, \
...
class GenerateGraphene:
    ...
    def get_is_excluded_field(self, m: Type[Model], field_name: str) -> bool:
        try:
            return field_name in self._model_excluded_fields[get_model_name(m)]
        except:
            return False
    ...
    def register_object_list_type(
        ...
        By default the following operations will be possible:
            ...
            - Query list of instances
        """
        if not issubclass(object_type, DjangoListObjectField):
            raise Exception(
                'Expected subclass of "DjangoListObjectField", got {}'.format(object_type)
            )
    ...
    def register_graphql_objects(
        ...
        for model_name, object_list_type in self._object_list_types.items():
            if not self.get_is_excluded_field(model, "list"):
                optional_s = "s" if model_name[-1] != "s" else ""
                field_list_name = "paginated_{}{}".format(model_name, optional_s)
                set_optionally(
                    CopiedQuery, field_list_name, AuthDjangoFilterListField(object_list_type)
                )

        for model_name, object_type in self._object_types.items():
            if not self.get_is_excluded_field(model, "single"):
                field_name = model_name
                set_optionally(
                    CopiedQuery, field_name, AuthDjangoObjectField(object_type)
                )

        for model_name, object_type in self._object_types.items():
            if not self.get_is_excluded_field(model, "list"):
                field_list_name = "{}s".format(model_name)
                set_optionally(
                    CopiedQuery, field_list_name, AuthDjangoFilterListField(object_type)
                )
        ...
        for model_name, mutation_class in self._model_mutations.items():
            ...
            if not self.get_is_excluded_field(model, "create"):
                create_name = "create_{}".format(model_name)
                set_optionally(
                    CopiedMutations, create_name, mutation_class.CreateField()
                )

            if not self.get_is_excluded_field(model, "update"):
                update_name = "update_{}".format(model_name)
                set_optionally(
                    CopiedMutations, update_name, mutation_class.UpdateField()
                )

            if not self.get_is_excluded_field(model, "delete"):
                delete_name = "delete_{}".format(model_name)
                set_optionally(
                    CopiedMutations, delete_name, mutation_class.DeleteField()
                )
