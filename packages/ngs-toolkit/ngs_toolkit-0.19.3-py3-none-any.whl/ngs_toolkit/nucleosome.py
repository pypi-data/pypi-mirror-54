...
def nucleosome_changes(analysis, samples):
    ...
    groups = list()
    for attrs, _ in list(df.groupby(
...
def investigate_nucleosome_positions(self, samples, cluster=True):
    ...
    groups = list()
    for attrs, index in list(df.groupby(
        ["library", "cell_line", "knockout", "clone"]
    ).groups.items()):
        name = "_".join([a for a in attrs if not pd.isnull(a)])
        groups.append(name)
    ...
    for group in groups:
        ...
        for region_name, bed_file in list(regions.items()):
    ...
    for group in [
        ...
        for region_name, bed_file in [list(regions.items())[-1]]:
            for label, signal_file in signal_files:
                # Skip already done
                if (
                    len(
    ...
    regions = pickle.load(open(regions_pickle, "rb"))
    sel_regions = {k: v for k, v in list(regions.items()) if "diff" in k}
    ...
    for region_name in list(sel_regions.keys()):
        df = pd.read_csv(
            os.path.join(
                output_dir,
                "{}.coverage_matrix.csv".format(
...
def phasograms(
    ...
    for i, group in enumerate(groups):
        ...
        y = y.ix[list(range(plotting_window[0], plotting_window[1]))]
        y /= y.sum()
    ...
    for i, group in enumerate(groups):
        ...
        y = x.value_counts().sort_index()
        y = y.ix[list(range(plotting_window[0], 300))]
        y /= y.sum()
