...
def read_transcription_file(file, header = False, replicates = False, sep = '\t', 
    binning = False, quant_max = 0.9, quant_min = 0.5, step = 0.125, norm = True, factor = 1e9):
    ...
    factor : numeric
        ...
        Default is 1e6 (Reads per million)
    '''
    abund_dict = {}
...
def contextualize(model, transcriptome, samples = 500, 
    ...
    min_coefficient_dict, max_coefficient_dict, gene_hits = _assign_coefficients(transcriptome, riptide_model, minimum, gpr, defined)
...
def _assign_coefficients(raw_transcription_dict, model, minimum, gpr, defined_coefficients):
