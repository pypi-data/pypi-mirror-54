...
model_weights_file_name = '/usr/local/lib/python3.6/site-packages/deepcon/weights-rdd-covariance.hdf5'
...
def main(aln, file_rr, png=None):
    ...
    input = tf.keras.layers.BatchNormalization()(input_original)
    input = tf.keras.layers.Activation('relu')(input)
    input = tf.keras.layers.Convolution2D(128, 1, padding = 'same')(input)
    input = tf.keras.layers.Reshape((L, L, 128, 1))(input)
    input = tf.keras.layers.MaxPooling3D(pool_size = (1, 1, 2))(input)
    input = tf.keras.layers.Reshape((L, L, 64 ))(input)
    tower = input
    # End - Maxout Layer
    n_channels = 64
    d_rate = 1
    for i in range(32):
        block = tf.keras.layers.BatchNormalization()(tower)
        block = tf.keras.layers.Activation('relu')(block)
        block = tf.keras.layers.Convolution2D(64, 3, padding = 'same')(block)
        block = tf.keras.layers.Dropout(dropout_value)(block)
        block = tf.keras.layers.Activation('relu')(block)
        block = tf.keras.layers.Convolution2D(n_channels, 3, dilation_rate=(d_rate, d_rate), padding = 'same')(block)
    ...
    tower = tf.keras.layers.Convolution2D(1, 3, padding = 'same')(tower)
    output = tf.keras.layers.Activation('sigmoid')(tower)
    model = tf.keras.models.Model(input_original, output)
    print ('')
