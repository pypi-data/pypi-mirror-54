...
if __name__ == '__main__':
    lib = pd.read_csv("data/rescue/KY_Library_v1.1_annotated.csv", index_col=0).dropna(subset=["GENES"])
    ...
    dfs = {f: pd.read_csv(f"data/rescue/{f}", index_col=0, sep="\t").groupby(lib["GENES"]).median() for f in dfiles}

    #
    dfs_gene = pd.concat([dfs[f].drop(columns=["Plasmid_v1.1"]).iloc[:, 0].rename(f.split(".")[0]) for f in dfs], axis=1, sort=False)
    dfs_gene = pd.concat([dfs_gene, lib.groupby("GENES")["EXONE"].count().rename("N_SGRNAS")], axis=1, sort=False)
    dfs_gene.to_excel(f"reports/rescue/rescue_gene_level_counts_mean.xlsx")
    ...
    for k, genes in highlights.items():
        ...
        for i in [0, 1]:
            for j in [0, 1]:
                ...
                for g in genes:
                    ax.scatter(x.loc[g], y.loc[g], c=genes[g], marker="o", edgecolor="white", linewidth=.3, s=10, label=g)
        ...
        plt.savefig(f'reports/rescue/rawcounts_scatter_{k}_gene_level.png', bbox_inches='tight')
        plt.close('all')
        ...
        for g in genes:
            plt.scatter(x.loc[g], y.loc[g], c=genes[g], marker="o", edgecolor="white", linewidth=.3, s=10, label=g)
        ...
        plt.savefig(f'reports/rescue/rawcounts_scatter_averge_{k}_gene_level.png', bbox_inches='tight')
        plt.close('all')
        ...
        for g in genes:
            plt.scatter(
                plot_df.loc[g, "index"], plot_df.loc[g, "fc"], c=genes[g], marker="o", edgecolor="white",
        ...
        plt.savefig(f'reports/rescue/rawcounts_scatter_foldchange_{k}_gene_level.png', bbox_inches='tight')
        plt.close('all')
