...
def create_app():
    ...
    def runtime_log(tid):
        ...
        if task:
            with open(task['runtime_log']) as outfile:
    ...
    def run_api():
        ...
        if request.method == 'POST' and project_id:
            ...
            runner = Worker(alltasks, original_input, request_id)
            response = runner.run()
            return response
        else:
    ...
    def create_project(id=None):
        ...
        if request.method == 'POST' and form.validate_on_submit():
            projects.insert(dict(id=uuid1().hex, name=form.name.data))
    ...
    def create_task(pid):
        ...
        if request.method == 'POST' and form.validate_on_submit():
            ...
            tasks.insert(dict(id=task_id, pid=pid, name=form.name.data, weight=form.weight.data, script=form.python_script.data, runtime_log=task_log_path))
