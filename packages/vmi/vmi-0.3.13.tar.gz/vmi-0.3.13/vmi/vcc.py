...
def ccMatchPd_ClosedPts(closed_pts: List[Union[np.ndarray, List[float]]],
                        match_vt: Union[np.ndarray, List[float]],
                        base: vtk.vtkPolyData, thick_min: float = 2) -> TopoDS_Shape:
    """
    ...
    zmin, zmax = min(z_pt), max(z_pt)
    zmin_pt = z_pt[zmin] - thick_min * match_vt
    zmax_pt = z_pt[zmax] + thick_min * match_vt

    top_pts = [vmi.ptOnPlane(pt, zmin_pt, match_vt) for pt in closed_pts]
    top_pd = vmi.pdTriangulate_Polyline(top_pts)
    top_pd = vmi.pdRemesh(top_pd)

    bottom_pd = vmi.pdRayCast_Pd(top_pd, match_vt, base, zmax_pt)
    ...
    top_pts = [vmi.ptOnPlane(pt, zmin_pt, match_vt) for pt in bottom_pts]
    top_wire = vmi.ccWire(vmi.ccSegments(top_pts, True))
    top_face = vmi.ccFace(top_wire)
