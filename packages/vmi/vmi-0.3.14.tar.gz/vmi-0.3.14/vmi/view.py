...
class View(QGLWidget, Menu, Mouse):
    ...
    def mouseOnFPlane(self):
        return self.pickFPlane(self._MousePos)
    ...
    def pickFPlane(self, pos: Union[QPoint, List[float], None] = None):
        """拾取场景的焦平面
    ...
    def camera(self, *args) -> vtk.vtkCamera:
        if len(args) == 0:
...
def pickle_View(instance: View):
    data = ['_Renderer', '_RenderWindow', '_Interactor', '_OrientationMarker', '_Camera']
    data = {kw: vmi.gets_vtkInstance(getattr(instance, kw)) for kw in data}
...
def unpickle_View(data):
    instance = View()
    for kw in data:
        vmi.sets_vtkInstance(getattr(instance, kw), data[kw])
    return instance
