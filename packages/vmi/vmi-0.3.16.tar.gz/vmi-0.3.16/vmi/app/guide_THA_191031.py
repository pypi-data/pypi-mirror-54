...
def init_voi():
    ...
    pd = vmi.imIsosurface(voi_image, 2500)
    metal_prop.clone(pd)
...
def update_cup_camera_plane():
    ...
    cs.orthogonalize([0])
    cs.origin(cup_cs.origin() - 2.5 * cup_radius * cs.axis(0) - 2.5 * cup_radius * cs.axis(1))
    guide_view.setCamera_FPlane(cs, 5 * cup_radius)
...
def update_guide_path():
    ...
    pts = [centers[0] - 100 * cup_cs.axis(2), centers[0] + 50 * cup_cs.axis(2)]
    line = vmi.ccPd_Sh(vmi.ccEdge(vmi.ccSegment(pts[0], pts[1])))
    circle = vmi.ccPd_Sh(vmi.ccEdge(vmi.ccCircle(outer_radius, centers[0], css[0].axis(2), css[0].axis(0))))
    hole = vmi.ccPd_Sh(vmi.ccEdge(vmi.ccCircle(hole_radius, centers[0], css[0].axis(2), css[0].axis(0))))
    guide_axes_prop[0].clone(vmi.pdAppend([line, circle, hole]))
...
def update_guide_plate():
    ...
    top_edges = [vmi.ccEdge(vmi.ccCircle(hole_radius, pt, cup_cs.axis(2), cup_cs.axis(0))) for pt in tops]
...
def LeftButtonPressRelease(**kwargs):
    global pelvis_cs, cup_radius, cup_cs, cup_RA, cup_RI, hole_radius
    if kwargs['picked'] is bone_value_box:
        ...
        visible = pelvis_prop.visibleToggle()
        pelvis_visible_box.draw_text(back_color=QColor.fromRgbF(1, 1, 0.6) if visible else QColor('black'))
    elif kwargs['picked'] is metal_visible_box:
        visible = metal_prop.visibleToggle()
        metal_visible_box.draw_text(back_color=QColor.fromRgbF(1, 0.4, 0.4) if visible else QColor('black'))
    elif kwargs['picked'] is cup_radius_box:
        r = vmi.askInt(5, cup_radius, 50)
        if r is not None:
            cup_radius = r
...
def NoButtonWheel(**kwargs):
    global cup_radius, cup_RA, cup_RI, hole_radius
    if kwargs['picked'] is bone_value_box:
        global bone_value
...
if __name__ == '__main__':
    ...
    metal_prop = vmi.PolyActor(pelvis_view, rgb=[1, 0.4, 0.4])
    ...
    guide_axes_prop = [vmi.PolyActor(guide_view, rgb=[0.4, 1, 0.4], line=3) for i in range(5)]
