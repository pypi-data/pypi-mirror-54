...
def ccMatchPd_ClosedPts(closed_pts: List[Union[np.ndarray, List[float]]], match_vt: Union[np.ndarray, List[float]],
                        match_dmax: float, base_poly: vtk.vtkPolyData, base_offset: float = 0,
                        thick_vt: Union[np.ndarray, List[float]] = None, edge_length: float = 1,
                        remesh_num: float = 10) -> TopoDS_Shape:
    """
    ...
    :param edge_length: 匹配面网格的尺寸大小
    :param remesh_num: 匹配面网格优化的迭代次数
    :return: 与模型相匹配的实体
    """
    ...
    top_pd = vmi.pdRemesh(top_pd, edge_length, remesh_num)
    ...
    # z_pt = {cs.inv().mpt(bottom_pd.GetPoint(i))[2]: bottom_pd.GetPoint(i)
    #         for i in range(bottom_pd.GetNumberOfPoints())}
    # zmin = min(z_pt)

    if thick_vt is not None:
        trsf = gp_Trsf()
        trsf.SetTranslation(ccVec(thick_vt))
        t = BRepBuilderAPI_Transform(trsf)

        t.Perform(bottom_wire, True)
        top_wire = t.ModifiedShape(bottom_wire)

        t.Perform(bottom_face, True)
        top_face = t.ModifiedShape(bottom_face)
    else:
