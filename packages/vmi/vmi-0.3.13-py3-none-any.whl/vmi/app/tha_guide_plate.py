...
def update_guide_axis():
    outer_radius = hole_radius + 2
    css = [cup_cs.rotate(-original_LR * guide_angles[0], cup_cs.axis(2)),
           cup_cs.rotate(-original_LR * guide_angles[1], cup_cs.axis(2)),
           cup_cs.copy()]
    centers = [css[0].mpt([guide_radiuss[0], 0, 0]), css[1].mpt([guide_radiuss[1], 0, 0]), cup_cs.mpt([0, 0, 0])]
    for i in range(len(centers)):
        pts = [centers[i] - 50 * cup_cs.axis(2), centers[i] + 50 * cup_cs.axis(2)]
        ...
        guide_axes_prop[i].clone(vmi.pdAppend([line, circle]))

    pts = [css[0].mpt([guide_radiuss[0] + outer_radius, -outer_radius, 0]),
           css[0].mpt([guide_radiuss[0] + outer_radius, outer_radius, 0]),
           css[1].mpt([guide_radiuss[1] + outer_radius, -outer_radius, 0]),
           css[1].mpt([guide_radiuss[1] + outer_radius, outer_radius, 0]),
           css[1].mpt([guide_radiuss[1] - outer_radius, outer_radius, 0]),
           css[1].mpt([guide_radiuss[1] - outer_radius, outer_radius, 0]),
    ...
    tgs = [css[0].mvt([0, 1, 0]),
           ...
           css[2].mvt([0, -1, 0]),
           css[2].mvt([1, 0, 0])]
    tgflags = [True, True, True, False, True, True, True, True]
    print(pts, tgs, tgflags)
    sh = vmi.ccWire(vmi.ccBSpline_Kochanek(pts, True))
    guide_path_prop.clone(sh)
...
def LeftButtonPressRelease(**kwargs):
    ...
    if kwargs['picked'] is bone_value_box:
        ...
        r = vmi.askInt(-180, guide_angles[i], 180)
        if r is not None:
            guide_angles[i] = r
            guide_angle_boxs[i].draw_text(['上', '后'][i] + '位角：{}°'.format(guide_angles[i]))
            update_guide_axis()
...
def NoButtonWheel(**kwargs):
    ...
    if kwargs['picked'] is bone_value_box:
        ...
        i = guide_angle_boxs.index(kwargs['picked'])
        guide_angles[i] = min(max(guide_angles[i] + kwargs['delta'], -180), 180)
        guide_angle_boxs[i].draw_text(['上', '后'][i] + '位角：{}°'.format(guide_angles[i]))
        update_guide_axis()
...
if __name__ == '__main__':
    ...
    guide_path_prop = vmi.PolyActor(guide_view, rgb=[0.4, 1, 0.4], line=3)
    guide_axes_prop = [vmi.PolyActor(guide_view, rgb=[0.4, 1, 0.4], line=3),
                       vmi.PolyActor(guide_view, rgb=[0.4, 1, 0.4], line=3),
                       vmi.PolyActor(guide_view, rgb=[0.4, 1, 0.4], line=3)]
    ...
    aceta_visible_box = vmi.TextBox(guide_view, size=[0.03, 0.01], pos=[0, 0.04], anchor=[0, 0],
                                    back_color=QColor.fromRgbF(1, 1, 0.6), pickable=True)
    aceta_visible_box.mouse['LeftButton']['PressRelease'] = LeftButtonPressRelease

    guide_plate_visible_box = vmi.TextBox(guide_view, size=[0.03, 0.01], pos=[0.03, 0.04], anchor=[0, 0],
                                          back_color=QColor.fromRgbF(0.4, 0.6, 1), pickable=True)
