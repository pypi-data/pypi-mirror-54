...
class Constraint:
    ...
    def fit(self, field, value):
        ...
        if max_ and max_ < self.compare(value):
            return ModelError.FIELD_FORMAT((self.error_template % 'bigger').format(
                field.name, field.verbose_name, max_
            ))
        if min_ and min_ > self.compare(value):
            return ModelError.FIELD_FORMAT((self.error_template % 'smaller').format(
        ...
        def validate(value):
            for constraint in CONSTRAINTS:
                if isinstance(field, constraint.field):
                    if not isinstance(value, constraint.type):
                        return ModelError.FIELD_FORMAT('%s(%s) has wrong type' % (attr, verbose))
            ...
            if field.choices:
                ...
                for choice in field.choices:
                    if value == choice[0]:
                        ...
                        break
                if not choice_match:
                    return ModelError.FIELD_FORMAT('%s(%s) is beyond choices' % (attr, verbose))
            ...
            if callable(custom_validator):
                try:
                    ...
                    return PError.VALIDATOR_CRUSHED(attr, verbose)
        ...
        for attr in attr_jar:
            ...
            if field:
                try:
                    ...
                    return PError.VALIDATOR_CRUSHED(attr, field.verbose_name)
            else:
                custom_validator = getattr(cls, '_valid_%s' % attr, None)
                if callable(custom_validator):
                    try:
                        ...
                        return PError.VALIDATOR_CRUSHED(attr, attr)
