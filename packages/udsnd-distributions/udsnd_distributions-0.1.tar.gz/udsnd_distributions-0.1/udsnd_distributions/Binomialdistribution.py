...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        part_one = math.factorial(self.n)/(math.factorial(k) * math.factorial(self.n - k))
        part_two = math.pow(self.p, k) * math.pow(1 - self.p, self.n - k) 
        # part_two = (self.p ** k) * ((1 - self.p) ** (self.n - k))
    ...
    def plot_bar_pdf(self):
        ...
        # Creating the values at which the PDF is calculated
        x_values = list(range(0, self.n + 1, 1))

        # Getting the PDF values
        y_values = []
        for value in x_values:
           y_values.append(self.pdf(value))
