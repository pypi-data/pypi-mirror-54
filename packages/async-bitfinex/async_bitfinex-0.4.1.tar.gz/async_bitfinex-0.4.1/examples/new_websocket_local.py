...
async def main():
    ...
    # trades_future1 = client.subscribe_to_trades(symbol="BTCUSD", callback=async_print, timeout=3)
    # channel_details = await trades_future1

    # store the channel detials
    # self.channels[channel_details["chanId"]] = channel_details

    # Now you can fetch all details related to a channel using its channel id
    # the channel id is the first in every list returned from bitfinex
    # or stored in "chanId" for error and subscribed messages
    # self.channels[channel_details["chanId"]]
    ...
    #     result1 = await client.unsubscribe("trades", channel["chanId"])
    #     print(result1)
    ...
    canceled = await cancel_order(client, order_id=order_response["id"])
    print(canceled)
