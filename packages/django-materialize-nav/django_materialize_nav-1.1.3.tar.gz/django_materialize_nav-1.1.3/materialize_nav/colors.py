...
__all__ = ['COLORS', 'get_rgb', 'modify_val', 'lighten', 'darken', 'saturate', 'desaturate', 'opacify',
    ...
    if isinstance(color, str):
        if not color.startswith("#"):
            if color not in COLORS:
                raise ValueError("Invalid color name given {}!".format(repr(color)))
            color = COLORS[color]
        color = color[1:]
        length = len(color)
...
def lighten(color, amount):
    ...
    hls = colorsys.rgb_to_hls(*(c/255 for c in color[:3]))
    ...
    return "#{:02x}{:02x}{:02x}{:02x}".format(*new_rgb, alpha)
