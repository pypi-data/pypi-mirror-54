...
class CryptoICA(BaseCryptoService):
    ...
    def sign(self, session, alias, fingerprint, password=None, hash_alg=None, sign_alg=None):
        ...
        self.last_reader = readers[0]  # type: TokenRecord
        ...
        self.server.cmd(session, '00A4000C023F00', self.last_reader.reader, 1)
        response_all = self.server.cmd(session, '00A4010C020604', self.last_reader.reader)

        response_data = BaseCryptoService.check_response(response_all, self.last_reader.reader, "Selection before signing failed")
        ...
        if password is not None:
            ...
            if (self.last_reader.pin is not None) and (self.last_reader.pin == encoded_password):
                logging.error("Blocked repeated use of incorrect PIN at reader %s, remaining tries %s"
                              % (self.last_reader.reader, response_all[-1:]))
                # pin_ok = False
                response_data = Constant.CMD_READER_WRONG_PIN
            else:
                password_command = '00200086%02X%s' % (len(password), encoded_password)
                response_all = self.server.cmd(session, password_command, self.last_reader.reader)

                response_data = BaseCryptoService.check_response(response_all, self.last_reader.reader, "Error verifying PIN")
                if response_data is None:  # all good
                    self.last_reader.pin = None

                    # different versions of card need different PIN version
                    password_command = '00200081%02X%s' % (len(password), encoded_password)
                    response_all = self.server.cmd(session, password_command, self.last_reader.reader)
                    response_data = \
                        BaseCryptoService.check_response(response_all, self.last_reader.reader, "Error verifying PIN2")
                    ...
                    logging.error("Incorrect PIN to reader %s, remaining tries %s"
                                  % (self.last_reader.reader, response_all[0][-1:]))
                    self.last_reader.pin = encoded_password
                    # pin_ok = False
                    response_data = Constant.CMD_READER_WRONG_PIN
        ...
        if response_data is None:
            ...
            keep_trying = True
            response_data = None
            temp_file_id = self.last_reader.file_id
            ...
            while keep_trying:
                # h MSE:Set sha-256
                self.server.cmd(session, '002241AA04890214%s' % sha_id, self.last_reader.reader)
                # h MSE:Set DST
                response_all = self.server.cmd(session, '002241B60A 840380%02X00 89031323%s' % (temp_file_id, sha_id),
                                               self.last_reader.reader)

                response_data = BaseCryptoService.check_response(response_all, self.last_reader.reader,
                ...
                if response_data is None:
                    ...
                    response_all = self.server.cmd(session, "002A9E9A00", self.last_reader.reader)
                    response_data = BaseCryptoService.check_response(
                        response_all, self.last_reader.reader, "Signing unsuccessful id=%d" % temp_file_id, error=0)

                    if response_data is None:  # success
                        if self.last_reader.file_id == 0:
                            self._update_file_id(self.last_reader, temp_file_id)
                        ...
                        if self.last_reader.file_id == 0 and key_id_limit > 0:
                            key_id_limit -= 1
                            temp_file_id -= 1
                        else:
                            keep_trying = False
