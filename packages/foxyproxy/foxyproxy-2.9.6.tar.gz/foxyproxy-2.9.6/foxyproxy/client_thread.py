...
class ClientThread(threading.Thread):
    ...
    MAX_COUNTER_RUNS = 2  # maximum number of requests to process
    ...
    def run_tcp(self):
        ...
        try:
            ...
            for command in self.req_commands:
                ...
                if self.signer.last_reader is not None:
                    self.sync_back.reader = self.signer.last_reader.reader
                    self.sync_back.pin = self.signer.last_reader.pin
                    self.sync_back.file_id = self.signer.last_reader.file_id
