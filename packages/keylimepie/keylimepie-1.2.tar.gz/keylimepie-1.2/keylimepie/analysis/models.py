...
class diskmodel:
    ...
    def __init__(self, path, verbose=False, mindens=1e3, **kwargs):
        ...
        # Currently only important are the grid [1] and EnergyLevel populations [4]
    ...
    def grid_levels(self, nlevels):
        ...
        for j in np.arange(nlevels):
            if j in list(self.gridded['levels'].keys()):
                continue
...
class slabmodel:
    ...
    def __init__(self, path, verbose=False, mindens=1e3, **kwargs):
        ...
        # Read in the .fits data. HDU[1] is the grid and HDU[4] are the EnergyLevel
