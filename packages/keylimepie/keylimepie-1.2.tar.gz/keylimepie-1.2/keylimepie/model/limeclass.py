...
import keylimepie
from keylimepie.model.headerclass import Header
from keylimepie.analysis.collisionalrates import LamdaFile
...
class Model:
    ...
    def __init__(
        ...
        self.aux = os.path.join(keylimepie.__path__[0], '_aux')
        self.directory = directory

        # Check whether the required files are there. If so, move them.
        header_path = os.path.abspath(os.path.join('..', header))
        if not os.path.isfile(header_path):
            raise ValueError(f'No header file found at {header_path}.')
        else:
            shutil.copy(header_path, os.getcwd())
            self.header = Header(header_path)

        rates_path = os.path.join(self.aux, rates)
        if not os.path.isfile(rates_path):
            raise ValueError(f'No collisional rates found for {rates}.')
        else:
            shutil.copy(rates_path, os.getcwd())
            self.moldatfile = rates
            self.rates = LamdaFile(rates)

        self.dust = kwargs.get('dust', None)
        if self.dust is not None:
            dust_path = os.path.join(self.aux, self.dust)
            if not os.path.isfile(dust_path):
                raise ValueError('No dust opacities found.')
            shutil.copy(dust_path, os.getcwd())
        ...
        try:
            self.g2d = self.header.params['gastodust']
        except:
        ...
        self.cmdprefix = kwargs.get('cmdprefix', '')
        self.cmdsuffix = kwargs.get('cmdsuffix', '&')
