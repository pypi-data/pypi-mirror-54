...
def makeFile(m, model, save=True):
    """Generate a model_m.c file and write it to disk unless save=False"""
...
def writeInterpolationFuncs(temp, model):
    ...
    path = os.path.join(keylimepie.__path__[0], '_interpolation')
    with open(os.path.join(path, '%dD_%s.c' % (model.ndim, model.coordsys))) as f:
        lines = f.readlines()
    for line in lines:
...
def writeGastoDust(temp, model, ming2d=1.):
    ...
    if type(model.g2d) is float:
        ...
        temp.append('\t*gtd = findvalue(x, y, z, gastodust);\n\n')
