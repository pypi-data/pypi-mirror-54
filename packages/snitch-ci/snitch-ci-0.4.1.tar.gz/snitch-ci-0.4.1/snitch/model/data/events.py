...
class MouseImageClick(Event):
    ...
    def __str__(self):
        return 'CLICK: [{0}#{1}] @{2}'.format(self.path, self.index, self.time)
...
class MouseClick(Event):
    ...
    def __str__(self):
        return '{3}CLICK: [x={0}, y={1}] @{2}'.format(
            self.x,
            self.y,
            self.time,
...
class MouseDrag(MouseClick):
    ...
    def __str__(self):
        return 'DRAG: [x0={0}, y0={1}] -> [x1={2}, y1={3}] @{4}'.format(
            self.x, self.y, self.xx, self.yy, self.time)
...
class MouseAltClick(MouseClick):
    ...
    def __str__(self):
        return '{3}ALT CLICK: [x={0}, y={1}] @{2}'.format(
            self.x,
            self.y,
            self.time,
            format_modifiers(self.modifiers))
...
class MouseDoubleClick(MouseClick):
    ...
    def __str__(self):
        return '{3}DOUBLE CLICK: [x={0}, y={1}] @{2}'.format(
            self.x,
            self.y,
            self.time,
            format_modifiers(self.modifiers))
...
class TextEntered(Event):
    ...
    def __str__(self):
        ...
        if self.text is not None:
            txt = ''.join(self.text)
        return 'TYPE: [txt={0}] @{1}'.format(txt, self.time)
...
class Delay(Event):
    ...
    def __str__(self):
        return 'PAUSE: [dur={0}]'.format(self.duration)

    def execute(self):
        log.debug('»»» waiting %dms', self.duration)
