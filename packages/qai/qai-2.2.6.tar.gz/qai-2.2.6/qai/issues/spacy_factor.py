...
class SpacyFactor(object):
    ...
    @staticmethod
    def match_case(spacy_span, suggestions):
        """
        ...
        try:
            if spacy_span and len(spacy_span):
                if spacy_span[0].text.islower():
                    suggestions = [s.lower() for s in suggestions]

                # len condition: exclude capitalization caused by "I"
                elif len(spacy_span[0].text) > 1 and spacy_span[0].text.isupper():
                    suggestions = [s.upper() for s in suggestions]

                # title case - first letter check & conversion due to contractions
                elif len(spacy_span[0].text) > 1 and spacy_span[0].text[0].isupper():
                    for i, s in enumerate(suggestions):
                        if len(s.strip()):
                            words = s.split(" ")
                            suggestions[i] = " ".join(
                                [words[0][0].upper() + words[0][1:]] + words[1:]
                            ).strip()
        except:
            suggestions_log = ",".join(suggestions)
            print(
                f"Case matching failed for span:{spacy_span.text} and {suggestions_log}"
            )
