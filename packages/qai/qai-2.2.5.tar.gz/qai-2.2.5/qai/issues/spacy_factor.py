...
class SpacyFactor(object):
    ...
    def match_case(spacy_span, suggestions):
        ...
        if spacy_span and len(spacy_span):
            ...
            elif spacy_span[0].text.isupper() and len(spacy_span[0].text)>1:
                suggestions = [s.upper() for s in suggestions]

            elif spacy_span[0].text.istitle() and len(spacy_span[0].text)>1:
                for i, s in enumerate(suggestions):
                    words = s.split(" ")
                    suggestions[i] = " ".join([words[0].title()] + words[1:]).strip()
