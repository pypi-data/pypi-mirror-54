...
class Spiral(gdspy.Cell):
    ...
    def get_number_of_spirals(self, corner_dl):
        ...
        for n in range(50):  # Do not exceed 50 iterations
            ...
            print(
                "For spiral with n="
                + str(n)
                + " loops, minimum length in constrained area is "
                + str(ml)
                + "um long"
            )
            if n == 1:
        ...
        if nmax == 0:
            raise ValueError(
                "max_length appears to be greater than length.  Either length is too large or program is broken."
            )
    ...
    def spiral_length(self, h, corner_dl, n):
        ...
        length = (
            w
            + 2 * (w - s)
            + sum([2 * (w - s - 2 * (i + 1) * s) for i in range(n - 1)])
            + (w - 2 * n * s)
        )
        length += (
            h
            + (h - s)
            + sum([2 * (h - 2 * (i + 1) * s) for i in range(n)])
            + (h - s - 2 * n * s)
        )
    ...
    def build_cell(self):
        ...
        if self.direction == "WEST":
            ...
            wgr = gdspy.CellReference(
                wg, rotation=(self.direction * 180 / np.pi) - 90.0
            )
