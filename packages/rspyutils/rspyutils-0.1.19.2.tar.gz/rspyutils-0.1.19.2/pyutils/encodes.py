...
class FeaturesEncode(object):
    ...
    def get_scaler_transform(self, df: pd.DataFrame, cols: list, log_col=None, method="normalize"):
        ...
        if method == "normalize":
            for v in cols:
                if v in set(log_col):
                    df[v] = df[v].parallel_apply(lambda x: log(max(x, 1)))
    ...
    def get_discrete_transform(self, df: pd.DataFrame, cols: dict):
        for v in cols:
            df[v] = df[v].parallel_apply(lambda x: str(int(x * cols[v])))
        df_type = df.astype({x: "category" for x in cols})
        return df_type
    ...
    def get_singular_truncation_transform(self, df: pd.DataFrame, cols: list, thr=None, method="min"):
        ...
        if method == "min":
            for v in cols:
                df[v] = df[v].parallel_apply(lambda x: min(x, thr))
        else:
            for v in cols:
                df[v] = df[v].parallel_apply(lambda x: max(x, thr))
        return df
    ...
    def get_category_type_transform(self, df: pd.DataFrame, cols: list, int_cols=None):
        ...
        for v in cols:
            if df[v].dtype == 'float64' or df[v].dtype == 'float32' or (v in int_cols):
                df[v] = df[v].parallel_apply(lambda x: str(int(x)))
        df.astype({x: "category" for x in cols})
        return df
    ...
    def get_time2category_transform(self, df: pd.DataFrame, cols: list, method="timestamp", dim="day"):
        ...
        if method == "timestamp":
            if dim == "sec":
                for k in cols:
                    df[k] = df[k].parallel_apply(lambda x: dates.str_to_timestamp(x))
            elif dim == "day":
                for k in cols:
                    df[k] = df[k].parallel_apply(lambda x: dates.get_date_from_str(x).day)
            elif dim == "month":
                for k in cols:
                    df[k] = df[k].parallel_apply(lambda x: dates.get_date_from_str(x).month)
            else:
            ...
            if dim == "sec":
                for k in cols:
                    df[k] = df[k].parallel_apply(
                        lambda x: dates.get_date_diff(start_date=str(x), end_date=now_time).seconds)
            elif dim == "day":
                for k in cols:
                    df[k] = df[k].parallel_apply(
                        lambda x:
                        dates.get_date_diff(start_date=str(x), end_date=now_time).days)
            elif dim == "month":
                for k in cols:
                    df[k] = df[k].parallel_apply(
    ...
    def get_cross_transform(self, df: pd.DataFrame, cols: list, vector_col: list):
        def func(df: pd.DataFrame, col1: str, col2: str):
            try:
        ...
        cross_col = []
        for c in cols:
            df['{}*{}'.format(c[0], c[1])] = df.parallel_apply(lambda x: func(x, c[0], c[1]), axis=1)
