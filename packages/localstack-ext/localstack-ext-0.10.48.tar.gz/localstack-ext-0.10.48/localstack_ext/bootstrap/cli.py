...
def import_auth():
 try:
  ...
  raise JnPty('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=JnPtl)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=JnPtE)
def get_params(args,names=[],mandatory=JnPtl):
 result=()
 for name in names:
  value=JnPth
  if name in args:
  ...
  if not value and mandatory:
   raise JnPty('Please provide %s=... parameter'%name)
  result+=(value)
 if JnPtc(result)==1:
  return result[0]
 return result
