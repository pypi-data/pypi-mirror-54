...
def import_auth():
 try:
  ...
  raise gYcaP('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=gYcao)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=gYcat)
def get_params(args,names=[],mandatory=gYcao):
 result=()
 for name in names:
  value=gYcaq
  if name in args:
  ...
  if not value and mandatory:
   raise gYcaP('Please provide %s=... parameter'%name)
  result+=(value)
 if gYcaR(result)==1:
  return result[0]
 return result
