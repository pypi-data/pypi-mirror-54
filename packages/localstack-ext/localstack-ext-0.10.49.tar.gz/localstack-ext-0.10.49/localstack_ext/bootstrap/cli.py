...
def import_auth():
 try:
  ...
  raise ByfJl('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ByfJX)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ByfJi)
def get_params(args,names=[],mandatory=ByfJX):
 result=()
 for name in names:
  value=ByfJt
  if name in args:
  ...
  if not value and mandatory:
   raise ByfJl('Please provide %s=... parameter'%name)
  result+=(value)
 if ByfJh(result)==1:
  return result[0]
 return result
