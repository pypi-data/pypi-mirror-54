...
def import_auth():
 try:
  ...
  raise rFskn('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rFskB)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rFskY)
def get_params(args,names=[],mandatory=rFskB):
 result=()
 for name in names:
  value=rFskU
  if name in args:
  ...
  if not value and mandatory:
   raise rFskn('Please provide %s=... parameter'%name)
  result+=(value)
 if rFsko(result)==1:
  return result[0]
 return result
