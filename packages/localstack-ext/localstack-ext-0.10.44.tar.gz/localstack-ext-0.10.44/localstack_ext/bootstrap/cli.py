...
def import_auth():
 try:
  ...
  raise LqPol('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=LqPob)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=LqPoI)
def get_params(args,names=[],mandatory=LqPob):
 result=()
 for name in names:
  value=LqPox
  if name in args:
  ...
  if not value and mandatory:
   raise LqPol('Please provide %s=... parameter'%name)
  result+=(value)
 if LqPov(result)==1:
  return result[0]
 return result
