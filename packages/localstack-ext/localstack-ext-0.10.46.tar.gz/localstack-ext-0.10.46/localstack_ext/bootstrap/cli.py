...
def import_auth():
 try:
  ...
  raise XpJVz('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=XpJVj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=XpJVD)
def get_params(args,names=[],mandatory=XpJVj):
 result=()
 for name in names:
  value=XpJVo
  if name in args:
  ...
  if not value and mandatory:
   raise XpJVz('Please provide %s=... parameter'%name)
  result+=(value)
 if XpJVh(result)==1:
  return result[0]
 return result
