...
def import_auth():
 try:
  ...
  raise HmvnA('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=HmvnR)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=HmvnG)
def get_params(args,names=[],mandatory=HmvnR):
 result=()
 for name in names:
  value=Hmvnu
  if name in args:
  ...
  if not value and mandatory:
   raise HmvnA('Please provide %s=... parameter'%name)
  result+=(value)
 if Hmvnf(result)==1:
  return result[0]
 return result
