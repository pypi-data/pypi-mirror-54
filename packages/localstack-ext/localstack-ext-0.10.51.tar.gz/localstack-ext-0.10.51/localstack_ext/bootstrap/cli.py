...
def import_auth():
 try:
  ...
  raise JafFG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=JafFl)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=JafFP)
def get_params(args,names=[],mandatory=JafFl):
 result=()
 for name in names:
  value=JafFV
  if name in args:
  ...
  if not value and mandatory:
   raise JafFG('Please provide %s=... parameter'%name)
  result+=(value)
 if JafFm(result)==1:
  return result[0]
 return result
