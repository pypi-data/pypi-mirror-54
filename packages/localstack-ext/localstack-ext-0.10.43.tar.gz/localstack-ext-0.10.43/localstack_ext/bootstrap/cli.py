...
def import_auth():
 try:
  ...
  raise ambyx('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ambyY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ambyh)
def get_params(args,names=[],mandatory=ambyY):
 result=()
 for name in names:
  value=ambyH
  if name in args:
  ...
  if not value and mandatory:
   raise ambyx('Please provide %s=... parameter'%name)
  result+=(value)
 if ambyr(result)==1:
  return result[0]
 return result
