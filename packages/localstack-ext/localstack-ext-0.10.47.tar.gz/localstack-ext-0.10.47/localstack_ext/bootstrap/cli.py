...
def import_auth():
 try:
  ...
  raise OPzae('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=OPzaI)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=OPzaS)
def get_params(args,names=[],mandatory=OPzaI):
 result=()
 for name in names:
  value=OPzaV
  if name in args:
  ...
  if not value and mandatory:
   raise OPzae('Please provide %s=... parameter'%name)
  result+=(value)
 if OPzar(result)==1:
  return result[0]
 return result
