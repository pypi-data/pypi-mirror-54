...
class Libro(object):
        ...
        for dteDoc in vals:
            for docData in dteDoc.get("documentos", []):
                if not docData.get('sii_xml_request'):
                    ...
                    if self.TipoOperacion == 'COMPRA' and docData['Encabezado'].get('Emisor') or docu._receptor.RUTRecep == DTE._dte_emisor.RUTEmisor:
                        new_recep = {
    ...
    def getResumen(self, rec):
        ...
        if self.TipoOperacion == 'VENTA':
            det['RUTDoc'] = util.format_vat(rec._receptor.RUTRecep)
            det['RznSoc'] = rec._receptor.RznSocRecep[:50]
        else:
            det['RUTDoc'] = util.format_vat(rec.Emisor.RUTEmisor)
    ...
    def validar(self):
        ...
        for rec in self.Documento:
            ...
            if self.TipoOperacion == 'BOLETA':
                _logger.warning("resumen boleta %s" %rec)
