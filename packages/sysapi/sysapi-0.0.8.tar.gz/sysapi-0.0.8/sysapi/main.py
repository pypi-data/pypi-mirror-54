...
def get_installed():
    return do_get_installed(config_file)

@app.get("/v1/danger/reset")
def get_reset_warning_DANGEROUS():
    return do_reset()

@app.get("/v1/users")
def get_users():
    return {"users": do_get_users(db)}
...
async def create_user(*, user_in: UserIn):
    new_user = do_create_user(db,user_in)
    return user_in
...
def get_snapshots():
    (res, detail) = do_get_snapshots(db)
    if not res: raise HTTPException(status_code=409, detail=detail)
...
def get_snapshot(snapshot: str = Path(...,min_length=1, regex="^@[a-zA-Z0-9-_']+$")) :
    if snapshot in do_get_snapshots(db):
        return do_get_snapshot(db,snapshot)
    else:
...
def get_file_at_path(
    ...
    user = do_get_user(db,username)
    if user is None: raise HTTPException(status_code=404, detail="User unknown")
    snap = do_get_snapshot(db,snapshot)
