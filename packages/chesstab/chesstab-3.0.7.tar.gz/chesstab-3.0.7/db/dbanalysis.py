...
"""bsddb interface to chess database for chess engine analysis index.
"""
...
class AnalysisDS(DBDataSource):
    ...
    def find_position_analysis(self, fen):
        ...
        # Berkeley DB uses bytes, not str, so fen coerced to bytes.
    ...
    def find_engine_analysis(self, engine):
        ...
        # Berkeley DB uses bytes, not str, so engine coerced to bytes.
        if not isinstance(engine, bytes):
            engine = engine.encode('utf8')
    ...
    def find_engine_position_analysis(self, engine=None, fen=None):
        ...
        # Berkeley DB uses bytes, not str, so engine and fen coerced to bytes.
        if not isinstance(engine, bytes):
            engine = engine.encode('utf8')
        if not isinstance(fen, bytes):
            fen = fen.encode('utf8')
