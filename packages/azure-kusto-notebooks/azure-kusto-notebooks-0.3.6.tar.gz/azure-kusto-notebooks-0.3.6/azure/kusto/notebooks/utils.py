...
def get_client(cluster):
    ...
    if c is None:
        ...
        token = ac.acquire_token_with_device_code(
            cluster, user_code_info, client_id)
...
def print_result_stats(result):
    ...
    stat_column_index = next(
        (x.ordinal for x in query_stats.columns if x.column_name == 'Payload'), None)
...
def promp_for_aad_device_login(url, text, code):
    ...
    if not isnotebook():
        ...
        print('To authenticate, please go to', url,
              'and paste', code, 'in the box.')
        return
...
<tr style="font-size:25px" id='{buttonId}'>
    <td>{code}</td>
    <td>{url}</td>
