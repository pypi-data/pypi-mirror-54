...
from featurize_jupyterlab import constants as C
...
def main_process(config_file):
    ...
    try:
        while True:
            res = env.rpc.heyYo(env.config['identity'], current_status)
...
def training_process_entry(config_file):
    from .training_process import main as training_process_main
    training_process_main(config_file)


def spawn_training_process(config_file):
    child_process = Process(target=training_process_entry, args=(config_file,))
    child_process.start()
