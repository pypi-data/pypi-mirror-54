...
class BTC(Coin):
    ...
    def history(self: "BTC") -> dict:
        ...
        """
        return json.loads(self.server.onchain_history())  # type: ignore
    ...
    def process_updates(self: "BTC", updates: Iterable[dict]) -> None:
    ...
    def rate(
        ...
        """
        rate_str = self.server.exchange_rate(currency)
    ...
    def configure_webhook(self: "BTC", autoconfigure: bool = True) -> None:
        ...
        self.server.subscribe(list(self.event_handlers.keys()))
        if autoconfigure:
            self.server.configure_notifications("http://localhost:6000")
