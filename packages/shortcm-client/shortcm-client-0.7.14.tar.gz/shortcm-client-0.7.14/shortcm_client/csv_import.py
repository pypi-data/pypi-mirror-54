...
def import_csv(filename, secret_key, domain, path_column, cloaking, delimiter, skip_lines, out_file, **kwargs):
    s = requests.Session()
    retry = Retry(
        total=5,
            ...
            for link_status in j:
                if link_status.get('error'):
                    ...
                    with open(out_file, "a"):
                        out_file.writelines(link_status)
            pb.update(idx * 1000)
            time.sleep(0.5)
...
def add_parser(subparsers):
    ...
    import_parser.add_argument(
        '--out-file', dest='out_file', help='File to write links into', type=str, default=None)
