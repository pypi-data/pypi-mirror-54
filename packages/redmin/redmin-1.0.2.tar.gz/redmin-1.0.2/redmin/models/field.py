...
def get_model_fields(model, user=None):
    fields = get_all_default_fields().get(model, {})
    if user:
...
class Field(BaseField, CommonField):
    ...
    def get_default_fields(cls, model):
        return get_all_default_fields().get(model, {})

    @classmethod
    def get_default_field(cls, model, attribute):
        return cls.get_default_fields(model).get(attribute)
    ...
    def get_user_fields(cls, user, model):
        result = get_model_fields(model, user=user)
        # for field in result.values():
        #    print(field, vars(field))
        return result

    @classmethod
    def get_field(cls, user, model, attribute):
        return cls.get_user_fields(user, model).get(attribute)
