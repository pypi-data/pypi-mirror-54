...
class Map4DRGB:
    def __init__(self, computation, **kwargs):
        ...
        def create_panel_widget(ui, document_controller):
            def select_button_clicked(channel):
                ...
                if not graphics:
                    return
                graphics_variable = self.computation._computation._get_variable('map_regions_' + channel)
    ...
    def execute(self, src, map_regions_r, map_regions_g, map_regions_b, gamma_r, gamma_g, gamma_b, enabled_r,
        ...
        try:
            ...
            rgb_data = np.zeros(src.xdata.data_shape[:2] + (3,), dtype=np.uint8)
            ...
            metadata['nion.map_4d_rgb.parameters'] = {'src': src._data_item.write_to_dict(),
...
class Map4DRGBMenuItem:
    ...
    def menu_item_execute(self, window: API.DocumentWindow) -> None:
        ...
        if data_item:
            ...
            computation = self.__api.library.create_computation('nion.map_4d_rgb',
...
Symbolic.register_computation_type('nion.map_4d_rgb', Map4DRGB)
