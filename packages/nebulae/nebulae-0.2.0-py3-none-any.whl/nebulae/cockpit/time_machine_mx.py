        ...
        if self.param['ckpt_path'] is None:
            ...
            for arch in architecture:
                ...
                if saved_no > max_saved:
                    moment = arch
                    max_saved = saved_no
            # spot = SymbolBlock.imports(self.param['ckpt_path']+'.json', self.sc.in_node,
            #                            self.param['ckpt_path']+'.params')
            # if self.param['ckpt_scope'] is None:
            #     pass
            # else:
            #     spot.collect_params('^[^(%s)]'%self.param['ckpt_scope']).initialize(ctx=self.sc.engine.context)
            self.sc.load_parameters(moment, self.sc.engine.context)
            if self.param['ckpt_scope'] is None:
                pass
            else:
                values = self.sc.collect_params()
                if not values.prefix is self.param['ckpt_scope']:
                    values.initialize(ctx=self.sc.engine.context)
