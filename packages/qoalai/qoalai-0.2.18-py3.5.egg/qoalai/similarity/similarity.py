...
class Similarity(): 
    ...
    def calculate_loss(self, feature1, feature2, feature3, feature4): 
        pos_loss = mse_loss_mean(feature1, feature2)
        neg_loss = mse_loss_mean(feature1, feature3)
        other_loss = mse_loss_mean(feature1, feature4)
        self.loss = pos_loss - 1.3 * neg_loss - other_loss
        ...
        while True:
            ...
            for i in self.classes:
                for j in range(perclass_sample):
                    ...
                    # ------------------------------------ #
                    idx_pos= random.randint(0, len(file_list_by_class[i]))
                    ...
                    idx_neg = random.randint(0, len(file_list_by_class[tmp_class]))
                    ...
                    # ------------------------------------ #
                    idx_other = random.randint(0, len(file_list_by_class['other']))
    ...
    def optimize(self, 
        ...
        for i in range(iteration):
            ...
            for j in range(subdivition):
                x_train, x_pos, x_neg, x_other = next(train_generator)
                print (x_train.shape)
                print (x_pos.shape)
                print(x_neg.shape)
                print (x_other.shape)
                ...
                feed_dict[self.input_placeholder_pos] = x_pos
                feed_dict[self.input_placeholder_neg] = x_neg
                feed_dict[self.input_placeholder_other] = x_other
