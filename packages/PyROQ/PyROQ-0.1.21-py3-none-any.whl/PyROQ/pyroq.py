...
def surros_of_test_samples(nsamples, nparams, params_low, params_high, tolerance, ndim, known_bases_copy, distance, deltaF, f_min, f_max, waveFlags, approximant):
    nts=nsamples
    ndim, inverse_V, emp_nodes = empnodes(ndim, known_bases_copy)
    test_points = generate_params_points(nts, nparams, params_low, params_high)
    surros = numpy.zeros(nts)
    for i in numpy.arange(0,nts):
        test_mc =  test_points[i,0]
        ...
        if nparams == 12: 
            test_lambda1 = test_points[i,10]
            test_lambda2 = test_points[i,11]
        surros[i] = surroerror(ndim, inverse_V, emp_nodes, known_bases_copy, test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref, distance, deltaF, f_min, f_max, waveFlags, approximant)
    if (surros[i] > tolerance):
