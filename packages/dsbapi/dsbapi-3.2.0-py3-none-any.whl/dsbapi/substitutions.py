...
def _parse_substitution_table(
    ...
    if substitution_table is not None:
        ...
        for tr in substitution_table.findAll("tr"):
            # 0         1   2       3       4       5       6           7
            # Stunde	Art	Klasse	Raum	Lehrer	Fach	Fach alt	Vertretungs-Text
            # classes, period, subject_new, subject_old, teacher_new, teacher_old, room, type, text

            classes = tr.attrs["class"]
            if "head" in classes:
                continue
            elif "TeacherHead" in classes:
                currentTeacher = tr.text.strip("Lehrer: ")
            else:
                subData = [x.text for x in tr.findAll("td")]
                assert len(subData) == 8, "invalid subData"

                if is_teacher_subs:
                    sub = Substitution(
                        tuple(subData[2].split(", ")),
                        subData[0],
                        subData[5],
                        subData[6],
                        subData[4],
                    ...
                    raise RuntimeError('not supported for student subs table urls')
