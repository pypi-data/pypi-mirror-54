...
def create_sample_sheet(dir_path, matrix_file=False, output_file='samplesheet.csv',
    sample_type='', sample_sub_type='', controls=[]):
    ...
    if matrix_file:
        ...
        if controls:
            control_index = {idx:'' for idx in range(len(_dict))}
            for control_sample_name in controls:
                if control_sample_name in _dict['Sample_Name']:
                    idx = min([idx for idx,i in enumerate(_dict['Sample_Name']) if i == control_sample_name])
                    control_index[idx] = control_sample_name
            _dict['Control'] = list(control_index.values()) # idx should retain same order.
    else:
