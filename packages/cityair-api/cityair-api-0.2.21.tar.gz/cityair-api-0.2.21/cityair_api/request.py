...
class CityAirRequest:
    ...
    def get_devices(self, format: str = 'list', include_offline: bool = True, include_children: bool = False, **kwargs):
       ...
       # print(df['children'])
        #df['children'].apply(lambda children_info: print(pd.isna(children_info)) if not isinstance(children_info, Iterable) else 0)

        df['children'].apply(lambda children_info: [child_info.pop('id') for child_info in children_info] if isinstance(children_info, Iterable) else [])
        df['children'] = df['children'].apply(lambda list_with_duplicates: list(set(list_with_duplicates)))
        if format == 'dicts':
            res = []
