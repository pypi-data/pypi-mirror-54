...
class Transfer:
    ...
    def send_data(self):
        ...
        if filenumber > self.__CONFIG(['limits']['datafile max']):
            raise Exception(
                "Transaction has %d files. Limit is set to %d. Please split your transaction in smaller chunks"%(filenumber, self.__CONFIG(['limits']['datafile max']) )
            )
        ...
        if sizeGb > self.__CONFIG['limits']['datasize max']:
            raise Exception(
                "Transaction is %.2fGb big. Limit is set to %d. Please split your transaction in smaller chunks"%(sizeGb, self.__CONFIG(['limits']['datasize max']) )
            )
