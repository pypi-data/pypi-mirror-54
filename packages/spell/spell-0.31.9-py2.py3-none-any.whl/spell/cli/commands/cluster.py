...
@click.command(name="delete",
               short_help="Facilitates the deletion of your Spell cluster by removing the associated "
...
def delete(ctx, cluster_name, profile):
    cluster = deduce_cluster(ctx, cluster_name)

    cluster_type = cluster['cloud_provider']
    if cluster_type == 'AWS':
        delete_aws_cluster(ctx, cluster, profile)
    elif cluster_type == 'GCP':
        if profile:
