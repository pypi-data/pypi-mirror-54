...
def delete_aws_cluster(ctx, cluster, profile):
    try:
        import boto3
        from botocore.exceptions import BotoCoreError
    except ImportError:
    ...
    for i in range(num_retries):
        try:
            spell_client.is_cluster_drained(cluster["name"])
            click.echo("Cluster is drained!")
            break
        except BadRequest as e:
            click.echo("Cluster is still draining all machine types. "
                       "This can take a long time! Retrying in 30s...")
            time.sleep(30)
        if i == num_retries - 1:
            raise ExitException("Timed out waiting for Spell to drain the cluster. Please try again "
