    ...
    if current_version < 5:
        ...
        try:
            ...
            with click.progressbar(length=100, show_eta=False) as bar:
                while response['status'] != 'DONE':
                    bar.update(response['progress'])
                    response = compute_service.globalOperations().get(
                        project=project_id, operation=response['name']).execute()
                bar.update(100)
...
def delete_gcp_cluster(ctx, cluster):
    raise ExitException("This feature does not exist for GCP yet. Please contact Spell for support")
