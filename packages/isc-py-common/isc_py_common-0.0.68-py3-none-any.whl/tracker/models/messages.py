                    ...
                    if request.user_id != to_whom_id:
                        LoginRequest.send_bot_message(user=to_whom_id, bot=MessagesManager.get_tracker_bot(),
                                                      message=f'<h3>Вам назначено новое дело:</h3> #{res.guid}'
                                                              ...
                                                              f'<p>{self.get_border(res.message)}'
            ...
            if request.user_id != message.to_whom.id:
                LoginRequest.send_bot_message(user=message.to_whom.id, bot=MessagesManager.get_tracker_bot(),
                                              message=f'<h3>Изменение дела:</h3> #{message.guid}'
                                                      ...
                                                      f'<p/>{self.get_border(message.message)}'
            ...
            with transaction.atomic():
                ...
                LoginRequest.send_bot_message(user=to_whom_id, bot=MessagesManager.get_tracker_bot(),
                                              message=f'<h3>Вам назначено новое дело:</h3>#{getAttr(data_clone, "guid")}'
                                                      ...
                                                      f'<p/>{self.get_border(data_clone.get("message"))}'
                    ...
                    if user_id:
                        LoginRequest.send_bot_message(user=user_id, bot=MessagesManager.get_tracker_bot(),
                                                      message=f'<h3>Изменение дела:</h3>#{data_clone.get("guid")}'
                                                              ...
                                                              f'<p/>{self.get_border(data.get("message"))}'
        ...
        with transaction.atomic():
            for id, mode in tuple_ids:
                if mode == 'hide':
                    ...
                    if message.user.id != request.user_id:
                    ...
                    LoginRequest.send_bot_message(user=message.to_whom.id, bot=MessagesManager.get_tracker_bot(),
                                                  message=f'<h3>Удаление дела:</h3> #{message.guid}'
                                                          ...
                                                          f'<p/>Удалил: {User.objects.get(id=request.user_id).get_short_name}'
                                                  )
                    res += qty
        return res
