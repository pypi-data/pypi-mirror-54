...
def replace_ims(text,tofile=True):
    ...
    for im in ims:
        ...
        if tofile:
            try:
                ...
                warn(f"Couldn't copy file '{im[2]}' to output, does it exist?")
        ...
        for linkedPage in self.partlist.links():
            if os.path.exists(linkedPage):
                if linkedPage.endswith('.md') or linkedPage.endswith('.stl'):
                    ...
                    lpdir = os.path.join(config.outDir,lpdir)
                    if not os.path.exists(lpdir):
                        os.makedirs(lpdir)
                    outpath = os.path.join(lpdir,lpname)
                    shutil.copyfile(linkedPage,outpath)
                    if linkedPage.endswith('.stl'):
                        with codecs.open(outpath[:-3]+'md', "w", "utf-8") as outfile:
                            output = u''
                            output += f'# {lpname}\n\n'
                            output += f'<stl-part-viewer src="/{linkedPage}" width="500" height="500" floorcolor="0xf1f1f1"></stl-part-viewer>'
                            outfile.write(output)
