...
def decimalize(v, q = None):
    ...
    if isinstance(v, (tuple, list)):
        return type(v)(decimalize(x, q) for x in v if hasattr(x, "real"))
    else:
...
def extract_words(chars,
    ...
    for char in chars:
        char['y_center'] = (char["doctop"] + char['bottom'] / 2)
    sorted_chars = sorted(non_sp_chars, key=lambda char: char['y_center'])
    ...
    for idx_a in range(num_chars):
        for idx_b in range(idx_a + 1, num_chars):
            if sorted_chars[idx_b]["y_center"] > sorted_chars[idx_a]["y_center"] + y_tolerance:
                break
