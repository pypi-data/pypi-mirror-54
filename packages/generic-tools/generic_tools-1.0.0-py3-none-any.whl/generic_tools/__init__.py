...
class MysqlDataBase:
    ...
    def pd_read_sql(self, sql, parmas=""):
        try:
            data = pd.read_sql(sql, con=self.db, params=parmas).to_dict('records')
        except Exception as e:
            raise Exception("pd execute {} failed. the reason {}".format(sql, e))
        return data
...
if __name__ == "__main__":
    dd = datetime_to_string(datetime_cal(datetime.datetime.now(), -3600))
    dd2 = datetime_to_string(datetime_cal(datetime.datetime.now(), 3600))
    d3 = timestamp_to_datetime(time.time(), fmt="%H:%M")
    print(dd)
    print(dd2)
    print(d3)
