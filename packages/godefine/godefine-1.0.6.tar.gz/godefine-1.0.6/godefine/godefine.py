...
def grab_vars(input_file: typing.AnyStr, var_line: typing.Optional[typing.List],
              ch_dir_path: typing.Optional[typing.AnyStr]) -> typing.Dict:
    if var_line is None:
        var_line = {}
    out = {}
    save_cwd = os.getcwd()
    ...
    input_file_parent_path = input_file_path.absolute().parent.absolute()
    try:
        os.chdir(input_file_parent_path)
            ...
            for line in var_file.readlines():
                ...
                if len(include_ls) != 0:
                    for include_item in include_ls:
                        new_dict = grab_vars(include_item, None, ch_dir_path)
                        if len(new_dict) != 0:
...
def main():
    cmd_args = parse_args()
    try:
        user_specified_vars = grab_vars(cmd_args.input_vars, cmd_args.vars, True)
    except Exception as err:
