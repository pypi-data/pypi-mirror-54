...
bool ParentParticleSeg::process(IOManager& mgr) {
  ...
  std::set<int> _primary_trackids;
  std::set<int> _found_primary_nodes;

  for (auto& particle : ev_particle.as_vector()) {
    if (debug){
      std::cout << "Ancestor track ID, ancestor PDG, id, parent ID, PDG, track id, proc: ("
                ...
                << particle.pdg_code()          << ", "
                << particle.track_id()          << ", "
                << particle.creation_process()  << ", "
                << ")" << std::endl;
    }

    // Particles are top level if their process == primary
    ...
    if (particle.creation_process() == "primary"){
      ...
      _found_primary_nodes.insert(particle.track_id());
    }
    else{
      // No, not a primary
      node->primary = false;
    ...
    // _primary_trackids.insert(particle.ancestor_track_id());

  }

  // // Fill out the rest of the ancestor nodes:
  for (auto& trackID : _primary_trackids) {
    if (_found_primary_nodes.find(trackID) == _found_primary_nodes.end()) {
