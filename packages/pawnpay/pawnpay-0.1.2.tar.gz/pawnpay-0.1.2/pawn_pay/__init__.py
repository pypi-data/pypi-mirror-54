...
class TemporalSubspaceHolocapacitor(object):
    ...
    def __init__(self, store_id: str, api_key: str, testing=False):
        ...
        try:
            auth = requests.get(url=self._baseurl, headers=self._headers)
        except (
        ...
        if auth.status_code == 418:
            self.authcheck = True
        elif auth.status_code == 429:
            sleep(60)
            auth = requests.get(url=self._baseurl, headers=self._headers)
            if auth.status_code == 418:
                self.authcheck = True
    ...
    def __post(self, url: str, data: dict):
        ...
        Returns:
            requests.models.Response: The API response Requests object
        """
        return requests.post(url=url, headers=self._headers, data=json.dumps(data))
    ...
    def __patch(self, url: str, data: dict):
        ...
        Returns:
            requests.models.Response: The API response Requests object
        """
        return requests.patch(url=url, headers=self._headers, data=json.dumps(data))
    ...
    def sync_tickets(self, local_tickets: type(TicketList)):
        ...
        for ticket in list(
                filter(
                    lambda x: all(
                        [(x.number not in [y.number for y in to_delete]),
                         (x.number not in [z.number for z in to_create])]
                    ),
                    local_tickets,
                )
        ):
