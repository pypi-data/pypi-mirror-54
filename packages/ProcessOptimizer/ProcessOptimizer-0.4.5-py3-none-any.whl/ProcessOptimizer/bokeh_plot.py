...
def set_globals(parsed_result):
    ...
    button_generate.on_click(lambda: handle_button_generate(result))
...
def handle_button_generate(result):
    ...
    if active_list:  # Only plot if there is at least one selection
        ...
        plots, colorbar_interval = get_plots_layout(
            result, active_list, n_points, x_eval, confidence)
        layout.children[0].children[1] = Div(
            text="""fdasfdasfdasfdas""", width=300, height=200)
        print("zzzzzz")
...
def get_plot_list(result, active_list, n_points, x_eval, confidence):
...
def get_plots_layout(result, active_list, n_points, x_eval, confidence):
    global x_eval_selectors
