...
class Deployment:
    ...
    def _spec(self):
        ...
        output = self.kubectl.run(command, stderr=subprocess.DEVNULL, stdout=subprocess.PIPE, check=True)
    ...
    def annotate_restart(self, restart_id):
        ...
        command = ('patch', 'deployment', self.name, '--type', 'json', '--patch', json.dumps(patch))
        try:
            self.kubectl.run(command, stdout=subprocess.DEVNULL, stdin=subprocess.DEVNULL, check=True)
