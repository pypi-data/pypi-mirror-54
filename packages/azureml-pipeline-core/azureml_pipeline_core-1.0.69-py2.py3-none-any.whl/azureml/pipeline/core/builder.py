...
from azureml.pipeline.core.pipeline_output_dataset import PipelineOutputDataset, _DatasetRegistration
...
class PipelineStep(object):
    ...
    def __init__(self, name, inputs, outputs, arguments=None, fix_port_name_collisions=False, resource_inputs=None):
        ...
        PipelineStep._update_inputs_with_arguments(arguments, inputs)
    ...
    def _validate_io_types(inputs, outputs, resource_inputs):
        def validate_input_type(input):
            if not isinstance(input, PipelineData) and not isinstance(input, InputPortBinding) \
                    ...
                    and not isinstance(input, PipelineDataset) and not isinstance(input, PipelineOutputDataset):
        ...
        def validate_output_type(output):
            if not isinstance(output, PipelineData) and not isinstance(output, OutputPortBinding) \
                    and not isinstance(output, PipelineOutputDataset):
    ...
    def _get_input_port_name(input):
        ...
        if isinstance(input, PipelineOutputDataset):
    ...
    @staticmethod
    def _update_inputs_with_arguments(arguments, inputs):
        ...
        for i in range(len(arguments)):
            ...
            if not isinstance(argument, PipelineDataset) and not isinstance(argument, PipelineOutputDataset) \
            ...
            name = argument.input_name if isinstance(argument, PipelineOutputDataset) else argument.name
    ...
    def create_input_output_bindings(self, inputs, outputs, default_datastore, resource_inputs=None):
        ...
        for step_output in outputs:
            ...
            elif isinstance(step_output, PipelineOutputDataset):
                dataset_registration = _DatasetRegistration(step_output._registration_name,
...
class PipelineData(object):
    ...
    def as_dataset(self):
        ...
        """
        from azureml.pipeline.core.pipeline_output_dataset import PipelineOutputDataset
        return PipelineOutputDataset(self)
