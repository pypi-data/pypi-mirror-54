...
# This is a serious hack to avoid a deadlock issue that
# arises when concurrently executing many contexts.
# There's some sort of issue with how the underlying v8 executor
# refers to contexts. Verified execution is thread-safe, but
# should fully sort this out before release.
...
class Intervention:
    ...
    and only variables initialized in the constructor are modified
    ...
    >>> Intervention(year=1975, land_yield_factor=2)

    """

    def __init__(self, year=1975, **kwargs):
        """Construct an intervention object.

        Parameters
        ----------
            year: int
                Year to intervene in the simulator.
            kwargs: dict
...
def decode_states(js_context):
    ...
    Return the recorded state values and the time each value was sampled
    during the run.
    """
    def unpack(values):
...
def set_config(js_context, config, intervention):
    ...
    if intervention:
        ...
        js_context.eval(f"policyYear = {intervention.year}")
    else:
...
def simulate(initial_state, config, intervention=None, seed=None):
    ...
    to end_time with a step size of delta_t.

    Ignore the seed argument since the simulator is deterministic.
    """
