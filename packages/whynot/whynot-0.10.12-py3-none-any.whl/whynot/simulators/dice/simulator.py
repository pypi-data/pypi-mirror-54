...
class Config:
    ...
    deland: float = 0.2
    #: Industion emissions 2010 (GtC02 per year)
    e0: float = 33.61
...
class State:  # pylint:disable-msg=too-few-public-methods
    ...
    TOCEAN: float = 0.0068
    #: Carbon concentration in atmostphere GtC
    MAT: float = 830.4
...
def simulate(initial_state, config, intervention=None, seed=None, stochastic=True):
    """Simulate the DICE model.

    :param initial_state: State object
    :param config: Config object
    :param seed: seed number for randomization
    :param stochastic: whether or not to use randomness in the config.
                       Useful for testing.
    :return: Run object storing states and time steps
    """
    if intervention:
