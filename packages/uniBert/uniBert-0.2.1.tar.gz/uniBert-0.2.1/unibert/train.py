...
from bert_gen_onebyone import *
from bert_classifier import *
from bert_tagging import *
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(iterator):
            if 'classifier' in arg.model:
                ...
                inputs = batch['input']
                labels = batch['label']
                loss, _ = model(inputs, labels, eval=True)
            else:
                output = model(batch['input'], batch['target'], batch['type'], batch['mask'])
                loss = output[0]
            t_loss += loss.mean().item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="once", choices=['once', 'onebyone', 'classify', 'tag'])
    ...
    if "once" in arg.model:
        ...
        train_dataset = loadClassifierDataset(arg.train)
        eval_dataset = loadClassifierDataset(arg.valid)
        model = BertMtClassifier(train_dataset.task, arg.bert)
    elif 'tag' in arg.model:
        train_dataset = loadTaggerDataset(arg.train)
        eval_dataset = loadTaggerDataset(arg.valid)
        model = BertTagger(train_dataset.label, arg.bert)
