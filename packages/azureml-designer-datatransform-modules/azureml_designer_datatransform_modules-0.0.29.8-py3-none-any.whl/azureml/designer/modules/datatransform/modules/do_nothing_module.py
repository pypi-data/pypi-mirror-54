...
class DoNothingModule(ModuleBase):
    ...
    def run(self):
        ...
        # logger.info(f"Upload from {input_path} to datastore: {datastore_name} on path {upload_path}")
        # current_run = Run.get_context()
        # workspace = current_run.experiment.workspace
        # # workspace = Workspace(subscription_id='f8f8b81c-3ebd-427c-b3b7-ebb226c4eea2',
        # #         resource_group='jietong-test',
        # #         workspace_name='jietong-test-4')
        # datastore = Datastore.get(workspace, datastore_name)
        # datastore.upload(src_dir=input_path,
        #          target_path=upload_path,
        #          overwrite=True,
        #          show_progress=True)
        # logger.info(f"Done Upload")
    ...
    def copy_anything(src, dst):
        ...
        #         logger.info(f"start copy from {source_file_path} to {target_file_path}")
        #         with open(source_file_path) as s:
        #             with open(target_file_path, 'w+') as t:
        #                 stream = s.read()
        #                 t.write(stream)
        #         logger.info(f"finished copy from {source_file_path} to {target_file_path}")
        try:
