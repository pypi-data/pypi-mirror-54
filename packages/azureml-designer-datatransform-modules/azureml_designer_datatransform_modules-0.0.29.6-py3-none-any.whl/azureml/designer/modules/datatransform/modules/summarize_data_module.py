...
#                         raise ValueError("arg %r does not match %s" % (a,t))
#             return f(*args, **kwds)
...
class SummarizeDataModule(ModuleBase):
    def __init__(self):
        meta_data = ModuleMetaData(
            ...
            description="Generates a basic descriptive statistics report for the columns in a dataset. Formerly known as Descriptive Statistics.")
...
class Series_Summarize():
    ...
    def _special_handle(self, fn, return_time_span = False):
        ...
        if self._is_timedelta:
            return None
            # return pd.Timedelta(fn(self._preprocess_data)) # pyarrow does not support Timedelta
        if return_time_span:
            return None
            # return pd.Timedelta(fn(self._preprocess_data)) # pyarrow does not support Timedelta
    ...
    def get_summarize(self) -> pd.DataFrame:
        functions = {
            ...
            "Range":  self._data.max() - self._data.min() if self._is_numeric() and not self._is_datetime and not self._is_timedelta else 0 if self._data.max() == self._data.min() else 1 if self._is_boolean else None,
            ...
            # "P0.5": self._special_handle(lambda x:x.quantile(0.005)) if self._is_full_process() else None,
            # "P1": self._special_handle(lambda x:x.quantile(0.01)) if self._is_full_process() else None,
            # "P5": self._special_handle(lambda x:x.quantile(0.05)) if self._is_full_process() else None,
            # "P95": self._special_handle(lambda x:x.quantile(0.95)) if self._is_full_process() else None,
            # "P99": self._special_handle(lambda x:x.quantile(0.99)) if self._is_full_process() else None,
            # "P99.5": self._special_handle(lambda x:x.quantile(0.995)) if self._is_full_process() else None,
        }
