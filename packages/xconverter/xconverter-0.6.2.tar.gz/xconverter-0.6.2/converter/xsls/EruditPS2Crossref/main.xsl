...
<xsl:stylesheet
    ...
    <xsl:template match="styled-content" mode="references">
        <citation>
            <xsl:attribute name="key">
                <xsl:value-of select="../../@id"/>
            </xsl:attribute>
            <xsl:apply-templates select="../pub-id[@pub-id-type='doi']" mode="doi"/>
    ...
    <xsl:template match="suffix">
        <suffix><xsl:value-of select="substring(.,1,10)"/></suffix>
    </xsl:template>
    ...
    <xsl:template match="abstract|trans-abstract">
        <jats:abstract>
            ...
            <xsl:apply-templates select="*" mode="abstract-main-content"/>
        </jats:abstract>
    </xsl:template>

    <xsl:template match="*" mode="abstract-main-content">
        <xsl:variable name="elem_name"><xsl:if test="not(contains(':', name()))">jats:</xsl:if><xsl:value-of select="name()"/></xsl:variable>
        <jats:p>
            <xsl:apply-templates select="*|text()" mode="abstract-sub-content"/>
        </jats:p>
    </xsl:template>

    <xsl:template match="*" mode="abstract-sub-content">
        <xsl:variable name="elem_name"><xsl:if test="not(contains(':', name()))">jats:</xsl:if><xsl:value-of select="name()"/></xsl:variable>
