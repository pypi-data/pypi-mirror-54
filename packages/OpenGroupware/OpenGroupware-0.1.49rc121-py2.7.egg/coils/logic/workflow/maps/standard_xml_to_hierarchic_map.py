...
class StandardXMLToHierarchicMap(WorkflowMap):
        ...
        for key in keys:
            ...
            for element in r['attributes']:
                if isinstance(element, basestring):
                    ...
                    fk, en, = element
                _type = get_standardxml_type_name(rows[0][fk])
                _value = unicode(record_set[0][fk])
                if record_set[0][fk] is not None:
                    SubElement(root, en, dataType=_type, ).text = _value
                else:
                    SubElement(root, en)
    ...
    def run(self, rfile, wfile):
        ...
        for keys, fields in StandardXML.Read_Rows(rfile, fallback_to_text=True):
            print keys, fields
            fields.update(keys)  # merge keys & fields
            row = fields
        ...
        wfile.write(tostring(document))  # YIKES!  This is sad.
