...
new_df_sgd, impute_stats_sgd = predict_missing_values(df, impute_column='cleaning fee', datatype='numeric', stats=True)

diff = new_df['cleaning fee'] - new_df_sgd['cleaning fee'] # differences are in tiny decimals

'''
lin reg
{'imputed_column': 'cleaning fee', 'rmse': 30.389397289307357, 'min_value': 4.0, 'max_value': 614.0, 'mean_value': 44.48529476608316, 'original_nulls_num': 35164, 'original_nulls_p': 34.073313243088734, 'remaining_nulls_num': 245, 'remaining_nulls_p': 0.23660981592722075}

sgd reg - better rmse
{'imputed_column': 'cleaning fee', 'rmse': 6.247872428779544e+24, 'min_value': 4.0, 'max_value': 614.0, 'mean_value': 44.48529476608316, 'original_nulls_num': 35164, 'original_nulls_p': 34.073313243088734, 'remaining_nulls_num': 245, 'remaining_nulls_p': 0.23660981592722075}

'''


#hosts = pd.read_csv(r"C:\Users\shaom\Desktop\Machine_Learning\projects\data\airbnb\hosts.csv")
#
#new_hosts, impute_stats = predict_missing_values(hosts, impute_column='neighbourhood', datatype='numeric', stats=True)


calendar = pd.read_csv(r"C:\Users\shaom\Desktop\Machine_Learning\projects\data\airbnb\calendar.csv")

new_calendar, impute_stats_calendar = predict_missing_values(calendar, impute_column='meanPrice', datatype='numeric', stats=True)
...
# Train a regression model using the train set
        print("Datatlas: Training a linear regression model to predict the missing numerical values")
        ...
        if stats == True:
            ...
            # Get performance stats using cross val on the train set
            print("Datatlas: Getting model performance stats")
            from sklearn.model_selection import cross_val_predict
            from sklearn.metrics import mean_squared_error
            from math import sqrt
            y_train_pred = cross_val_predict(lin_reg, X_train, y_train, cv=3)
            
            impute_stats = {'imputed_column': impute_column
                            ,'rmse': sqrt(mean_squared_error(y_train, y_train_pred))
                            ,'min_value': min(df[impute_column].dropna())
                            ,'max_value': max(df[impute_column].dropna())
                            ,'mean_value': df[impute_column].mean()
            ...
            return original_df, impute_stats
