...
class HTMLField(Field):
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, extra_context=None, **kwargs):
        ...
        for f in self.fields:
            context.update({'field': mark_safe(getattr(form.instance, f))})
...
class ButtonLink(LayoutObject):
    ...
    def render(self, form, form_style, context, template_pack):
        ...
        # TODO: REVER como pegar

        for i in self.parms:
            # e = 'form.instance.' + self.parms[i]
            try:
                # val = eval(e)
                val = eval(self.parms[i])
            except:
                # val = self.parms[i]
                # return HTML('').render(form, form_style, context, template_pack)
                return ''

            parms.update({i: getattr(form.instance, self.parms[i], val)})

        href = reverse(self.named_url, kwargs=parms)
        layout_object = HTML("""<a role="button" class="btn btn-default" href="{0}">{1}</a>""".format(href, form[self.field].label))
        return layout_object.render(form, form_style, context, template_pack)
        # return """<a role="button" class="btn btn-default" href="/#{0}"> Cancel</a>""".format(href)


class Link(LayoutObject):
    """
    Usado para mostrar um link para uma URL com parâmetros
