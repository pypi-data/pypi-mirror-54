...
def configuraButtonsNoForm(form, acao, goto, workitem=None):
    ...
    if acao in (ACAO_ADD, ACAO_EDIT, ACAO_WORKFLOW_START):
        ...
        fields = [Submit('save', i.condition, css_class="btn-danger btn-sm") for i in workitem.activity.transition_inputs.all()]
        form.helper.layout.extend([
            FormActions(
                Submit('save', 'Salvar', css_class="btn-primary btn-sm"),
...
def common_workflow_table_process(request, table=ProcessesInstanceTable, goto='index', my_work=False, news=False,
    ...
    query = ProcessInstance.objects.all_safe(user=request.user, my_work=my_work, news=news, pending=pending, subject=subject)
