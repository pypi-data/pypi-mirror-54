...
def dataTimestamps(mapId, Type, element, dataType, className = 'all classes', token = None):
...
def dataIds(mapId, Type, elementIds, dataType, timestampNumber, className = 'all classes', token = None):
...
def dataTiles(mapId, Type, timestampNumber, element, dataType, className = 'all classes', token = None):
    if Type == 'custom_polygon':
        element = gpd.GeoSeries([element]).__geo_interface__['features'][0]
