...
def simpleHotPlugBounceTest (uniqueID, quarchName, driveName, insertTime, bouncePattern, bounceSignals, repeats=1, onTime=10, offTime=5, bounceOnPlug=True, bounceOnPull=False):
    ...
    # Log start of test
    sendLogMessage (time.time(), "testDescription", "Starting (Bounce) Hotplug test at: " + str(insertTime) + "mS on device: " + driveName + ", over " + str(repeats) + " repetitions", os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId = uniqueID + "." + str(counter));
    ...
    # Track errors from this time step   
    if errorSubCounter == 0:
        sendLogMessage (time.time(), "testResult", "Hotplug test passed all repetitions", os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult":True}, uId = uniqueID + "." + str(counter));
    else:
        sendLogMessage (time.time(), "testResult", "Hotplug sweep test failed: " + str(errorSubCounter) + " sub-test points", os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult":False}, uId = uniqueID + "." + str(counter));
