...
def chooseQuarchModuleQPS(moduleName, myQps=None):
    ...
    if "DISABLED" in outMode:
        ...
        comms.newNotifyChoiceOption("qpsmodule", selection[1], connectionWithoutConType)
    ...
    return selection.strip() + "=" + outMode
...
def ActivateRemoteServer(portNumber=9742, localHost=False):
    ...
    try:
        ...
        if not dtsGlobals.validVersion:
            ...
            printText("QCS incompatible with this QuarchPy version. Restarting server")
            ...
            while continueScript:
                ...
                try:
                    ...
                    if myobj.moduleName == "driveTestCore":
                        ...
                        modulePointer = importlib.import_module("." + myobj.moduleName, "quarchpy.disk_test") #should disk_test be removed from this to allow tests to access all parts of QPy (UI, QIS etc)
...
def main(argstring):
    ...
    parser.add_argument('-l', '--logLevel', help='Logging level which will configure the severity of output', choices=["debug", "info", "warning", "error","critical"],default=None, type=str.lower)
    args = parser.parse_args(argstring)

    if args.logLevel is not None: setUpLogging(args.logLevel)
