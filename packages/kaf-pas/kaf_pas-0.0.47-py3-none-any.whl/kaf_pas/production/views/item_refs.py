...
from kaf_pas.production.models.item_refs import Item_refs
...
@JsonResponseWithException()
def Item_refs_Copy(request):
    return JsonResponse(DSResponse(request=request, data=Item_refs.objects.copyFromRequest(request=request), status=RPCResponseConstant.statusSuccess).response)
