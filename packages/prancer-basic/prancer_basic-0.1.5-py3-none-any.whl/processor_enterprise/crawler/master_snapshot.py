...
from processor.helper.json.json_utils import get_field_value, json_from_file,\
    ...
    collectiontypes, get_json_files, MASTERTEST, save_json_to_file
from processor.helper.config.config_utils import config_value, framework_dir
from processor.database.database import DATABASE, DBNAME, get_documents, sort_field, insert_one_document
...
def generate_container_mastersnapshots_filesystem(container):
    ...
    for snapshot_file in snapshot_files:
        ...
        if parts[-1] in snapshots and parts[-1] not in populated:
            ...
            snapshot_file_data = generate_snapshots_from_mastersnapshot_file(snapshot_file)
        ...
        for doc in docs:
            if doc['json']:
                ...
                if snapshot in snapshots and snapshot not in populated:
                    ...
                    snp_docs = get_documents(snp_collection, dbname=dbname, sort=snp_sort, query=snp_qry)
                    snp_json_data = {}
                    if snp_docs and len(snp_docs):
                        logger.info('Number of snapshot Documents: %s', len(snp_docs))
                        snp_json_data = snp_docs[0]['json']
