...
def gw_info(info, session=None):
    ...
    gw = gws[0] if len(gws) == 1 else find_gw(gws)
    gw_arn = gw["GatewayARN"]
    fss = session.SGWClient.list_file_shares(GatewayARN=gw_arn)["FileShareInfoList"]
    shares = session.SGWClient.describe_nfs_file_shares(FileShareARNList=[fs["FileShareARN"] for fs in fss])["NFSFileShareInfoList"]
    client_fs = next((share for share in shares if share["Path"] == info["clusterVolumes"]["rwx"]["nfs"]["path"]), None)
    indico_fs = next((share for share in shares if share["Path"] == info["clusterVolumes"]["rox"]["nfs"]["path"]), None)
