...
@click.option('-xml', '--xmlfile', type=click.Path(exists=True), help='abs path to ui file')
@click.option('-ob', '--objname', default='', type=str, help='object name of target container')
...
@click.option('-ppm', '--pp_mouse', default=False, is_flag=True, help='compile++ option - do the mouse clicks events')
@click.option('-ppk', '--pp_keys',  default=False, is_flag=True, help='compile++ option - do the keys events')
def cli(run, xmlfile, objname, nobadwidgets, outputfile, pp_mouse, pp_keys):
	...
	if run and not (xmlfile and objname):
		...
		if '/' not in xmlfile or '.' == xmlfile[0]:
			xmlfile = os.path.join(os.getcwd(), xmlfile)

		# objname = 'groupBox_2'
		# xmlfile = '/media/gogol/w/workss/psg/untitled.ui'

		try:
			psg_ui = just_compile({'objname': objname.strip('"').strip("'"),
								   'xmlfile': xmlfile.strip('"').strip("'"), 'no_bad_widgets': nobadwidgets})
			# compile++
			if pp_mouse:
