...
"""Default interval for how frequently results are written to influx database."""
INFLUXDB_STATS_INTERVAL_SEC = 5
...
# 新增函数，将定期产生的某些stats数据传入influxdb
def stats_influxdb(inf_obj, frequency):
    from . import runners
    while True:
        data = influx_stats(runners.locust_runner.request_stats)
        timestamp = time.time()
        influx_timestamp = int((timestamp + 8 * 60 * 60) * 1000 * 1000 * 1000)
        inf_obj.insert_data(influx_timestamp, **data)
        gevent.sleep(frequency)

# 新增函数，从EntryStats中拼装需要写入influxdb的数据
def influx_stats(stats):
    total_rps = 0.0
    total_reqs = 0
    total_failures = 0
    influx_data = {}
    ...
    for key in sorted(six.iterkeys(stats)):
        ...
        total_reqs += r.num_requests
        total_failures += r.num_failures
        request_name = r.method + ' ' + r.name
        request_name_rps = 'rps:' + request_name
        request_name_reqs = 'reqs:' + request_name
        request_name_fails = 'fails:' + request_name
        influx_data[request_name_rps] = r.current_rps
        influx_data[request_name_reqs] = r.num_requests
        influx_data[request_name_fails] = r.num_failures
    influx_data['rps:total'] = total_rps
    influx_data['reqs:total'] = total_reqs
    influx_data['fails:total'] = total_failures
