...
class PlugInMgr:
    ...
    #def loadPlugin(self,
def set_mapper_file(self, mappername):
#    def set_mapper_file(self, mappernameddd):
    """
    set the mapper file location and import it.

    (Only needed if you are not using the command line functionality.)

    Args:
        mappername (string): The base filename (**no extension**) of the mapper
            file to use. (eg fft_mapper, not fft_mapper.py)


    # Example
        index_file = MakeIndex()
        cmd_options = parse_cmd()
        index_file.set_mapper_file("test_mapper")

    Note:

        If the file can not be located, the application will abort.
    """
    modpath, modfilename = os.path.split(mappername) # split to path and name
    modfilename = os.path.splitext(modfilename)[0] # remove extension
    print("Loading Mapper File: ", modfilename)
    sys.path.append(modpath) # Add folder the mapper is in to the python path
    try:
        self.MapperFile = importlib.import_module(modfilename)#(mappername)
    except ModuleNotFoundError:
        print("Unable to load mapper file(%s)" % mappername)
        sys.exit(1)
        #raise Exception("Unable to load mapper (%s)" % fqpn)
