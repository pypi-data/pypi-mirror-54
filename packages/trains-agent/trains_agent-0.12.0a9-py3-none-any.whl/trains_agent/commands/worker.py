...
import psutil
import six
from trains_agent.backend_api.services.v2_3 import queues as queues_api
from trains_agent.backend_api.services.v2_3 import tasks as tasks_api
...
from trains_agent.helper.package.requirements import (
    RequirementsManager,
    CaffeRequirement,
    LightnetRequirement,
)
...
class Worker(ServiceCommandSection):
    ...
    def run_one_task(self, queue, task_id, worker_args):
        ...
        if self.docker_image:
            docker_cmd = copy(self.docker_image)
            docker_cmd[-1] = docker_cmd[-1] + 'execute --disable-monitoring --id ' + task_id
            cmd = Argv(*docker_cmd)
        else:
    ...
    def run_tasks_loop(self, queues, worker_params):
        ...
        while True:
            ...
            for queue in queues:
                ...
                print(
                    "No tasks in Queues, sleeping for {:.1f} seconds".format(
    ...
    def daemon(self, queues, log_level, foreground=False, docker=False, **kwargs):
        ...
        if docker is not False and docker is not None:
            temp_config, docker_image = self.get_docker_config_cmd(docker)
    ...
    def send_logs(self, task_id, lines):
        """
        ...
        try:
            events_service.send_log_events(
                self.worker_id, task_id=task_id, lines=lines, level="DEBUG"
            )
            return len(lines)
        except Exception as e:
            print("\n### Error sending log: %s ###\n" % e)
    ...
    def freeze_task_environment(self, task_id=None):
        ...
        if not task_id:
            return freeze
        request = tasks_api.SetRequirementsRequest(task=task_id, requirements=freeze)
        try:
    ...
    def get_docker_config_cmd(self, docker_args):
        ...
        if len(docker_arguments) > 1:
            ...
            docker_arguments = self._session.config.get("agent.default_docker.arguments", None)
        python_version = '3'
        ...
        host_cache = Path(self._session.config["sdk.storage.cache.default_base_dir"]).expanduser().as_posix()
        host_pip_dl = Path(self._session.config["agent.pip_download_cache.path"]).expanduser().as_posix()
        host_vcs_cache = Path(self._session.config["agent.vcs_cache.path"]).expanduser().as_posix()
        ...
        host_apt_cache = Path(self._session.config.get("agent.docker_apt_cache", '~/.trains/apt-cache')).expanduser().as_posix()
        host_pip_cache = Path(self._session.config.get("agent.docker_pip_cache", '~/.trains/pip-cache')).expanduser().as_posix()
        host_ssh_cache = mkdtemp(prefix='trains_agent.ssh.')

        docker_cmd = self._get_docker_cmd(worker_id=self.worker_id, docker_image=docker_image,
    ...
    def _get_docker_cmd(worker_id, docker_image, docker_arguments,
        ...
        # copy the .ssh folder to the temp folder
        try:
        ...
        if docker_arguments:
            base_cmd += list(docker_arguments) if isinstance(docker_arguments, (list, tuple)) else [docker_arguments]
