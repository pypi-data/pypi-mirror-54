...
class RequirementsManager(object):
    ...
    def get_cuda_version(config):  # type: (ConfigTree) -> (Text, Text)
        cuda_version = os.environ.get('CUDA_VERSION') or config['agent.cuda_version']
        cudnn_version = os.environ.get('CUDNN_VERSION') or config['agent.cudnn_version']
...
class CaffeRequirement(CudaSensitiveSubstitution):

    name = 'seematics.caffe'


class LightnetRequirement(CudaSensitiveSubstitution):
