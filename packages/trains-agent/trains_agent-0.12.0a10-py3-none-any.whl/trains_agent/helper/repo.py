...
class VCS(object):
    ...
    def _set_ssh_url(self):
        ...
        if not getenv(ssh_agent_variable) or (self.session.config.get('agent.git_user') and
                                              self.session.config.get('agent.git_pass')):
            new_url = self.resolve_ssh_url(self.url)
            if new_url != self.url:
                print("replacing ssh url '{}' with https url '{}'".format(self.url, new_url))
                self.url = new_url
    ...
    def add_auth(cls, config, url):
        ...
        config_user = config["agent"]["{}_user".format(cls.executable_name)]
        config_pass = config["agent"]["{}_pass".format(cls.executable_name)]
        if (
