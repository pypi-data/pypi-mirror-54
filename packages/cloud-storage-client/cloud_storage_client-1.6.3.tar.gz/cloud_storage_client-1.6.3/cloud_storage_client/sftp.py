...
class SFTPClient():
  ...
  def __init__(self, host, port, username, password, secret_key=None):
    ...
    if secret_key is None:
      ...
      print('key file created in {}!'.format(private_key_path))
      ...
      if len(agent_keys) == 0:
        print('There is no keys in the agent, trying to add the key directly with the agent:')
        command = 'ssh-add {}'.format(private_key_path)
        ...
        agent_keys = paramiko.Agent().get_keys()
        if len(agent_keys) == 0:
          print('It didnt work as well, trying setting directly the key file')
          agent_keys = [private_key_path]

      self.client = pysftp.Connection(host=host, username=username, private_key=agent_keys[-1],
                                      private_key_pass=password, cnopts=cnopts)
