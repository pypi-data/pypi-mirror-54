...
function GetOciSubcommands_waas() {
    $ociSubcommands = @{
        'waas' = 'access-rule address-list address-rate-limiting caching-rule captcha certificate custom-protection-rule device-fingerprint-challenge edge-subnet good-bot human-interaction-challenge js-challenge policy-config protection-rule protection-settings purge-cache recommendation threat-feed waas-policy waf-blocked-request waf-config waf-log waf-request waf-traffic-datum whitelist work-request'
        'waas access-rule' = 'list update'
        'waas address-list' = 'change-compartment create delete get list'
...
function GetOciCommandsToLongParams_waas() {
    $ociCommandsToLongParams = @{
        ...
        'waas policy-config update' = 'certificate-id cipher-group client-address-header force from-json help if-match is-behind-cdn is-cache-control-respected is-https-enabled is-https-forced is-origin-compression-enabled is-response-buffering-enabled max-wait-seconds tls-protocols waas-policy-id wait-for-state wait-interval-seconds'
