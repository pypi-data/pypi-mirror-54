...
class Cursor(object):
    ...
    def _get_fields(self):
        ...
        # TODO: remove this once apiserver returns both field names and types
        for r in results:
            self._schema.update(r)

        # make a list of fields and types, and return
        fields = self._cursor.fields()

        # handle case when fields is None - eg: 'select *'
        fields = fields or [{'name': k} for k in sorted(self._schema)]

        self._fields = [
            (f['name'], type(self._schema[f['name']]).__name__) for f in fields
        ]
...
BINARY = _DBAPITypeObject(['object', 'array'])
NUMBER = _DBAPITypeObject(['int', 'float'])
DATETIME = _DBAPITypeObject('date', 'time', 'timestamp', 'datetime')
