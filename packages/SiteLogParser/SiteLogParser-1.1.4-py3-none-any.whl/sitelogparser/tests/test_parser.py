...
class TestSitelogParser(unittest.TestCase):
    ...
    def setUp(self):
        self.test_log = "SCOR"
        sitelog_file = os.path.join(os.path.dirname(__file__), TEST_DATA[self.test_log]["file"])
        self.parser = None
    ...
    def test_antennas(self):
        self.assertEqual(len(self.sitelog_data["sitelog"]["gnss_antennas"]["list"]), TEST_DATA[self.test_log]["antennas"])

    def test_receivers(self):
        self.assertEqual(len(self.sitelog_data["sitelog"]["gnss_receivers"]["list"]), TEST_DATA[self.test_log]["receivers"])
