...
def tree_classfier_two_hyperparameter_tuning(X_train, y_train,
    ...
    dtree = classifier_algo
    grid_search = GridSearchCV(estimator = dtree, param_grid = param_grid, cv = cv_folds, verbose = 1, refit=refit, return_train_score=True, scoring=model_scoring)
...
def kesh_label_encoder_fit_transform(train_df, test_df=pd.DataFrame(), binary_classification_target_column='', columns_to_treat=[], verbose=False, useCrossTabRatio=False):
    
    if (binary_classification_target_column=='') :
        raise Exception('binary_classification_target_column is not provided')
    ...
    if len(columns_to_treat)==0: # Apply for all categorical
        ...
        columns_to_treat = all_columns
        
    columns_to_treat = train_df[columns_to_treat].select_dtypes(include=['category', object]).columns.tolist()
    
    for a_column_to_treat in columns_to_treat:
        if verbose:
            print(a_column_to_treat)
        ct=pd.crosstab(train_df[a_column_to_treat], train_df[binary_classification_target_column])
        cross_df=ct.div(ct.sum(1).astype(float), axis=0)
        cross_df=cross_df.sort_values(cross_df.columns[0])        
        ...
        for aVal in value_order:
            if verbose:
                print('Replacing [' + str(aVal) + '] with '+str(cat_label))
            if (useCrossTabRatio==False):
                mapping_labels[aVal] = cat_label
                cat_label = cat_label+1
            else:
                mapping_labels[aVal] = cross_df[1][aVal]
            
        
        if (test_df.shape[0]>0) :
            train_unique_values = list(train_df[a_column_to_treat].unique())
            test_unique_values = list(test_df[a_column_to_treat].unique())
            remaining_test_values =  [i for i in test_unique_values if not i in train_unique_values]
            
            for aVal in remaining_test_values:
                if verbose:
                    print('Replacing test extra label [' + str(aVal) + '] with '+str(cat_label))
                if (useCrossTabRatio==False):
                    mapping_labels[aVal] = cat_label
                    cat_label = cat_label+1
                else :
                    mapping_labels[aVal] = 0.0001
                
        train_df[a_column_to_treat] = train_df[a_column_to_treat].map(mapping_labels)
        if (test_df.shape[0]>0) :
            test_df[a_column_to_treat] = test_df[a_column_to_treat].map(mapping_labels)
    if (test_df.shape[0]>0) :
