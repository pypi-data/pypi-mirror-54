    ...
    def __onNewTopic(self,node_request):
        ...
        if node_request["socket"] == "publisher" or node_request["socket"] == "subscriber":
            ...
            if "msg_type" in node_request:
                self.topic_register.update({topic:{"port":self.current_port,"socket":node_request["socket"], "mode":node_request["mode"], "msg_type":node_request["msg_type"] }})
            else:
                 self.topic_register.update({topic:{"port":self.current_port,"socket":node_request["socket"], "mode":node_request["mode"], "msg_type":"json" }})
