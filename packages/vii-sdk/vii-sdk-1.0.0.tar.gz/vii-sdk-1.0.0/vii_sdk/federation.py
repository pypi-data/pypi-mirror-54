...
from vii_sdk.utils import is_valid_address
from .exceptions import FederationError, ViiAddressInvalidError


def federation(address_or_id, fed_type='name', domain=None, allow_http=False):
    """Send a federation query to a Vii Federation service.

    For more info, see the `complete guide on Vii Federation.
    ...
    :param str domain: The domain that corresponds to the address or ID;
        this is where the Vii.toml file lives (which ultimately points to
...
def get_federation_service(domain, allow_http=False):
    """Retrieve the FEDERATION_SERVER config from a domain's Vii.toml.
    ...
    """
    st = get_vii_toml(domain, allow_http)
    if not st:
        return None
...
def get_auth_server(domain, allow_http=False):
    """Retrieve the AUTH_SERVER config from a domain's Vii.toml.
...
def get_vii_toml(domain, allow_http=False):
    """Retrieve the Vii.toml file from a given domain.

    Retrieve the Vii.toml file for information about interacting with
    Vii's federation protocol for a given Vii Anchor (specified by a
    domain).

    :param str domain: The domain the .toml file is hosted at.
    :param bool allow_http: Specifies whether the request should go over plain
        HTTP vs HTTPS. Note it is recommend that you *always* use HTTPS.
        :return: The vii.toml file as a an object via :func:`toml.loads`.

    """
    toml_link = '/.well-known/vii.toml'
    if allow_http:
        protocol = 'http://'
    else:
        protocol = 'https://'
