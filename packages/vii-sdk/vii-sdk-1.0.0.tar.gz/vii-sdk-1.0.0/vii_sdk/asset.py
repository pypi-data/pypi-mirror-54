...
from vii_sdk.exceptions import AssetCodeInvalidError, ViiAddressInvalidError
...
class Asset(object):
    ...
    corresponding issuer on the Vii network.

    For more information about the formats used for asset codes and how issuers
    work on Vii's network, see `Vii's guide on assets`_.

    :param str code: The asset code, in the formats specified in `Vii's
        guide on assets`_.
    :param issuer: The strkey encoded issuer of the asset. Note if the
        currency is the native currency (VII), no issuer is necessary.
    :type issuer: str, None

    .. _Vii's guide on assets:
        https://www.vii.vip/developers/guides/concepts/assets.html
    ...
    def __init__(self, code, issuer=None):
        ...
        if issuer is not None:
            try:
                ...
                raise ViiAddressInvalidError('Invalid issuer account: {}'.format(issuer))

        if code.lower() != 'vii' and issuer is None:
            raise ViiAddressInvalidError("Issuer cannot be `None` except native asset.")
    ...
    def guess_asset_type(self):
        if self.code.lower() == 'vii' and self.issuer is None:
            asset_type = 'native'
    ...
    def native():
        ...
        Currently, the native currency is VII
