...
from .exceptions import ViiAddressInvalidError, NotValidParamError
...
class Operation(object):
    ...
    An operation is an individual command that mutates Vii's ledger. It is
    ...
    For more on operations, see `Vii's documentation on operations
    <https://www.vii.vip/developers/guides/concepts/operations.html>`_ as
    well as `Vii's List of Operations
    <https://www.vii.vip/developers/guides/concepts/list-of-operations.html>`_,
    ...
    def to_xdr_amount(value):
        ...
        See `Vii's documentation on Asset Precision
        <https://www.vii.vip/developers/guides/concepts/assets.html#amount-precision-and-representation>`_
...
class CreateAccount(Operation):
    ...
    :param str starting_balance: Amount in VII the account should be
        funded for. Must be greater than the [reserve balance amount]
        (https://www.vii.vip/developers/learn/concepts/fees.html).
    :param str source: The source account for the payment. Defaults to the
        transaction's source account.
...
class PathPaymentStrictReceive(Operation):
    ...
    of offers. This allows the asset sent (e.g., 450 VII) to be different from
...
class ChangeTrust(Operation):
    ...
    <https://www.vii.vip/developers/guides/concepts/assets.html>_`.
...
class SetOptions(Operation):
    ...
    doc <https://www.vii.vip/developers/guides/concepts/multi-sig.html>`_.
    ...
    def __init__(self,
        ...
        if self.signer_address is not None and self.signer_type is None:
            try:
                ...
                raise ViiAddressInvalidError('Must be a valid vii address if not give signer_type')
...
class CreatePassiveSellOffer(Operation):
    ...
    CreatePassiveSellOffer operation on Vii's network.
    ...
    example, if an offer exists to buy 5 BTC for 30 VII, and you make a passive
    sell offer to buy 30 VII for 5 BTC, your passive sell offer does not take the first
    offer.
...
class AccountMerge(Operation):
    ...
    AccountMerge operation on Vii's network.

    Transfers the native balance (the amount of VII an account holds) to
...
class Inflation(Operation):
    ...
    Inflation operation on Vii's network.
...
class ManageData(Operation):
    ...
    ManageData operation on Vii's network.
...
class BumpSequence(Operation):
    ...
    BumpSequence operation on Vii's network.
