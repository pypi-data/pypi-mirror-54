...
from .exceptions import DecodeError, ConfigurationError, MnemonicError, ViiAddressInvalidError, \
    ViiSecretInvalidError, NotValidParamError, NoApproximationError
...
versionBytes = {
    'account': binascii.a2b_hex('a8'),  # V 168 21 << 3
    'seed': binascii.a2b_hex('40'),  # I 64 8 << 3
...
class ViiMnemonic(Mnemonic):
    def __init__(self, language='english'):
        self.radix = 2048
        self.vii_account_path_format = "m/44'/148'/%d'"
    ...
    def derive(self, seed, index):
        ...
        path = self.vii_account_path_format % index
...
def is_valid_address(address):
    try:
        ...
        raise ViiAddressInvalidError('Invalid Stellar Address: {}'.format(address))
...
def is_valid_secret_key(secret):
    try:
        ...
        raise ViiSecretInvalidError('Invalid Stellar Secret: {}'.format(secret))
