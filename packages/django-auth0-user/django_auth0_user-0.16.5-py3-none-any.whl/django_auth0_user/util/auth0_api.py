...
def setup_auth0_rules(dry_run=True):
    ...
    logger.debug(f'Current rules: {current_rule_names}')

    settings_rules = AUTH0_RULES
    settings_rule_names = set(settings_rules.keys())
    logger.debug(f'Rule names defined in settings: {settings_rule_names}')
    ...
    logger.debug(f'Rules to create: {rules_to_create}')
    ...
    logger.debug(f'Rules to check: {rules_to_check}')
    rules_to_update = set()
    ...
    for rule_name in rules_to_update:
        logger.debug(f'Rules to update: {rules_to_update}')
        if not dry_run:
            rule_id = current_rules[rule_name]['id']
            auth0.rules.update(rule_id, {
                'script': settings_rules[rule_name]['script'],
                'name': rule_name,
                'order': settings_rules[rule_name]['order'],
                'enabled': settings_rules[rule_name]['enabled'],
            })
