...
class Conv2d(Layer):
    ...
    def _get_output_size(self, x):
        ...
        out_w = (w + pw * 2 - kw) // sw + 1
        return out_h, out_w

    def __call__(self, x):
        n, c, h, w = x.shape
        out_h, out_w = self._get_output_size(x)

        x = F.im2col(x, self.kernel_size, self.stride, self.pad)
        W = F.reshape(self.W, (self.W.shape[0], -1))
        W = F.transpose(W)
