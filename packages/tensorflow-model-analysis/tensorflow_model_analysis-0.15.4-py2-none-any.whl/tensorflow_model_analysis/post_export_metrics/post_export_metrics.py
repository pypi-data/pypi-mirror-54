...
from tensorflow_model_analysis.types_compat import Any, Dict, List, Optional, Text, Tuple, Type, Callable
...
def _check_feature_present(features_dict,
                           feature_key):
...
def _build_auc_metrics_ops(metric_key, labels,
                           predictions,
                           weights, num_thresholds,
                           curve):
...
def _string_labels_to_class_ids(labels_tensor,
                                classes_tensor):
...
class _PostExportMetric(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def populate_stats_and_pop(
      self, slice_key, combined_metrics,
      output_metrics):
...
class _ExampleCount(_PostExportMetric):
  ...
  def populate_stats_and_pop(
      self, unused_slice_key, combine_metrics,
      output_metrics):
...
class _Calibration(_PostExportMetric):
  ...
  _example_weight_key = Ellipsis  # type: Text
  _target_prediction_keys = Ellipsis  # type: List[Text]
  _labels_key = Ellipsis  # type: Text
  _metric_tag = None  # type: Text
  _tensor_index = Ellipsis  # type: int
...
class _PrecisionAtK(_PrecisionRecallAtK):
  ...
  def populate_stats_and_pop(  # pylint: disable=useless-super-delegation
      self, slice_key, combine_metrics,
      output_metrics):
