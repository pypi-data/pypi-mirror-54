...
def tf_apply(project, auto_approve=None):
    ...
    tf = Terraform(working_dir='.')
    if auto_approve:
        tf.apply(capture_output=False, auto-approve=True, var=get_hetzner_api_key_as_dict(project))
    else:
...
def tf_destroy(project, auto_approve=None):
    tf = Terraform(working_dir='.')
    if auto_approve:
        tf.destroy(capture_output=False, auto-approve=True, var=get_hetzner_api_key_as_dict(project))
    else:
