...
class SentencePieceVocabulary(object):
  ...
  def __init__(self, sentencepiece_model_file, extra_ids=0):
    ...
    sp_model = tf.gfile.GFile(sentencepiece_model_file, "rb").read()
    self._tokenizer.LoadFromSerializedProto(sp_model)
    self._tf_tokenizer = tf_text.SentencepieceTokenizer(model=sp_model)
